...
| **MassRelations.CartanFlavonPotential** (Round 22 Claim C sub-(ii)) | Z_6 × Z_16-invariant flavon potential `V(φ_1, φ_2) = -a·cos(6 φ_1) - b·cos(16 φ_2)` on SU(3)_flavor Cartan torus T².  Theorem `fn_vevs_are_potential_minima` proves that the Round-21 FN flavon VEVs `(-π/3, -π/8)` are global minima.  Z_6 of φ_1 directly ties to Claim A's SU(3) Cartan structure (generator π/3 = 2 · π/6); Z_16 of φ_2 has leading candidate SO(10) 16-spinor R-symmetry origin. |
| **MassRelations.Z2OrbifoldDepth** (Round 23 Claim C sub-(i)) | Z_2-orbifold-depth function `binaryTreeDepth g := 2^(g-1)`, matching the FN-1 charge magnitude `-Δq^(1)_g`.  Theorem `binaryTreeDepth_matches_FN_charge_magnitude` proves the orbifold-depth interpretation is consistent with the Round-21 FN charge assignment.  Suggests heterotic-string Z_2^n orbifold compactification as candidate UV origin. |
| **MassRelations.HeavyFermionTower** (Round 24 Claim C Candidate 2) | Non-perturbative heavy-fermion-tower with instanton action `S_g = (π/3)·2^(g-1) + π/8`.  Theorem `tower_eq_FN` proves EFT-duality with the Round-21 FN-doubled model — they are two equivalent formulations of the same UV physics.  Theorem `tower_eq_TT` then connects to TT via the FN equivalence. |
