...
| **Round 35 / Priority 8 Phase C: end-to-end physical-mass Lean bridge** (upgrades `UpLeptonFormulaHolds` / `DownRationalFormulaHolds` / Koide from `True → trivial` placeholders to real theorems on Lean-valued physical masses; closes 02_SPEC §D.4) | MassRelations.PhysicalMasses | TT_formula_holds_on_physical, VV_formula_holds_on_physical, koide_identity_holds_on_physical, predicted_leptons_fixed_by_newton_flow (+ 4 positivity lemmas) | ✓ |
