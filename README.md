...
| **Spacetime** | 41 | Geodesic theorem, mass gap, orbit mass hierarchy, QEC stabilizer, GH convergence of CMCA spatial graphs, intrinsic area-scaling cut measure |
...
- Intrinsic area-scaling cut measure: the number of spacelike causal edges crossing a coordinate cut scales exactly as L², with zero metric and zero continuum geometry — first as a proof of concept on the certified causal graph (`cut_crossing_finset_card_eq_L_sq`), then ported to and proved for the actual three-tape ℤ₇ CMCA substrate under the reading confirmed by its certified `7^(3L)` state-space cardinality (`three_tape_cut_crossing_card_eq_L_sq`)
...
- QR(7)→octonion interface: the UGP color sector is generated by the (7,3,1) difference set QR(7) = {1,2,4}; F₂₁ acts by octonion automorphisms (F₂₁ ↪ G₂) and the pencil count |QR(7)| = 3 = N_c is intrinsic to the design (`weights_eq_D`, `f21_acts_by_octonion_automorphisms`, `pencil_card_eq_Nc`)
- G₂/su(3) stabilizer certificates: derivation algebra dimension 14 (g₂) and apex stabilizer dimension 8 (su(3)) certified by exact integer arithmetic — Bareiss determinants, rank minors, bracket closure, and negative-definite Killing form — all by `native_decide`; Cartan–Killing classification cited, not formalized
