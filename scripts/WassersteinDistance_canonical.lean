...
private theorem couplingTransportCost_eq_zero_of_eq
  ...
  have hdiag : γ x x = μ.val x := by rw [← hsumx, hγ.2.2.2.1 x hx]
...
private theorem delta_three_anchor_contradiction
  ...
  · have hmul : d0u + d1u = d01 := by
      have heq := congr_arg (fun t => t * d01) hcoef
      field_simp [ne_of_gt h01] at heq
      linarith
    rw [hδ0, hδ1] at e2
    field_simp [ne_of_gt h01] at e2
    nlinarith [h0, h1, hu, h0u, h1u, h01, hmul, sq_nonneg δu]
...
private theorem probExpectation_dist_eq_all_imp_vertex_eq (M : FiniteMetricSpace)
  ...
  · have htPlus : t0 ∈ M.vertices.filter (fun t => 0 < δ t) :=
    ...
    · have hthirdEx := hthird
      ...
      set rest := ((M.vertices.erase t0).erase tMinus).erase u
      have hmemVert {t : ℕ} (ht : t ∈ rest) : t ∈ M.vertices :=
        Finset.mem_of_mem_erase (Finset.mem_of_mem_erase (Finset.mem_of_mem_erase ht))
      have htne0 {t : ℕ} (ht : t ∈ rest) : t ≠ t0 :=
        Finset.ne_of_mem_erase (Finset.mem_of_mem_erase (Finset.mem_of_mem_erase ht))
      have htneM {t : ℕ} (ht : t ∈ rest) : t ≠ tMinus :=
        Finset.ne_of_mem_erase (Finset.mem_of_mem_erase ht)
      have htneU {t : ℕ} (ht : t ∈ rest) : t ≠ u :=
        Finset.ne_of_mem_erase ht
      have hsum3 : δ t0 + δ tMinus + δ u = 0 := by
        have hrest : rest.sum δ = 0 :=
          Finset.sum_eq_zero fun t ht => hδ0 t (hmemVert ht) (htne0 ht) (htneM ht) (htneU ht)
        have hdecomp : M.vertices.sum δ =
            δ t0 + δ tMinus + δ u + rest.sum δ := by
  ...
  · push_neg at ht0pos
    ...
    · have hthirdEx := hthird
      ...
      set rest := ((M.vertices.erase tPlus).erase t0).erase u
      have hmemVert {t : ℕ} (ht : t ∈ rest) : t ∈ M.vertices :=
        Finset.mem_of_mem_erase (Finset.mem_of_mem_erase (Finset.mem_of_mem_erase ht))
      have htneP {t : ℕ} (ht : t ∈ rest) : t ≠ tPlus :=
        Finset.ne_of_mem_erase (Finset.mem_of_mem_erase (Finset.mem_of_mem_erase ht))
      have htne0 {t : ℕ} (ht : t ∈ rest) : t ≠ t0 :=
        Finset.ne_of_mem_erase (Finset.mem_of_mem_erase ht)
      have htneU {t : ℕ} (ht : t ∈ rest) : t ≠ u :=
      ...
      have hsum3 : δ tPlus + δ t0 + δ u = 0 := by
        have hrest : rest.sum δ = 0 :=
          Finset.sum_eq_zero fun t ht => hδ0 t (hmemVert ht) (htneP ht) (htne0 ht) (htneU ht)
        have hdecomp : M.vertices.sum δ =
            δ tPlus + δ t0 + δ u + rest.sum δ := by
