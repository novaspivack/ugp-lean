...
- `UgpLean.Gravity.PhiMDLProperTimeBridge` — L1→L2 bridge for τ=3/7: connects `EtherProperTimeRate.tauProper` (Rule 110 ether, CatAD) to the temporal voxel CC coefficient 9/112 and Ω_Λ=3π/14 at Level-2 (Φ_MDL); `phimdl_tau_from_ether`, `phimdl_voxel_coeff_from_ether_tau`, `phimdl_omega_lambda_from_ether_dynamics`, `l1_to_l2_tau_bridge`; 082-TAU-ACTION: `phimdl_tau_action_coupling_sq`, `phimdl_tau_sq_from_ether`, `phimdl_cc_coefficient_factored`, `tau_action_coupling_catad` (τ²=9/49 action coupling, CatAD, zero sorry, 2026-05-30)
...
- `UgpLean.Substrate.CMCAHilbertFockBridge` — EPIC_080 G22: `fock_vacuum_maps_to_cmca_vacuum`, `bps_psc_sector_has_beable_lift`, `cmca_hilbert_inductive_limit`, `cmca_hilbert_fock_bridge_master` (CatAL, zero sorry; 2026-05-29)
- `UgpLean.Substrate.CogwheelDynamicsG21` — G21 Level 1 discrete Schrödinger (ugp-lean canonical): `discrete_schrodinger_composition`, `discrete_schrodinger_step_recurrence` (CatAL, zero sorry); `discrete_evolution_unitary`, `f_mdl_cycle_hamiltonian_vacuum`, `f_mdl_information_loss_convergence`, `kg_factors_to_chiral_schrodinger` (CatAD); Level 2 gates on G42/G26 (2026-05-29)
...
- `UgpLean.Gravity.PMDLGravityTheorems` — EPIC_079 Ranks 079-MDL-UNIQUE, 079-FIXED-POINT, 079-UNIFIED-POLY: `vacuum_unique_fixed_point_z7`, `unique_cubic_gravity_coupling`, `gte_gravity_mass_hierarchy`, `gte_polynomial_three_roles_k_zero`; G46 Hawking kink thresholds: `kink_crit_mass_formula`, `kink_over_hawking_temp_ratio`, `kink_bose_enhanced_at_Mcrit` (CatAD, zero sorry, 2026-05-29)
...
- `UgpLean.Gravity.CMBSpectralTilt` — EPIC_078/G33 CMB-LEAN-1: `beta_g_z2_formula`, `n_s_formula`, `cmca_z2_sublayer_spectral_tilt`, `cmb_spectral_index_equals_gte_prediction`, `z2_eft_predicts_cmb_tilt` (theorem, CatAL); CMB spectral tilt n_s = 1 − ln(2)/(2π²); zero sorry, zero axioms (2026-05-29)
...
- `UgpLean.Algebra.F21SU3Embedding` — EPIC_080 Ranks 080-SU3-CONTINUUM / 080-SU3-BURNSIDE-LEAN: F₂₁↪SU(3) embedding CatAL zero sorry; explicit `rhoA`/`rhoB`/`omega7`; `matrix_algebra_finrank_nine` CatAL; Burnside span/commutant named axioms CatAD (`f21_burnside_density_certificate`, `f21_su3_continuum_master`) (2026-05-29)
- `UgpLean.Algebra.SMGaugeGroup` — EPIC_080 G23: `sm_gauge_group_certificate` bundles SU(3)×SU(2)_L×U(1)_Y from Z₇ winding (CatAL U(1)_Y/SU(3) arithmetic + CatAD SU(2)_L gauging axiom); zero sorry (2026-05-29)
- `UgpLean.Algebra.SRRGCABridge` — EPIC_080 Rank 080-MDLSRRG-LEAN (partial): `srrg_equals_mdl_minimization`, `ew_scale_log_decomposition`, `L_EW_log_decomposition`, `srrg_mdl_bridge_master` CatAL zero sorry; K_CMCA/β identification deferred (2026-05-29)
