...
framework in \lean{}~4 against Mathlib, comprising 143 modules with a
...
A new \textbf{Framework} layer (\lstinline|GTEFrameworkInstance|) instantiates
the GTE-Möbius substrate as a \texttt{NemS.Framework} with \texttt{DiagonalCapable}
and \texttt{PSCBundle}, firing \lstinline|transputation_classification| to yield
\lstinline|gte_tpc_from_nems_classification| (zero \texttt{sorry}, one Cook-bridge axiom).
...
The artifact is organized into 143~\lean{} modules across thirteen layers
...
\node[layer, fill=pink!10,  below=of self]  (fwk)
  {\textbf{Framework}~(1)\quad \textbf{GTEFrameworkInstance}
   (NemS + transputation classification on GTE substrate)};
...
(query~$0$ tests vacuum membership; all other queries are trivially true).
Observational equivalence therefore reduces to agreement on the vacuum query.

A canonical selector \lstinline|GTESelector| collapses each observational class to
either the vacuum representative \lstinline|fmdl_vacuum5| or the gen-1 canonical state
\lstinline|fmdl_gen1_z7|, proved idempotent and observationally inverse.
Non-categoricity is witnessed by \lstinline|gte_not_categorical|: vacuum and gen-1
agree on all queries except query~$0$.
