...
framework in \lean{}~4 against Mathlib, comprising 119 modules with a
...
The artifact is organized into 119~\lean{} modules across twelve layers:
...
\node[layer, fill=cyan!6,   below=of mass]  (braid)
  {\textbf{BraidAtlas}~(7)\quad ChargeTheorem, CompositeTriples, ChiralitySquaring, ChargeDerivation,
   ...
   \textbf{PSC}~(2)\quad RCCInfiniteFamilies, ThreeRouteForcing};
\node[layer, fill=lime!6,   below=of braid] (null)
  {\textbf{NullDiscipline}~(2)\quad SaturationBarrier, TheoremEligibility};
\node[layer, fill=violet!6, below=of null]  (ipt)
  {\textbf{IPT}~(1)\quad InformationProfitThreshold};
...
biconditional; see \S\ref{sec:coxeter_biconditional}).
...
field-embedding version is flagged as future work requiring Mathlib's
\texttt{IsCyclotomicExtension} infrastructure).
...
biconditional $h \mid 60 \iff 2h \mid 120$.  Together, the three modules
\texttt{CoxeterConductor}, \texttt{CoxeterConductorTowerLaw}, and
\texttt{CoxeterBiconditional} form a complete arithmetic + degree-theoretic
certification of the $\mathbb{Q}(\zeta_{120})$ filter.
