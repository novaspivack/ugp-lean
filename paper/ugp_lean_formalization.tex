...
(\emph{\ugp{} primes}, OEIS~A394412) with ten machine-checked instances.
The artifact comprises 44~\lean{} modules, builds against Mathlib
v4.29.0-rc3, and maintains a strict \emph{zero-sorry, zero-custom-axiom}
policy on the core proof path.  Novel contributions include: a
...
proof that the divisor count $\tau(2^n-16)$ is unbounded, and an
...
\item \textbf{\ugp{} primes} (\S\ref{sec:ugpprimes}): A new class of
  primes (OEIS~A394412~\cite{OEISA394412}) with a formal predicate, an
  existence proof, and ten machine-checked instances across three ridge
  levels.
...
\S\ref{sec:conjectures} resolves seven conjectures and states three open ones.
...
The artifact is organized into 44~\lean{} modules across seven layers:
...
\node[layer, fill=orange!8] (gte)   at (0,3*\vs)
  {\textbf{GTE}~(8)\quad Evolution, Orbit, UpdateMap,
   GeneralTheorems, MersenneGcd, PrimeFactorAnalysis,
   MirrorDualConj, UGPPrimes};
...
\paragraph{OEIS sequence.}
The \ugp{} primes form OEIS sequence
A394412~\cite{OEISA394412}.  Exhaustive computation finds 57~terms up to
$10^8$.  The first terms are $823, 1637, 2137, 6977, 9007, 17033,
27817, 30391, \ldots$\  The density of \ugp{} primes among all primes up
to~$N$ falls faster than $1/\ln N$; empirically the count grows as
$C \cdot N / (\ln N)^5$ with $C \approx 30$.
...
in ugp-lean; three remain open.  \Cref{tab:conjectures} summarizes
the status.
...
Three conjectures remain open.  All are formally stated as \lean{}
propositions.
...
The expected waiting time for a sign change in the M\"obius function
along a linear progression $\alpha t + \beta$ with $\gcd(\alpha,\beta)=1$
...
\begin{table}[ht]
\centering
\caption{Conjecture status.  Seven proved, three open.}
...
$\mu$-flip distance   & \lstinline|MuFlipDistanceConjecture|       & Open \\
\bottomrule
\end{tabular}
\end{table}
...
constraint to ridges $2^n - 16$ is novel.  The OEIS
sequence~A394412~\cite{OEISA394412} records the first 57~terms.
...
and the precise formal statement of the mirror-dual conjecture
