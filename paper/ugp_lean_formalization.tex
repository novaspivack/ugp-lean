...
\[
  \lstinline|mphi_scc| = \lstinline|m_tau_pdg_eV| = 1\,776\,860\,000\,\text{eV}.
\]
...
$\lstinline|fpi_scc_eV| = ({}^\uparrow\!\lstinline|mkink_scc|)/\pi$.
...
\subsection{QEC Stabilizer Code Interpretation of the
...
\item \textbf{Code subspace.}
  $\lstinline|InCodeSubspace|(b) \Leftrightarrow \lstinline|PSCAdmissible|(b)$:
  the code words are exactly the four orbit-certified PSC states.
\item \textbf{Stabilizer closure.}
  $\lstinline|InCodeSubspace|(b) \Rightarrow \lstinline|InCodeSubspace|(\lstinline|fmdl_step5|(b))$:
  $f_{\mathrm{MDL}}$ maps every code word to a code word.
\item \textbf{Perfect error detection.}
  $\neg\lstinline|InCodeSubspace|(b) \Rightarrow \mathrm{DWeight}(b) = 0$:
...
\item \lstinline|qec_dweight_projector| --- $\mathrm{DWeight}(b) > 0
  \Leftrightarrow \lstinline|InCodeSubspace|(b)$: the DWeight function is the
