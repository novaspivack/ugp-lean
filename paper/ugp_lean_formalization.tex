...
canonical orbit, and a new class of primes (\emph{\ugp{} primes}) with
ten machine-checked instances.

A central result is the \textbf{machine-checked proof that the \ugp{}
substrate is Turing-universal}: the Universal Windowed Cellular Automaton
(\uwca{}) built natively from the \ugp{} arithmetic substrate exactly simulates
Rule~110 via a four-pass round (neighbor distribution, minterm detection,
OR-accumulation, commit).  The simulation theorem \texttt{uwca\_sweep\_implements\_rule110}
is proved by exhaustive case analysis over all eight neighborhoods.
Since Rule~110 is Turing-universal~\cite{Cook2004}, the arithmetic sieve
that selects the canonical prime seed $(1,73,823)$ also defines a substrate
in which arbitrary computation can be realized.  This is the sense in which
the \ugp{} is genuinely ``universal''.
The artifact comprises 49~\lean{} modules, builds against Mathlib
...
a new \emph{mirror shift algebra}: the universal law
$c_1(b_2,q_2) - c_1(q_2,b_2) = b_1(q_2-b_2)$ and shared residue
$c_1 \equiv 20 \pmod{b_1}$, holding at every ridge level;
and a suite of \emph{structural theorems}: the Fibonacci renormalization
spectrum (eigenvalues $\varphi$ and $-\varphi^{-1}$, decay rate $\varphi^{-2}$),
loop topology from mirror pairs, minimality-duality at $n=10$, fingerprint
fixed-point (Tarski), and the decidability phase transition (local decidable,
global $\Sigma^0_1$-complete from Turing universality).
...
\S\ref{sec:structural} presents the structural theorems.
\S\ref{sec:conjectures} states the four remaining open conjectures.
...
The artifact is organized into 49~\lean{} modules across seven layers:
...
\begin{table}[ht]
\centering
\caption{Open conjectures, formally stated in \texttt{Conjectures.lean}.}
...
Four conjectures are formally stated in \cref{tab:conjectures}.
