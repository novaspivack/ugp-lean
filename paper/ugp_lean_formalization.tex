...
{\footnotesize\pdfprotrudechars=0\tableofcontents}
\clearpage
\bigskip
...
\begin{table}[h]
\centering\small
\begin{tabular}{@{}p{5.5cm}p{1.8cm}p{7.7cm}@{}}
\toprule
...
\subsection{Event-Triggered Coupling Escape
  (\texttt{Universality/DynamicalCouplingBridge.lean})}\label{subsec:dynamical-coupling-bridge}
...
\subsection{Klein--Gordon Lorentz Invariance
  (\texttt{Universality/LorentzInvariance.lean})}\label{subsec:lorentz-invariance}
...
\subsection{Beable Z\textsubscript{7} Winding Partition Instance
  (\texttt{Universality/BeableWindingPartitionInstance.lean})}\label{subsec:beable-winding-partition}
...
Supporting theorems: \lstinline|beableWindingPartitionInstance|,
\lstinline|born_rule_from_structural_beable_amplitude|,
\lstinline|second_quantization_constructive|,
\lstinline|kink_quantization_master_bundle|.
...
\subsection{'t Hooft Effect-Measure Bridge
  (\texttt{Universality/ThooftEffectMeasureBridge.lean})}\label{subsec:thooft-effect-measure}
...
zero \lstinline|sorry|, \textbf{conditional CatAL}: seven private axioms plus
...
The seven private axioms are kernel workarounds for matrix operations at
dimension $7^5$ (PSD/trace lemmas, opaque sector projectors, effect-one
normalization); they are not physical assumptions.
...
(\texttt{ugp-lean-exp/UgpLean/Spacetime/} and \texttt{UgpLean/Substrate/}),
pending graduation to the canonical repository.  All theorems carry zero \texttt{sorry} unless
...
\subsection{Chiral Mirror Speed Symmetry
  (\texttt{Spacetime/ChiralMirrorSpeedSymmetry.lean})}\label{subsec:chiral-mirror-speed}
...
Supporting theorems: \lstinline|rule124Fin2_is_spatial_mirror|,
...
Supporting theorems: \lstinline|ether_temporal_period_eq_seven|,
...
§5o & Vector meson nonet & \texttt{vector\_\allowbreak meson\_\allowbreak nonet\_jp1},
  \texttt{berry\_\allowbreak hyperfine\_\allowbreak su3\_trace} & 4 \\
...
§8 & Second octet suppression &
  \texttt{baryon\_\allowbreak octet\_\allowbreak suppression\_z3},
  \texttt{second\_\allowbreak octet\_\allowbreak mdl\_bit\_gap} & 4 \\
§9 & J\textsuperscript{P} = 1/2 forced &
  \texttt{jp\_spin\_\allowbreak half\_\allowbreak mdl\_forced},
  \texttt{mdl\_\allowbreak selects\_\allowbreak spin\_half\_\allowbreak baryon} & 7 \\
\midrule
& & Total §5i–§9 & \textbf{64} \\
\bottomrule
\end{tabular}
...
\subsection{PSC-Preserving Transformations
  (\texttt{Substrate/PSCPreservingTransformation.lean})}\label{subsec:psc-preserving-transformation}
...
\subsection{$\Phi_{\mathrm{MDL}}$ Lagrangian Lorentz Scalarity
  (\texttt{Substrate/LagrangianLorentzScalar.lean})}\label{subsec:lagrangian-lorentz-scalar}
...
\subsection{SO(3) Noether Angular Momentum
  (\texttt{Substrate/NoetherAngularMomentum.lean})}\label{subsec:noether-angular-momentum}
