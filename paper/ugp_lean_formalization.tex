...
framework in \lean{}~4 against Mathlib, comprising 99 modules with a
...
A \textbf{MassRelations} layer (17 modules) formalizes the
...
mass ratio $Z$-independence, and the seesaw index as a GUT representation defect.
...
cellular automata, mass relations, braid topology
...
The artifact is organized into 99~\lean{} modules across eleven layers:
...
\node[layer, fill=orange!8] (gte)   at (0,3*\vs)
  {\textbf{GTE}~(21)\quad Evolution, Orbit, UpdateMap,
   ...
   \textbf{MersenneLadder, FiberBundle, LinearResponse, ScaleConnection}};
...
\node[layer, fill=teal!8]   (mass)  at (0,5*\vs)
  {\textbf{MassRelations}~(17)\quad KoideClosedForm, KoideNewtonFlow,
   ...
   \textbf{KoideAngle, ClaimCBridge, LeptonMassPrediction, ScaleTransport, SeesawIndex}};
\node[layer, fill=cyan!6]   (braid)  at (0,6*\vs)
  {\textbf{BraidAtlas}~(1)\quad \textbf{ChargeTheorem}};
\node[layer, fill=lime!6]   (null)  at (0,7*\vs)
  {\textbf{NullDiscipline}~(2)\quad \textbf{SaturationBarrier, TheoremEligibility}};
\node[layer, fill=violet!6]   (ipt)  at (0,8*\vs)
  {\textbf{IPT}~(1)\quad \textbf{InformationProfitThreshold}};
\node[layer, fill=purple!8] (univ)  at (0,9*\vs)
...
\node[layer, fill=gray!8]   (self)  at (0,10*\vs)
...
\draw[arr] (mass.north)  -- (braid.south);
\draw[arr] (braid.north) -- (null.south);
\draw[arr] (null.north)  -- (ipt.south);
\draw[arr] (ipt.north)   -- (univ.south);
...
Tenenbaum~\cite{Tenenbaum1995} III.6.  These are analytic number theory
...
The \texttt{BraidAtlas.ChargeTheorem} module derives electric charge from
the topological invariants of braid processes.
...
\lstinline|winding_sum_per_generation|
\textup{(in \texttt{BraidAtlas.ChargeTheorem})}
\end{leanref}
\end{theorem}
...
The \texttt{NullDiscipline} layer machine-classifies the programme's own
results by their evidential grade.
...
from three structural premises within the MFRR formal model.

\begin{theorem}[IPT derivation]\label{thm:ipt-lean}
Given:
\begin{enumerate}[nosep,label=(A\arabic*)]
\item The PSC contraction rate equals the inverse golden ratio: $|\psi| = 1/\varphi$
  (\lstinline|abs_psi_eq_inv_phi|, derived from the Fibonacci companion matrix spectrum);
\item The adjudication entropy of the $U(1)$ gauge group equals $H(U(1)) = \ln(2\pi)$
  (\lstinline|A2_adjudication_entropy|, from unitary $U(1)$ phase space);
\item PSC time-reversal symmetry splits the overhead equally: $1/2$ forward, $1/2$ backward
...
\paragraph{Scope.}
Theorem~\ref{thm:ipt-lean} is theorem-grade \emph{within the formal PSC/Fibonacci/$U(1)$-entropy model}.
The three premises A1--A3 are structural inputs to the derivation; whether they correctly
abstract a given physical or biological system is an empirical question separate from the
formal proof.
...
\[
  ...
  \;=\; \frac{(g_1 + g_2)^2}{g_1 g_2}
  \;=\; \frac{377525}{37264} \;\approx\; 127.31.
\]
...
The integer $29$ equals the gauge/matter representation defect of the
PSC-selected GUT group:
\[
  29 \;=\; \dim(\mathbf{adj}_{SO(10)}) - \dim(\mathbf{spinor}_{SO(10)})
     \;=\; 45 - 16.
\]
The seesaw exponent $29/9$ is therefore the ratio of this representation defect
to $N_c^2$, unifying all three structural decompositions of Theorem~\ref{thm:seesaw-closure}
under a single geometric interpretation.
...
(\cref{sec:massrelations}, 17 modules):
...
($\delta b_1 = 2^{N_c^2}-1$), mass ratio $Z$-independence, and the seesaw
index as SO(10) gauge/matter representation defect.
...
and $1/\alpha_{\rm EM} = 377525/37264$ from the certified bare couplings.
