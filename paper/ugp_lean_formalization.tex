...
determines exponent $29/9$, $M_R$ cancels algebraically, and the coarse bound
$0.029 < R < 0.030$ is certified via integer 9th-power comparisons
(\texttt{NeutrinoMassRatio}, 2026-05-16, zero sorry).
...
\item \textbf{Mass relations and physical spectrum} (\S\ref{sec:massrelations}):
  ...
  coarse bound $0.029 < R < 0.030$ certified zero sorry
  (\texttt{NeutrinoMassRatio}, 2026-05-16).
\end{itemize}
...
\begin{theorem}[Neutrino mass ratio (Phase~1), zero sorry]\label{thm:neutrino_mass_ratio}
Three theorems in \texttt{MassRelations.NeutrinoMassRatio} are machine-checked
...
\paragraph{Phase~2 (deferred).}
A tighter bound $|R - 0.02936| < 0.0001$ is mathematically argued
via 9th-power checks at $\pm 0.1\%$ precision, but is deferred
pending a \texttt{Real.rpow} \lstinline|norm_num| extension in Mathlib.
The full Lagrangian bridge --- formal derivation that Yukawa couplings
scale as $b^{q_1}$ and $b^{q_2}$ from the FN texture --- also remains open
...
and the seesaw mass-squared ratio arithmetic chain: FN texture $(q_1,q_2)=(3,2)$
gives exponent $29/9$, $M_R$ cancels algebraically, coarse bound
$0.029 < R < 0.030$ certified
(\texttt{NeutrinoMassRatio}, 2026-05-16, 3 theorems, zero sorry).
