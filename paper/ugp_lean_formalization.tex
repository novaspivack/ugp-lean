...
Module: \texttt{Spacetime/GeodesicTheorem.lean} (development branch, commit \texttt{1671d46}).
Status: the module exists in the development branch and establishes the formal
predicate structure (causal-graph geodesics, Ehrenfest-type averaging, the
equivalence principle statement).
Full closure of the unconditional geodesic uniqueness theorem is pending;
the module currently serves as the formal container for the geodesic
architecture and is the target for future Lean proof work.  It is the
first formal connection between the \gte{} emergent gravity programme and
particle trajectories in the 3D causal graph.
