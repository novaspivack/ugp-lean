...
framework in \lean{}~4 against Mathlib, comprising 144 modules with a
...
from the \ugp{} survivor space exactly simulates Rule~110~\cite{Wolfram1984,Wolfram2002,Cook2004} via a
four-pass round, verified by exhaustive case analysis
...
A new \textbf{Universality} layer (34 modules) extends
this to a complete computational picture, culminating in
\textbf{GUTStructure} (Beable Hilbert space, Weinberg-angle closure,
TPC hierarchy proxies) and \textbf{LawvereZone} (Zone L1/L2/L3 classification).
A new \textbf{Framework} layer (3 modules) completes the GTE-substrate program:
\lstinline|GTEFrameworkInstance| instantiates GTE as a \texttt{NemS.Framework} with
\lstinline|gte_tpc_from_nems_classification| (one Cook-bridge axiom);
\lstinline|GTEOptimalityInstance| derives GTE optimality and D-uniqueness;
\lstinline|GTEFinalCoalgebra| proves GTE is the \emph{final} $F_{\rm PSC}$
coalgebra in \texttt{PSCSys} — zero sorry, zero custom axioms.
\textbf{CUP-4} (\lstinline|cup4_parity_uniqueness|) proves that the
Standard Model generation orbit $(g_1\to g_2\to g_3)$ under the
total-parity morphism \emph{algebraically forces} Rule~110 as the unique
vacuum-transparent binary CA rule consistent with SM winding structure.
The same Rule~110 is independently realized by the \uwca{} substrate,
giving a \emph{two-way convergence}: Rule~110 is forced both from the SM
orbit and from the arithmetic substrate.
\textbf{Garden of Eden} (\lstinline|fmdl_gen1_is_garden_of_eden|,
\lstinline|native_decide| over $7^5 = 16{,}807$ states): the first-generation
state $\text{gen}_1=[1,5,2,2,1]$ has zero predecessors under $f_{\rm MDL}$,
giving a structural explanation of first-generation particle stability.
\textbf{Tape-level unification} (\lstinline|hypothesis_b_tape_level|):
a single Rule~110 Bool tape simultaneously computes both UGP dynamical sectors
--- the SM charge sector (fMDL winding, zero axioms) and the GTE mass cascade
(one named Cook universality axiom).
\textbf{PSC chain} (\lstinline|hypothesis_c_psc_forces_universality|):
PSC forces SM gauge structure, which forces the orbit, which forces Rule~110,
which is Turing-universal --- each step Lean-certified.
Three named physics bridge axioms span the module
(\lstinline|rcc_physics_ax|, \lstinline|rule110_simulates_computable|,
\lstinline|simultaneous_dual_tape_ax|), each backed by substantial
...
A \textbf{MassRelations} layer (22 modules) formalizes the
...
and the discrete arithmetic-mean identity $2\,a_\tau = a_e + a_\mu$ on the
...
uniqueness, fiber bundle, orbital stability, scale connection, GTB generation
prime certificate, \textbf{NuclearPairing}: nucleon b-seed parity
L001--L006 and the identity $5\sqrt{5}=\sqrt{125}$ underpinning
$\Delta_{\rm pair}=5^{3/2}=11.18$~MeV~[B]),
\textbf{BraidAtlas} charge derivation ($Q = W_g/N_c$ with anomaly cancellation forcing
$N_c = 3$, Coxeter--Conductor theorem, chirality squaring, EW boson c-values),
\textbf{Dark Braid Atlas} (mirror-branch dark sector: Q=0 for all dark particles,
dark gauge group SU(3)$_{\rm dark}$ only, RHN gap theorem, dark quark charge, and a
16-theorem machine-checked dark sector structural certificate),
\textbf{NullDiscipline} evidential classification (four-gate TEC,
...
\item \textbf{Mass relations and physical spectrum} (\S\ref{sec:massrelations}):
  The \texttt{MassRelations} layer (22 modules) covers
...
The artifact is organized into 144~\lean{} modules across thirteen layers
...
\node[layer, fill=teal!8,   below=of struc] (mass)
  {\textbf{MassRelations}~(22)\quad KoideClosedForm, KoideNewtonFlow,
   ...
   CKMTheta23, KoideS3DiscreteIdentities};
...
\node[layer, fill=purple!8, below=of ipt]   (univ)
  {\textbf{Universality}~(34)\quad Rule110, UWCA, UWCASimulation,
   ...
   GoEHierarchy, GoEStabilityHierarchy, GTEInfTapeEncoding, GTEComputability,
   ...
   Z5TransitivityUniqueness, GTPNeutralDiscrimination, SMOrbitCausalIsolation,
   EWBosonStructure, EWChiralBridge, \textbf{GUTStructure}, LawvereZone,
   CasimirMasslessEther};
...
\node[layer, fill=pink!10,  below=of self]  (fwk)
  {\textbf{Framework}~(3)\quad \textbf{GTEFrameworkInstance},
...
The \texttt{MassRelations} layer (22 modules) formalizes the
...
% ─────────────────────────────────────────────────────────────────
\subsection{GTE-NEMS Framework Instance (\texttt{GTEFrameworkInstance})}\label{subsec:gte-framework-instance}
...
% ─────────────────────────────────────────────────────────────────
\subsection{GTE Optimality (\texttt{GTEOptimalityInstance})}\label{subsec:gte-optimality-instance}
...
\textbf{Key theorem:} \lstinline|gte_d_unique| (zero \texttt{sorry}) proves that
...
\begin{leanref}
\lstinline|GTEOptimalityInstance|, \lstinline|gte_d_unique|
...
The \texttt{Framework.GTEFinalCoalgebra} module proves that the GTE-Möbius substrate
is the \emph{terminal object} in the category \texttt{PSCSys} of PSC-compatible spaces,
i.e., GTE is the final $F_{\rm PSC}$ coalgebra.  This is the C1 Final Coalgebra result
(7 stages, all zero sorry, zero custom axioms).
...
to agree with $f_{\rm MDL}$ on all fixed neighborhoods.
...
(the functor $F_{\rm PSC} = \mathbf{1}_{\mathsf{PSCSys}}$ is the identity).
...
The C1 Final Coalgebra result completes the categorical characterization of the GTE
substrate: GTE is simultaneously a terminal object (by this module), the unique
orbit-admissible MDL-minimal function (by \texttt{GUTStructure}), and a
\texttt{NemS.Framework} instance with transputation classification (by
\texttt{GTEFrameworkInstance}).
...
% ─────────────────────────────────────────────────────────────────
\subsection{Cook Rule~110 Reference (\texttt{CookRule110Ref})}\label{subsec:cook-rule110-ref}
...
% ─────────────────────────────────────────────────────────────────
\subsection{RCC Analytical Completion (\texttt{PSC.RCCComplete})}\label{subsec:rcc-complete}
...
% ═══════════════════════════════════════════════════════════════
\section{GUT Structure and Physical Formalization (\texttt{GUTStructure})}\label{sec:gutstructure}
% ═══════════════════════════════════════════════════════════════
...
\textbf{Charge formula (§49).}  \lstinline|charge_from_z7_winding|:
...
(\lstinline|weinberg_angle_from_hypercharge_sum|).
...
This section documents the twelve \texttt{Universality} modules added in the
\textbf{ugp-lean-exp} expansion beyond the original 22-module core.
...
\begin{itemize}[nosep]
\item \lstinline|gen1_is_goe|: gen$_1$ has zero predecessors (Garden of Eden).
\item \lstinline|gen2_unique_predecessor|: gen$_2$'s unique predecessor is gen$_1$.
\item \lstinline|gen3_unique_predecessor|: gen$_3$'s unique predecessor is gen$_2$.
\item \lstinline|sm_chain_fully_isolated|: no other state in $\mathbb{Z}_7^5$ maps
...
\begin{theorem}[\lstinline|z5_prime_unique_transitivity|, zero \texttt{sorry}]
...
\subsection{Dimensional Slice Uniqueness (\texttt{DimensionalSliceUniqueness})}\label{subsec:dimensional-slice}
...
\begin{leanref}
\lstinline|gtp_neutral_particles_distinct|
...
The \texttt{CasimirMasslessEther} module (235 lines, 15 items) certifies three results
from the Casimir/photon-vacuum analysis:
...
\subsection{Lawvere Zone (\texttt{LawvereZone})}\label{subsec:lawvere-zone}
...
GTE as the terminal $F_{\rm PSC}$ coalgebra in \texttt{PSCSys}, zero sorry, zero custom axioms (\texttt{GTEFinalCoalgebra}).

\medskip
\noindent\textbf{GTE extended structure}
(\cref{sec:gte-extended}, 5 new modules):
...
contraction mode, the universal 5-factor ridge-level scale connection, and the
GTB generation prime certificate (all six generation $c_1$ values certified prime,
