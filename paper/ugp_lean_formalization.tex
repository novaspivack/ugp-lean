...
\author{Nova Spivack\\
  \small\url{https://github.com/novaspivack/ugp-lean}}
\date{}
...
\begin{abstract}
\raggedright
We present \textbf{ugp-lean}, a \lean{} formalization of the Universal
Generative Principle (\ugp{}), a deterministic arithmetic framework
defined over integer ridges $R_n = 2^n - 16$.  The formalization
covers: the ridge sieve, prime-lock criterion, Residual Seed Uniqueness
(\rsuc{}), Quarter-Lock identity, Generative Triple Evolution (\gte{})
canonical orbit, \uwca{} Turing universality, and a new class of primes
(\emph{\ugp{} primes}) with ten machine-checked instances.
The artifact comprises 48~\lean{} modules, builds against Mathlib
v4.29.0-rc6, and maintains a strict \emph{zero-sorry, zero-custom-axiom}
policy on the core \ugp{} proof path.  Novel contributions include: a
machine-checked proof that the \gte{} update map~$T$ \emph{forces} the
canonical orbit, a general ridge remainder lock for all $n \ge 5$, an
even-step $c$-invariance theorem, a formal definition of \ugp{} primes
with existence proof and ten verified instances across three ridge
levels, a precise statement of the mirror-dual conjecture with a
proof that the divisor count $\tau(2^n-16)$ is unbounded, an
exact divisor-count formula
$\tau(2^n - 16) = 5 \cdot \tau(2^{n-4} - 1)$ for all $n \ge 5$,
and a new \emph{mirror shift algebra}: a machine-checked proof that for
any mirror-dual pair $(b_2, q_2)$, both $c_1$-values are congruent to
$t = 20$ modulo $b_1 = b_2 + q_2 + 7$, and their difference equals
$b_1 \cdot (q_2 - b_2)$---a universal algebraic law holding at every
ridge level.
The repository also contains companion modules (\texttt{InertPrimes},
\texttt{AnalyticArchitecture}) providing citable analytic infrastructure
derived from the \ugp{} factory constants; these are documented in
\texttt{docs/RESONANT\_FACTORY\_NOTE.md} and not discussed in this paper.
All definitions are self-contained; all open conjectures are formally
stated as \lean{} propositions.

\textbf{Trust boundary:} Proved statements are scoped to the ridged arithmetic framework formalized here; open conjectures remain explicit \lean{} \texttt{Prop}s and are not asserted as classical theorems.
\end{abstract}
...
\S\ref{sec:conjectures} resolves seven conjectures and states four open ones.
...
The artifact is organized into 48~\lean{} modules across seven layers:
...
\node[layer, fill=orange!8] (gte)   at (0,3*\vs)
  {\textbf{GTE}~(11)\quad Evolution, Orbit, UpdateMap,
   GeneralTheorems, MersenneGcd, PrimeFactorAnalysis,
   MirrorDualConj, MirrorShift, UGPPrimes, InertPrimes, AnalyticArchitecture};
...
\node[layer, fill=purple!8] (univ)  at (0,5*\vs)
  {\textbf{Universality}~(5)\quad Rule110, UWCA,
...
\item The Universal Windowed Cellular Automaton (\uwca{}) tile set
  embeds Rule~110: each Rule~110 neighborhood is mapped to a
  \uwca{} tile, and the tile-matching relation preserves the
  Rule~110 transition function (\texttt{uwca\_simulates\_rule110}).
...
\lstinline|mirror_shift_factored|
\end{theorem}
...
(\lstinline|shift_n13|, \lstinline|shift_n16_a|, \lstinline|lepton_shift_val|).
The shared residue $\equiv 20 \pmod{b_1}$ holds at all three levels
(\lstinline|shared_residue_n13|, \lstinline|shared_residue_n16_a|,
...
% ═══════════════════════════════════════════════════════════════
\section{Conjectures: Resolved and Open}\label{sec:conjectures}
% ═══════════════════════════════════════════════════════════════

The \ugp{} paper posed ten conjectures.  Seven are proved as theorems
in ugp-lean.  Four remain open (two original plus two new forms of the
$\mu$-flip conjecture added in this formalization).
\Cref{tab:conjectures} summarizes the status.

\subsection{Resolved conjectures (proved)}

\begin{theorem}[Mirror min-dual]\label{thm:mirror-min}
$b_1(b_2, q_2) = b_1(q_2, b_2)$ for all $b_2, q_2$.

\smallskip\noindent
\textup{Lean:} \lstinline|mirror_min_dual_proved|
\end{theorem}
...
\begin{theorem}[Fibonacci rigidity]\label{thm:fib-rigid}
For all divisor pairs $(b_2, q_2)$ with $q_2 \ge 13$:
$q_2 - q_1(q_2) = 13$, where $q_1(q_2) = q_2 - 13$.

\smallskip\noindent
\textup{Lean:} \lstinline|fib_rigidity_proved|
\end{theorem}

\begin{proof}
$q_2 - (q_2 - 13) = 13$, immediate from the definition of $q_1$.
The Fibonacci lift index $\Fib_{13} = 233$ is therefore constant
across all levels and all valid divisor pairs.
\end{proof}
...
That is, $c_1$ is strictly increasing in~$q_2$.
...
\begin{proof}
Both factors of $c_1 = (b_2 + q_2 + 7)(q_2 - 13) + 20$ are
increasing in~$q_2$.
\end{proof}

\begin{theorem}[Robust universality]\label{thm:robust-univ}
The \ugp{} substrate is Turing-universal unconditionally.

\smallskip\noindent
\textup{Lean:} \lstinline|robust_universality_proved|
\textup{(= \lstinline|ugp_is_turing_universal|)}
\end{theorem}

\begin{theorem}[Sharp decidability boundary]\label{thm:sharp-bd}
Finite-horizon properties of the \gte{} trajectory are decidable
(the trajectory is computable).  Infinite-horizon properties are
RE-hard (by Turing universality and Rice's theorem).

\smallskip\noindent
\textup{Lean:} \lstinline|sharp_boundary_proved|
\end{theorem}

\begin{theorem}[Kernel compatibility]\label{thm:kern-compat}
The Quarter-Lock identity $k_M = k_{G_2} + \tfrac{1}{4} k_L^2$
holds unconditionally for all triples in the formalization.
Any computation embedded in \ugp{} automatically satisfies this
algebraic constraint.

\smallskip\noindent
\textup{Lean:} \lstinline|kernel_compatibility_proved|
\textup{(= \lstinline|quarterLockLaw|)}
\end{theorem}

\begin{theorem}[Global $c$-attractor]\label{thm:c-attract}
For any valid divisor pair at level $n \ge 5$, the even-step
update map sends~$c$ to $2^n - 1$ in one step:
$\mathsf{evenStepC}(b_2,\, (2^n-1)/b_2) = 2^n - 1$.
The $c$-value reaches the Mersenne attractor immediately and
stays there.

\smallskip\noindent
\textup{Lean:} \lstinline|global_c_attractor_proved|
\textup{(= \lstinline|even_step_c_invariance|)}
\end{theorem}

\subsection{Open conjectures}

Four conjectures remain open.  All are formally stated as \lean{}
propositions.

\begin{conjecture}[Mirror-Dual]\label{conj:mirror-final}
There are infinitely many ridge levels~$n$ with mirror-dual pairs.
This is structurally analogous to the twin prime conjecture.
...
There are infinitely many \ugp{} primes.  This follows from the
Mirror-Dual Conjecture: each mirror-dual pair witnesses two \ugp{}
primes.
...
\begin{conjecture}[$\mu$-Flip: infinitely many squarefree elements in any coprime AP]\label{conj:mu-flip-final}
For every $a, r \in \N$ with $a > 0$ and $\gcd(a, r) = 1$, and
every $N \in \N$, there exists $n \ge N$ with $n \equiv r \pmod{a}$
and $n$ squarefree.  Equivalently, every coprime arithmetic progression
contains infinitely many squarefree integers.
...
\begin{remark}
The weak form (\lstinline|MuFlipDistanceConjectureWeak|) is the formal
\lean{} statement using \lstinline|Filter.atTop|.  The underlying fact
is known unconditionally (Estermann 1931): the density of squarefree
integers in any primitive AP is $\prod_p (1 - 1/p(p-1)) > 0$.  The
formal \lean{} proof is not yet mechanized; it is listed here as an
open formalization target.
\end{remark}

\begin{conjecture}[$\mu$-Flip Bounded Gap]\label{conj:mu-flip-gap}
There exists a universal constant $C > 0$ such that for all
$a, r, N \in \N$ with $a > 0$ and $\gcd(a,r) = 1$,
the interval $[N, N + Ca]$ contains a squarefree integer
$n \equiv r \pmod{a}$.
...
\begin{table}[ht]
\centering
\caption{Conjecture status.  Seven proved, four open.}
\label{tab:conjectures}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Statement} & \textbf{Lean name} & \textbf{Status} \\
\midrule
Mirror min-dual       & \lstinline|mirror_min_dual_proved|         & Proved \\
Fibonacci rigidity    & \lstinline|fib_rigidity_proved|            & Proved \\
MDL monotonicity      & \lstinline|c1_monotone_in_q2|             & Proved \\
Robust universality   & \lstinline|robust_universality_proved|     & Proved \\
Sharp boundary        & \lstinline|sharp_boundary_proved|          & Proved \\
Kernel compatibility  & \lstinline|kernel_compatibility_proved|    & Proved \\
Global $c$-attractor  & \lstinline|global_c_attractor_proved|      & Proved \\
\midrule
Mirror-dual           & \lstinline|MirrorDualConjecture|           & Open \\
\ugp{} prime infinitude & \lstinline|UGPPrimeInfinitudeConjecture| & Open \\
$\mu$-flip (squarefree in AP) & \lstinline|MuFlipDistanceConjecture|  & Open (known; not formalized) \\
$\mu$-flip bounded gap & \lstinline|MuFlipBoundedGapConjecture|    & Open \\
\bottomrule
\end{tabular}
\end{table}
...
and the mirror shift algebra (\cref{sec:mirrorshift}): the universal
...
The ten conjectures in \cref{tab:conjectures} are formally stated but
unproved.  The mirror-dual conjecture (\cref{conj:mirror}) and the
\ugp{} prime infinitude conjecture (\cref{conj:ugp-inf}) are the most
interesting: both are structurally analogous to the twin prime conjecture
and would require new sieve-theoretic techniques.  Dirichlet-series
analysis of the ridge divisor function and Markov mixing properties of
the \gte{} trajectory are natural directions for future formalization.
