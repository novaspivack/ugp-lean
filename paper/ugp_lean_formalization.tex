...
\texttt{gte\_poly\_double\_role}
  & Universality.BellViolationGTE
  & GTE polynomial simultaneously sources gravity and generates entanglement \\
\bottomrule
\end{tabular}
\caption{New theorems and named axioms from OQ-079-1 (gravity force law) and OQ-079-3
(Born rule + Bell inequality). Theorems marked (axiom) are named axioms documenting CatAD or
CatA results pending full Lean formalization (Mathlib PoissonKernel, PW library, or
analytic T-matrix bound). Theorems without (axiom) are zero sorry.}
