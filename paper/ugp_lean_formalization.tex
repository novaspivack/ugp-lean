...
% Ragged-right section titles avoid huge inter-word gaps when titles include
% unbreakable \texttt{...} fragments (article class otherwise justifies titles).
\usepackage{titlesec}
...
\noindent
Several \emph{fully mechanized} precision and cyclotomic layers appear under
\texttt{Phase4}, \texttt{GaloisStructure}, and \texttt{CyclotomicCompleteness}.

\smallskip\noindent
\textbf{Phase4.}
\texttt{Phase4.GaloisProtection} formalises one-loop non-renormalization (Galois protection of the
Quarter-Lock algebraic prefactor).
\texttt{Phase4.TwoLoopCoefficient} certifies the two-loop colour coefficient ($=8/9$).

\smallskip\noindent
\textbf{GaloisStructure.}
\texttt{GaloisStructure.CyclotomicLayers} records Galois-stability of UGP layers in
$\mathbb{Q}(\zeta_{120})$; \texttt{GaloisStructure.MinimalCyclotomic} certifies minimality of
$\mathbb{Q}(\zeta_{120})$ in that role.

\smallskip\noindent
\textbf{CyclotomicCompleteness.}
\texttt{CyclotomicCompleteness.CoxeterBiconditional} gives the Coxeter--conductor biconditional
(see \S\ref{sec:coxeter_biconditional}).
\texttt{CyclotomicCompleteness.CyclotomicContainment} handles the field-theoretic embedding
$\mathbb{Q}(\zeta_{2h}) \hookrightarrow \mathbb{Q}(\zeta_{120})$ for $h \mid 60$
(see \S\ref{sec:cyclotomic_containment}).

\smallskip\noindent
\textbf{Lighter entry points.}
The modules \texttt{Papers} and \texttt{Instance} supply chiefly \emph{citable stubs} and bridge
entry points; \texttt{Conjectures} records resolved and open claims; and \texttt{Phase4} also
contains stubs alongside the result modules named above.

\smallskip\noindent
\textbf{\texttt{TE22} (PSC universe scan).}
The \texttt{TE22} module provides a machine-checked framework for the TE2.2 PSC universe scan.

\smallskip\noindent
Two theorems are proved with zero \texttt{sorry}:
\texttt{ugp\_coupling\_predictions\_are\_independent} states that the three UGP-derived coupling
ratio predictions $C_{15}$, $C_{16}$, and $C_4'$ are algebraically derived from
\emph{ugp-lean} machine-checked rationals---not fitted to SM coupling data.
\texttt{ugp\_g1g2\_prediction\_close\_to\_SM} records that the UGP $g_1^2/g_2^2$ prediction is
within $2\%$ of the Standard Model value at $M_Z$.

\smallskip\noindent
Two further theorems govern gauge-group labelling.
\texttt{SM\_gauge\_uniquely\_selected} proves (by \texttt{decide}) that exactly one of the $60$
\mbox{(GaugeGroup, Dimension)} pairs satisfies \texttt{isSMGauge}.
\texttt{isSMGauge\_iff} gives the full logical characterisation of that label.

\smallskip\noindent
A fourth theorem, \texttt{SM\_is\_D\_minimizer\_extended}, is proved with zero \texttt{sorry} as an
alias \texttt{:=\ isSMGauge\_iff}, packaging the \emph{decidable fragment} of the SM-uniqueness
claim.

\smallskip\noindent
\textbf{Open scope.}
The full machine-checked proof that the SM is the unique $D$-minimizer over the 34{,}560-universe
scan remains an open programme: the framework and decidable fragment are in place, but
\texttt{native\_decide} certification awaits a computable \texttt{Fintype} instance on the full
\texttt{UniverseParams} product type and computable constraint functions.
A computational certificate lives in
\texttt{ugp-physics/MFRR/.../extended\_scan\_results.json} (SHA-256: \texttt{407078d7\ldots}).
...
% ═══════════════════════════════════════════════════════════════
\section[Galois-Protection Non-Renormalization (Phase4.GaloisProtection)]{Galois-Protection Non-Renormalization\\\texttt{Phase4.GaloisProtection}}
...
\noindent\textit{Migration note.}
The four modules \texttt{H9SelfConsistency}, \texttt{GoldenRatioFixedPoint},
\texttt{U1DirectProof}, and \texttt{LieExpSurjective} (formerly under
\texttt{CyclotomicCompleteness}) have been moved to \texttt{ugp-physics-lean}
as \texttt{UgpPhysicsLean.GXT.*}.

\smallskip\noindent
\texttt{GXT.LieExpSurjective} proves \lstinline|circle_exp_surjective_MAIN|
(zero sorry): $\mathrm{exp} : \mathbb{R} \to \mathrm{Circle}$ is surjective via the
open subgroup theorem; \lstinline|circle_iso_addCircle_2pi| establishes
$\mathrm{Circle} \cong \mathbb{R}/(2\pi\mathbb{Z})$ (zero sorry).  Together with
\lstinline|lie_algebra_finrank_one_isAbelian| (in \texttt{GXT.U1DirectProof}),
these certify A2 zero-sorry for the UGP/IPT derivation.

\smallskip\noindent
The one remaining sorry (\lstinline|u1_minimality_abstract|) does not block the
UGP theorem.\par}
...
\begin{align*}
  ...
  \frac{1}{1 - \ln 2/N_{\rm univ}}
    ...
    = 1 + \frac{\ln\varphi}{2\ln(2\pi)} = \mathrm{IPT}.
    \quad \checkmark
\end{align*}
...
\subsection[Koide S3 discrete arithmetic-mean identity]{Koide \texorpdfstring{$S_3$}{S3} Discrete Arithmetic-Mean Identity\\\texttt{MassRelations.KoideS3DiscreteIdentities}}
