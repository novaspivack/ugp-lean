...
\lstdefinelanguage{Lean4}{
  ...
  literate={→}{$\to$}1 {←}{$\leftarrow$}1 {∀}{$\forall$}1 {∃}{$\exists$}1
           ...
           {ℚ}{$\mathbb{Q}$}1 {ℤ}{$\mathbb{Z}$}1 {ℝ}{$\mathbb{R}$}1
           {κ}{$\kappa$}1 {⟨}{$\langle$}1 {⟩}{$\rangle$}1
           {₁}{$_1$}1 {₂}{$_2$}1 {₃}{$_3$}1
}
...
\newcommand{\CatAL}{\ensuremath{\mathrm{CatAL}}}
\newcommand{\CatA}{\ensuremath{\mathrm{CatA}}}
\newcommand{\CatAD}{\ensuremath{\mathrm{CatAD}}}
\newcommand{\CatB}{\ensuremath{\mathrm{CatB}}}
\newcommand{\GTE}{\ensuremath{\mathrm{GTE}}}
...
438~\lean{} modules across multiple organizational layers (canonical library covering
spacetime, QFT, and hadronic physics) with a strict zero-sorry policy throughout
and zero custom axioms on the core arithmetic/algebraic proof path; the
Turing-universality results are conditional on a small number of explicitly named,
standard classical axioms disclosed at the point of use.
...
\emph{Physical extension (canonical library)}: spacetime causal structure
...
\item \textbf{Mass relations and physical spectrum} (\S\ref{sec:massrelations}):
  The \texttt{MassRelations} layer (39 modules) covers
...
\S\ref{sec:spacetime} covers the spacetime and causal-structure formalization (canonical library).
...
The artifact is organized into 438~\lean{} modules across multiple organizational layers
(six modules migrated to \texttt{ugp-physics-lean}; see below). \emph{(2026-09-13: corrected
from 442 — a dead/orphaned-file audit removed 4 non-compiling files
(\texttt{ElegantKernel/Unconditional/UCLMassOrderingBridge.lean},
\texttt{Substrate/SechOverlapIntegralBounds\_\{r5mesh,r5bins,cosh\}.lean}) and wired in
one previously-orphaned-but-sound file (\texttt{Spacetime/UniversalSimulation.lean});
see \texttt{docs/MODULES.md} for full evidence. This source correction has not yet been
accompanied by a new Zenodo version of this paper as of this edit.)}
...
\node[layer, fill=orange!8, below=of class] (gte)
  {\textbf{GTE}~(27)\quad Evolution, Orbit, UpdateMap,
   ...
   \textit{new:} RidgeDerivation, FibonacciDerivation, SemanticFloorDerivation, PSCColorRankBundle};
...
\node[layer, fill=teal!8,   below=of struc] (mass)
  {\textbf{MassRelations}~(40)\quad KoideClosedForm, KoideNewtonFlow,
   ...
   \textit{new:} \textbf{SeesawTrialityPinning}};
...
\node[layer, fill=purple!8, below=of ipt]   (univ)
  {\textbf{Universality}~(66)\quad Rule110, UWCA, UWCASimulation,
   ...
   FockSpaceKink, \textbf{MDLDerivabilityCriterion}, ThooftEffectMeasureBridge,
   ...
   \textit{new:} \textbf{PhiMDLFockSpaceParticles}};
...
\noindent\textit{(continued below: SelfRef, Framework, Spacetime, Substrate, Algebra layers)}
...
\node[layer, fill=olive!8,  below=of fwk]   (space)
  {\textbf{Spacetime}~(35)\quad CausalGraph, CausalInvariance, LiftingTheorem,
   ...
   \textbf{PhiMDLZeroModeIndex};
   \textit{new:} \textbf{SpacelikeCutAreaScaling}, \textbf{ThreeTapeCutAreaScaling}
   (+ 5 Spectral sub-modules; canonical library, zero sorry unless noted)};
\node[layer, fill=brown!6,  below=of space] (subs)
  {\textbf{Substrate}~(12)\quad Substrate, LExtended, LagrangianLorentzScalar,
   ...
   \textbf{WindingCoinDecoupling} (canonical library, zero sorry)};

\node[layer, fill=magenta!6, below=of subs] (alg)
  {\textbf{Algebra}~(23)\quad EisensteinFunctor, F21SU3Embedding, FanoRegularAction,
   GaugeMDL, SMGaugeGroup, SRRGCABridge, PolynomialContinuumBridge,
   BaryonNumber, ChargeFromPolynomial, ChiralDoublet,
   ColorConfinementMDL, CyclotomicZ7Galois, FlavorGroupStructure,
   RSCodeOrbit, SU3GluonCount;
   \textit{new (octonion cluster):}
   \textbf{OctonionShadowInterface}, \textbf{HurwitzCosetCertificate},
   \textbf{G2StabilizerCertificate}, \textbf{TrialityInterface},
   \textbf{OctonionColorFlavorDisambiguation},
   \textbf{KinkSigmaParityAction}, \textbf{KinkSectorTrialityAction},
   \textbf{BraidAtlasPhaseEquivariance}
   (support layer; used by BraidAtlas, Universality, Framework; zero sorry)};

\draw[arr] (self.south)  -- (fwk.north);
\draw[arr] (fwk.south)   -- (space.north);
\draw[arr] (space.south) -- (subs.north);
\draw[arr] (subs.south)  -- (alg.north);
\end{tikzpicture}
\end{center}
...
decidable fragment of the SM-uniqueness claim.
Five further theorems close the Layer~I PSC enumeration by \texttt{native\_decide}
over all 34,560 \texttt{UniverseParams} candidates (zero \texttt{sorry}):
\texttt{universe\_params\_card} certifies the search space cardinality;
\texttt{psc\_layer\_i\_certificate} and \texttt{psc\_12\_survivors\_card}
confirm exactly 12 Layer~I survivors;
\texttt{psc\_enumeration\_forces\_ngen\_3} proves every Layer~I survivor has
$N_{\mathrm{gen}} = 3$;
\texttt{psc\_12\_survivors\_have\_ngen\_3} and
\texttt{psc\_admissible\_forces\_sm\_gauge} bundle the survivor-cardinality and
SM-gauge consequences.
The full soft-constraint $D$-minimizer over all 14 scan constraints remains
the residual structural target (computational certificate in
...
The \texttt{TE22} module is zero-\texttt{sorry}: every theorem in
\texttt{UgpLean.TE22.ScanCertificate} closes by algebraic reasoning,
\texttt{decide}, or \texttt{native\_decide}, including the 34,560-universe
Layer~I PSC enumeration (\texttt{psc\_enumeration\_forces\_ngen\_3},
\texttt{psc\_12\_survivors\_have\_ngen\_3}).  The residual open programme is
full soft-constraint $D$-minimization over all 14 scan constraints; the
Layer~I survivor structure and $N_{\mathrm{gen}} = 3$ consequence are
machine-certified.

\medskip\noindent\textbf{Documented axioms (98 global named axioms).}
...
An audit discharged several previously declared axioms in
\texttt{VEVProof/GoldstoneEntropyCorrection.lean} as proved theorems;
see \S\ref{sec:audit-capstones} for details.
The library currently holds \textbf{98} global custom named axioms,
counted by the reproducible command
\lstinline!grep -rhoE "^axiom [a-zA-Z0-9_']+" UgpLean --include="*.lean" | sort -u | wc -l!
(top-level \texttt{axiom} declarations across every \texttt{.lean} file, deduplicated by
name; standard Lean/Mathlib logical axioms \texttt{propext}, \texttt{Classical.choice},
\texttt{Quot.sound} are excluded, as they are not disclosed physics/mathematics premises
in the sense meant here), primarily in \texttt{MassRelations.TranscendentalMassBounds},
\texttt{Universality.CUP3DPhysicalIncompleteness}, \texttt{Substrate.WightmanAxioms},
\texttt{Gravity.PageWoottersZ7}, \texttt{Gravity.PMDLGravityTheorems}, and related
modules (see Table~\ref{tab:axiom-inventory} and \S\ref{subsec:frontier-closures}).
The following named axioms were proved as genuine theorems (zero sorry):
five \lstinline|: True|-type and existential axioms in \texttt{GaugeMDL},
\texttt{CCImpossibilityBundle}, \texttt{GUTStructure}, and \texttt{PMDLGravityTheorems};
and \lstinline|adj_mem_periodicCausalNeighbors| in \texttt{SpectralDimensionDegree},
proved via complete case analysis over all 20 adjacency branches.
The core physics proof path (charged leptons, couplings, generation count) remains
zero-custom-axiom.

\medskip\noindent\textbf{One additional disclosed axiom (\texttt{BraidAtlas.ChargeTheorem}):}
...
\subsection{Ridge constant derivation (\texttt{GTE.RidgeDerivation})}\label{subsec:ridge-derivation}

The \texttt{GTE.RidgeDerivation} module derives the ridge constant $c_{\rm ridge} = 16$
from first principles: the multiplicative structure of $\mathbb{Z}_7 \times \mathbb{Z}_3$
and the divisibility conditions imposed at ridge $n = 10$.

\begin{theorem}[GT-001/GT-009 ridge closure bundle]\label{thm:ridge-closed}
The ridge constant $16 = 2^{1 + \mathrm{ord}_7(2)}$ is the \emph{unique} power of~$2$
compatible with the $\mathbb{Z}_7 \times \mathbb{Z}_3$ divisibility condition at $n = 10$.

\begin{leanref}
\lstinline|gt001_gt009_ridge_closed|, \lstinline|ridge_constant_from_gf7_z3|,
\lstinline|ridge_divisibility_at_n10|, \lstinline|ridge_offset_unique_power_of_2|
\textup{(in \texttt{GTE.RidgeDerivation}, zero \texttt{sorry})}
\end{leanref}
\end{theorem}

\subsection{Fibonacci structure derivation (\texttt{GTE.FibonacciDerivation})}\label{subsec:fib-derivation}

The \texttt{GTE.FibonacciDerivation} module shows that the three key orbit parameters
$q_2$, $d$, and the generation gap are all consequences of the $\mathbb{Z}_7 \times \mathbb{Z}_3$
structure, with Fibonacci numbers appearing as a \emph{derived} output rather than an input.

\begin{theorem}[GT-010: $q_2$ from $\mathbb{Z}_7$ and $N_c$]\label{thm:q2-from-z7}
$q_2 = N_c \times 2^{\mathrm{ord}_7(2)}$ follows from the multiplicative order of~$2$
in $\mathbb{Z}_7$ and the QCD colour rank $N_c = 3$.

\begin{leanref}
\lstinline|gt010_q2_from_z7_nc|
\textup{(in \texttt{GTE.FibonacciDerivation}, zero \texttt{sorry})}
\end{leanref}
\end{theorem}

\begin{theorem}[GT-011: ridge offset equals Pisano period $\pi(7)$]\label{thm:pisano-7}
The ridge offset $d = 16$ equals the Pisano period $\pi(7)$, the period of the
Fibonacci sequence modulo~$7$.

\begin{leanref}
\lstinline|gt011_ridge_offset_eq_pisano_7|
\textup{(in \texttt{GTE.FibonacciDerivation}, zero \texttt{sorry})}
\end{leanref}
\end{theorem}

\begin{theorem}[GT-012: generation gap from $\mathbb{Z}_7 \times \mathbb{Z}_3$]\label{thm:gen-gap}
The generation gap $= F_{\mathrm{ord}_7(2) + N_c + 1}$ is derived purely from
$\mathbb{Z}_7 \times \mathbb{Z}_3$ structure.

\begin{leanref}
\lstinline|gt012_gap_from_z7_z3|
\textup{(in \texttt{GTE.FibonacciDerivation}, zero \texttt{sorry})}
\end{leanref}
\end{theorem}

\subsection{Semantic floor derivation (\texttt{GTE.SemanticFloorDerivation})}\label{subsec:semantic-floor-derivation}

The \texttt{GTE.SemanticFloorDerivation} module proves that the SemanticFloor
condition $a \in \{1, 9, 5\}$ is not a hardcoded sieve parameter but is
the orbit projection of the cascade formula $a' = m - (n+2-t)$.

\begin{theorem}[SemanticFloor from cascade]\label{thm:semantic-floor-cascade}
The SemanticFloor set $\{1, 9, 5\}$ is exactly the orbit projection of
$a' = m - (n+2-t)$ at ridge $n = 10$, certified by exhaustive
cascade computation.

\begin{leanref}
\lstinline|semantic_floor_a_condition_from_cascade|, \lstinline|a_orbit_values_at_n10|
\textup{(in \texttt{GTE.SemanticFloorDerivation}, zero \texttt{sorry})}
\end{leanref}
\end{theorem}

\subsection{Mersenne ladder uniqueness (\texttt{GTE.MersenneLadder} — extended)}\label{subsec:mersenne-ladder-unique}

In addition to the formulas described in~\S\ref{subsec:mersenne-ladder}, the
\texttt{GTE.MersenneLadder} module now includes a uniqueness theorem establishing
that no other ladder structure is admissible.

\begin{theorem}[Mersenne ladder uniqueness]\label{thm:mersenne-unique}
The even-step capacity $c' = 2^{n + 2N_c} - 1$ is the \emph{unique} Mersenne
candidate selected by double-Fibonacci minimality; all other Mersenne ladder
values fail the minimality criterion.

\begin{leanref}
\lstinline|mersenne_ladder_uniqueness_proved|
\textup{(in \texttt{GTE.MersenneLadder}, zero \texttt{sorry})}
\end{leanref}
\end{theorem}

\subsection{PSC colour-rank bundle (\texttt{GTE.PSCColorRankBundle})}\label{subsec:psc-color-rank}

The \texttt{GTE.PSCColorRankBundle} module certifies that $N_c = 3$ is forced
by the PSC, and that the generation count equals the colour rank.

\begin{theorem}[GT-013: $N_c = 3$ from PSC, Frobenius prime, and FGCI]\label{thm:gt013-psc-nc}
The PSC, combined with the Frobenius prime constraint and the
Frobenius–Generation Coincidence Identity (FGCI), forces $N_c = 3$.

\begin{leanref}
\lstinline|gt013_psc_nc_bundle|
\textup{(in \texttt{GTE.PSCColorRankBundle}, zero \texttt{sorry})}
\end{leanref}
\end{theorem}

\begin{theorem}[$N_{\rm gen} = \mathrm{colorRank}$]\label{thm:ngen-color-rank}
PSC enumeration forces $N_{\rm gen} = \mathrm{colorRank}$, providing an algebraic
identification of the generation count with the QCD colour rank.

\begin{leanref}
\lstinline|psc_implies_ngen_eq_colorRank|
\textup{(in \texttt{GTE.PSCColorRankBundle}, zero \texttt{sorry})}
\end{leanref}
\end{theorem}
...
quotient form $\mathbb{Q}[X]/\langle p\rangle$, which avoids an
\texttt{AdjoinRoot} typeclass-synthesis obstruction.
...
The module \texttt{CyclotomicCompleteness.CoxeterBiconditional} formalises the arithmetic backbone of the
...
The module \texttt{CyclotomicCompleteness.CyclotomicContainment} proves the field-theoretic content of the Coxeter--conductor theorem:
...
the structural Lean certificate eliminating the \lstinline|mirror_winding_number_zero|
...
across five prior formalization passes.  It imports
...
This result is therefore \textbf{Category~D (numerical,
...
Cat~D — structural derivation open)}
\end{leanref}
\end{theorem}
...
The module \texttt{BraidAtlas.DarkQuarkCharge} certifies dark quarks
...
The proof is identical in structure to the lepton case (
...
\begin{theorem}[Dark quark electric neutrality]\label{thm:dark_quark_neutral}
...
\lstinline|circle_exp_surjective_MAIN| (zero sorry):
...
\begin{theorem}[Nine-coefficient UCL master certification]\label{thm:ucl-master-083c}
All nine Elegant Kernel coefficients---$k_{L^2}$, $k_{\rm gen}$, $k_{\rm gen}^{(2)}$,
$k_L$, $k_{\rm const}$, $k_M$, and the Möbius triple $(k_a,k_b,k_c)$---are
simultaneously certified in one \CatAL\ bundle, with
$k_M = -\varphi/2 + 7/2048$ (Quarter-Lock) and tier-2/3 mass-spectrum consequences
(fermion mass ordering and lepton-sector Koide $Q=2/3$).

\begin{leanref} \lstinline|elegant_kernel_full_certification|,
\lstinline|thm_quarter_lock_catal|,
\lstinline|ucl_fermion_mass_ordering|,
\lstinline|ucl_lepton_sector_koide_identity|
\textup{(in \nolinkurl{ElegantKernel/Unconditional/MasterCertification},
\nolinkurl{QuarterLock}, \nolinkurl{UCLMassOrdering}, \nolinkurl{UCLKoide})}
\end{leanref}
\end{theorem}

\noindent
Fibonacci eigenvalue structure is certified in
\texttt{GTE/StructuralTheorems}
(\lstinline|neg_inv_golden_ratio_minimal_poly|);
the unconditional UCL$_2$ block is
\lstinline|thm_ucl2_summary| in \texttt{ElegantKernel/Unconditional/KGenFullClosure}.

% ═══════════════════════════════════════════════════════════════
\section[Mass Relations]{Mass Relations and Physical Spectrum}\label{sec:massrelations}
% ═══════════════════════════════════════════════════════════════

The \texttt{MassRelations} layer (39 modules in the canonical library, including
\texttt{NeutrinoSector} and \texttt{HiggsQuartic}) formalizes the
...
\subsection{CKM CP phase and Wolfenstein \texorpdfstring{$A$}{A} parameter (\texttt{MassRelations.CKMCPPhase})}\label{subsec:ckm-cp-phase}

The module \texttt{MassRelations.CKMCPPhase} certifies the GTE subgroup-order
reduction for the CKM CP-violating phase and the down-sector Wolfenstein parameter
from the certified orders $|H_{\rm lep}|=6$, $|H_{\rm down}|=3$, $|H_{\rm up}|=2$:
\[
\delta_{\rm CP} = \frac{\pi}{2} - \frac{|H_{\rm down}|}{|H_{\rm lep}|+|H_{\rm up}|}
= \frac{\pi}{2} - \frac{3}{8},
\qquad
A = \sin\!\left(\frac{\pi}{|H_{\rm down}|}\right) = \sin(\pi/3) = \frac{\sqrt{3}}{2}.
\]
The rational structure $\sin^2(\pi/3)=3/4$ is certified; the phase reduction ratio
$3/8$ is distinct from all neighbouring subgroup-order null variants.

\begin{theorem}[CKM CP phase formula, zero \texttt{sorry}]\label{thm:ckm-cp-phase-formula}
The CP-phase reduction ratio is $|H_{\rm down}|/(|H_{\rm lep}|+|H_{\rm up}|)=3/8$.

\begin{leanref}\lstinline|ckm_cp_phase_formula|,
\lstinline|cp_phase_reduction_ratio|,
\lstinline|cp_phase_ratio_distinct_from_nulls|
\textup{(in \texttt{MassRelations.CKMCPPhase}, zero \texttt{sorry})}\end{leanref}
\end{theorem}

\begin{theorem}[CKM Wolfenstein $A$ parameter, zero \texttt{sorry}]\label{thm:ckm-a-parameter}
The down-sector Wolfenstein parameter satisfies $A^2=\sin^2(\pi/3)=3/4$.

\begin{leanref}\lstinline|ckm_A_parameter_squared|,
\lstinline|ckm_A_parameter_sin2|
\textup{(in \texttt{MassRelations.CKMCPPhase}, zero \texttt{sorry})}\end{leanref}
\end{theorem}
...
The \ugp{} substrate is Turing-universal via a register-machine reduction:

\begin{enumerate}[nosep]
\item The Universal Windowed Cellular Automaton's (\uwca{}) CRT arithmetic register
  file simulates Minsky-style two-counter machines step-for-step (machine-checked,
  zero \texttt{sorry}).
\item Minsky's (1967) theorem: two-counter machines are equivalent to Turing
  machines / partial-recursive functions.  This is a named classical axiom
  (\lstinline|minsky_counter_machine_turing_complete_1967|), not mechanized ---
  Mathlib has no counter-machine or register-machine development as of the
  version used here.
\end{enumerate}

Separately, and not part of this reduction, the \uwca{}'s binary sweep layer exactly
simulates Rule~110 (machine-checked, zero \texttt{sorry},
\lstinline|uwca_sweep_implements_rule110|), and Rule~110 is itself Turing-universal
via Cook's theorem~\cite{Cook2004,Wolfram1984,Wolfram2002}
(\lstinline|cook_operational_stage3_tm_microstep_readback| in \texttt{rule110-lean},
formerly \lstinline|rule110_turing_universal_from_cook|, zero \texttt{sorry} modulo five
named classical Cook collision-analysis bridge axioms).  A finite-window binary
tile embedding of Rule~110 does not by itself establish Turing universality on the
\uwca{} substrate, since Cook's construction requires an unbounded tape; the
register-machine route above is the substrate's actual Turing-universality
certificate.
...
The \ugp{} substrate is Turing-universal, via its CRT arithmetic register file
simulating Minsky two-counter machines (conditional on one named classical axiom).

\begin{leanref} \lstinline|ugp_is_turing_universal|
\textup{(\texttt{Universality.TuringUniversal}; \CatAL{} conditional, zero
\texttt{sorry}, one named axiom:
\lstinline|minsky_counter_machine_turing_complete_1967|)}
\end{leanref}
\end{theorem}
...
\medskip
Two structural properties distinguish this from a generic encoding argument:

\medskip
\noindent\textbf{(1) Native emergence, not external wiring.}
The \uwca{}'s CRT register file is not imposed on the \ugp{} from outside: it is the
same native arithmetic substrate (prime-residue windows, ALU tiles) that hosts the
survivor-space geometry.  The register-file interpreter (\lstinline|uwca_counter_step|,
\lstinline|uwca_register_run|) is a zero-sorry, zero-axiom simulation of Minsky counter
machines on this substrate; only Minsky's classical equivalence theorem (counter
machines $\simeq$ Turing machines) is taken as an axiom, since Mathlib has no
counter-machine formalization.  Separately, the \uwca{}'s binary sweep layer is also
native: the five tiles correspond exactly to the five minterms of the Rule~110 table
($S_{110} = \{1,2,3,5,6\}$), and the four passes (P1--P4) map directly onto the GTE
update cycle.  There is no separate ``encoding step'' for either layer.

\medskip
\noindent\textbf{(2) The Rule-110 tile set is certified minimal.}
The 5-tile binary-sweep set is not just sufficient; it is \emph{necessary}: removing any
...
A generic Gödel-numbering argument gives no such minimality guarantee for the binary
layer---it provides a sufficient encoding, not a tight one.  (This minimality result
concerns the Rule-110 tile embedding specifically; the substrate's Turing universality
itself rests on the register-machine reduction above, not on this tile set.)
...
\begin{theorem}[GEN$_2$/GEN$_3$ positional topology degeneracy]\label{thm:gen23-topo}
The states $\mathrm{GEN}_2 = [2,5,2,0,2]$ and $\mathrm{GEN}_3 = [5,6,5,3,5]$
share the structural topology $(a,b,a,c,a)$: positions $\{0,2,4\}$ carry a single
repeated winding value, and positions $1$ and $3$ carry distinct minority values.
$\mathrm{GEN}_1 = [1,5,2,2,1]$ has the distinct topology $(a,b,c,c,a)$.
Formally, for each of GEN$_2$ and GEN$_3$: the cell values at positions 0, 2, 4
are equal, and the values at positions 1 and 3 differ from the outer value.

\noindent\emph{Physical consequence.}
Under WolframModel's topological (pattern-based) matching, the orbit rules
$\mathrm{GEN}_i \to \mathrm{GEN}_{i+1}$ match on canonical topology rather
than literal values.  Since GEN$_2$ and GEN$_3$ share the same topology,
both the rule $\mathrm{GEN}_2\!\to\!\mathrm{GEN}_3$ and the rule
$\mathrm{GEN}_3\!\to\!\mathrm{VAC}$ match any state with topology $(a,b,a,c,a)$
simultaneously.  In the multiway expansion, both fire at each step, producing
binary branching and a $2^{10}-1 = 1023$-event causal tree at depth~10.
This is the structural reason that the GTE WolframModel causal graph is a
fractal binary tree~(P49~\S5.3).

\begin{leanref}
\lstinline|gen2_outer_positions_constant|,
\lstinline|gen3_outer_positions_constant|,
\lstinline|gen2_gen3_topology_degenerate|
\textup{(in \texttt{Universality.CUP3DUniqueness}, zero \texttt{sorry},
\texttt{decide})}
\end{leanref}
\end{theorem}

\paragraph{Measurement orbit uniqueness and three-level bundles.}
\begin{theorem}[\lstinline|fmdl_orbit_is_unique_psc_trajectory|, zero \texttt{sorry}]
The orbit $\mathrm{GEN}_1\to\mathrm{GEN}_2\to\mathrm{GEN}_3\to\mathrm{VAC}$ is the
uniquely forced 3-step trajectory from $\mathrm{GEN}_1$ under the deterministic
$\mathsf{fmdl\_step5}$ map. Step-uniqueness at each stage and the Garden-of-Eden property
of $\mathrm{GEN}_1$ are included. (All by \texttt{decide} or previous theorems; zero \texttt{sorry}.)
\end{theorem}

\noindent Two partial bundle theorems certify pieces of the Three-Level MDL Unification
(P51~\S5): \lstinline|mdl_three_level_orbit_bundle| (Level~1$\to$2 orbit steps, \CatAL) and
\lstinline|mdl_three_level_polynomial_bundle| (orbit + SRRG $\varphi$ diagonal fixed point, \CatAL).
The full cross-module unification remains open.
...
initial tape.  This is a theorem (zero \texttt{sorry}), proved from the named axiom
\lstinline|cook_rule110_simulates_computable| in \texttt{Universality.CookComputableBridge}
(Cook's operational universality theorem
\lstinline|cook_operational_stage3_tm_microstep_readback| in \texttt{rule110-lean},
formerly \lstinline|rule110_turing_universal_from_cook|,
composed with universal Turing-machine/partial-recursive compilation), and underlies
...
(canonical library; zero \texttt{sorry} on all theorems listed below;
...
Klein-Gordon substrate) is Turing universal via Cook's theorem composed with a
proved stepwise Rule-110 simulation, and additionally certifies a Cook-independent
finite Boolean functional-completeness result (Route~2, below) that is distinct
from and not sufficient for Turing universality, plus a complementary A-glider
period-3 certificate.
...
\begin{keybox}{GF(7) finite Boolean functional completeness (Cook-independent, Route~2)}
\textbf{Theorem \lstinline|z7_bool3_finite_functional_completeness|.}
Every \lstinline|Bool -> Bool -> Bool -> Bool| function has a representative over
\lstinline|ZMod 7| on Boolean inputs, via the GF(7) polynomial representation.
This is a \emph{finite} Boolean functional-completeness result about a fixed input
size; it does \textbf{not} imply Turing universality on an unbounded tape.  Zero
\texttt{sorry}.  (The former unsound axiom
\lstinline|z7_boolean_completeness_implies_turing_universal|, which purported to
bridge this finite result to Turing universality via a Shannon-style TM$\to$circuit
argument, has been removed as unsound and no longer exists in the corpus.)
\end{keybox}
...
\begin{keybox}{$\Phi_{\rm MDL}$ Turing universality (Cook route)}
\textbf{Theorem \lstinline|phimdl_turing_universal|.}
$\Phi_{\rm MDL}$ is Turing universal.  Proved by composing
\lstinline|CookComputableBridge.cook_rule110_simulates_computable| (Cook's
operational universality theorem
\lstinline|cook_operational_stage3_tm_microstep_readback| in \texttt{rule110-lean},
formerly \lstinline|rule110_turing_universal_from_cook|,
composed with universal TM/partial-recursive compilation) with the proved
stepwise $\Phi_{\rm MDL}$--Rule~110 simulation
(\lstinline|phiMDL_evolution_simulates_rule110|).
Zero \texttt{sorry}; conditional on one named axiom:
\lstinline|cook_rule110_simulates_computable|.
\end{keybox}
...
\lstinline|z7_bool3_finite_functional_completeness| & 0 & Finite Boolean functional completeness, Cook-independent (not Turing universality) \\
...
\lstinline|phimdl_turing_universal| & 1 named & $\Phi_{\rm MDL}$ Turing universal, via the Cook route (\lstinline|cook_rule110_simulates_computable|) \\
\bottomrule
\end{tabular}
\caption{Theorems in \texttt{PhiMDLUniversality.lean}.  All are zero \texttt{sorry};
\lstinline|phimdl_turing_universal| is conditional on one named axiom,
\lstinline|cook_rule110_simulates_computable| (\texttt{CookComputableBridge.lean}).
The former unsound axiom \lstinline|z7_boolean_completeness_implies_turing_universal|
(a purported Shannon-style TM$\to$Boolean-circuit bridge) has been removed.}
\label{tab:phimdl-universality}
\end{table}

Cook's theorem~\cite{Cook2004,Wolfram1984} --- that Rule~110 is Turing universal via cyclic
tag system embedding --- remains the sole load-bearing route to Rule~110's (and hence
$\Phi_{\rm MDL}$'s) Turing universality; it is not a corollary of the finite Boolean
functional-completeness identity above, which is a separate, correctly-scoped result about
a fixed input size.
The algebraic characterization of Rule~110 as a polynomial over $\text{GF}(7)$ and its role
in Cook-independent finite Boolean functional completeness are documented in the companion
paper~\cite{SpivackGF7Universality}.
...
\Rightarrow \text{Turing universality (register-machine route)}$ in Lean.
...
\texttt{IsPSC U} is computationally universal, where \texttt{IsComputationallyUniversal}
is defined as \lstinline|UGP_substrate_turing_universal|.
...
\textup{(in \texttt{Universality.PSCUniversality}, zero \texttt{sorry}; conditional on
one named axiom \texttt{minsky\_counter\_machine\_turing\_complete\_1967}, via
\lstinline|UGP_substrate_turing_universal|)}
\end{leanref}
\end{theorem}
...
\[
  ...
  \;\Rightarrow\; \text{Turing universality (register-machine route).}
\]
...
The final arrow concludes \lstinline|UGP_substrate_turing_universal|, which is
established via the \uwca{}'s CRT register file simulating Minsky two-counter
machines, conditional on the named axiom
\lstinline|minsky_counter_machine_turing_complete_1967|.
The gauge-to-orbit step carries zero \texttt{sorry} and no additional axioms.
...
two named axioms: \texttt{rcc\_physics\_ax} from \texttt{PSC.RCCComplete}, and
\texttt{minsky\_counter\_machine\_turing\_complete\_1967} from
\texttt{Universality.RegisterMachine})}
\end{leanref}
\end{theorem}
...
\subsection{QFT-Level Mass Gap (\texttt{GaugedMassGap.lean})}\label{subsec:gauged-mass-gap}
...
(\lstinline|gte_mass_gap|), the 2D string tension $\sigma_{2\mathrm{D}} > 0$,
...
Composes the QFT mass gap with the color-confinement framework.
\end{theorem}

All theorems are zero \texttt{sorry}, zero custom axioms.

\subsection{$\Phi_{\mathrm{MDL}}$ fluctuation spectrum and Yukawa vertex
(\texttt{Substrate/PhiMDLFluctuationSpectrum})}\label{subsec:phimdl-fluctuation-083c}

The \texttt{PhiMDLFluctuationSpectrum} module certifies the loop-level
fluctuation potential and Yukawa overlap infrastructure used in the P42
$\Phi_{\mathrm{MDL}}$ field paper.  The fluctuation Lagrangian reduces to a
Pöschl--Teller well $V_{\rm fl}(x)=m_\phi^2[1-2\,\mathrm{sech}^2(m_\phi x)]$
(\lstinline|phimdl_fluctuation_is_poschl_teller|, \CatAL).  The sech integrals
$\int\mathrm{sech}\,dx=\pi$ and $\int\mathrm{sech}^3\,dx=\pi/2$ are closed via
Mathlib's fundamental theorem of calculus (\lstinline|integral_sech|,
\lstinline|integral_sech_cubed|, \CatAL).  The three-kink overlap amplitude
feeds \lstinline|phimdl_yukawa_vertex_catad| (\CatAD), and the asymptotic
overlap law \lstinline|sech_overlap_asymptotic| certifies $r\cdot I(r)\to\pi$.
Finite-$r$ lower bounds \lstinline|sech_overlap_at_five_ge| and
\lstinline|sech_overlap_at_eleven_ge| are zero-\texttt{sorry} in
\texttt{PhiMDLFluctuationSpectrum} via import from the companion stack
\texttt{Substrate/SechOverlapIntegralBounds}: the main module has zero
\texttt{sorry} and zero custom axioms; certified rational mesh bounds live in
\texttt{SechOverlapIntegralBounds\_cosh} and
\texttt{SechOverlapIntegralBounds\_r5bins} (all \CatAL); the only
non-computational step is isolated as two named \CatA\ axioms in
\texttt{SechOverlapIntegralBounds\_bridge}
(\lstinline|sech_overlap_at_five_ge_certified|,
\lstinline|sech_overlap_at_eleven_ge_certified|), recording the antitone
right-Riemann mesh-sum $\le$ integral comparison pending a Mathlib
\lstinline|AntitoneOn.sum_le_integral_Ico| proof.
The companion module \texttt{Gravity/YukawaOverlapExponent} derives the Dirac
scaling exponent $\alpha=N_c-1=2$ from three-tape DPP counting
(\lstinline|yukawa_overlap_exponent_catad|, \CatAD) and the leptogenesis
suppression $f_1 f_2 = 1/3025$ (\lstinline|leptogenesis_kink_overlap_catad|,
\CatAD\ conditional on the sech bridge axioms).
The Planck 2018 CMB+BBN baryon asymmetry
$\eta_B = (6.10\pm 0.06)\times 10^{-10}$ is bracketed by the GTE kink-overlap
interval certified in \lstinline|eta_B_PDG_in_GTE_bracket|
(\CatAL\ conditional on $\varepsilon_1^{\mathrm{CI}}=3.98\times 10^{-5}$ and
$\kappa=0.190$): sech lower bounds
\lstinline|sech_overlap_at_five_ge_pi|,
\lstinline|sech_overlap_at_eleven_ge_pi| (zero \texttt{sorry}) and the asymptotic
upper bound \lstinline|yukawa_suppression_asymptotic_is_upper_bound|.
The leptogenesis loop-function bracket
\lstinline|eta_B_loop_bracket| certifies Planck $\eta_B$ in
$[6.06,6.36]\times 10^{-10}$ from asymptotic loop
$f_{\mathrm{asym}}(x)=-3/(2\sqrt{x})$ ($\varepsilon_1=3.7982\times 10^{-5}$) and exact CRV loop
($\varepsilon_1^{\mathrm{CI}}=3.98\times 10^{-5}$); zero \texttt{sorry}.
...
\subsection{Chiral Symmetry Breaking (\texttt{ChiralSymmetryBreaking.lean})}\label{subsec:chiral-ssb}
...
and GTE quark masses $m_u = 2.16\,\mathrm{MeV}$,
...
$\CatAL$ (mesoscopic form); the spatial Rule~110 glider
...
generation orbit (zero \texttt{sorry}, zero named axioms;
...
as $\mathbb{Z}_7$ species-winding arithmetic (CatAL; zero sorry,
...
f$_{\rm MDL}$ under the two-layer (Rule~110 + Rule~124) chiral CA pair (CatAL;
...
$\CatAL$ (colorless arm); the gluon ($W_B=0$,
colored) arm has separate status.

\subsection{Invariant Subset Uniqueness (\texttt{Z7InvariantSubsets})}\label{subsec:z7-invariant-subsets}

The \texttt{Z7InvariantSubsets} module formalizes invariant-subset classification for
the full GTE polynomial $p(L,C,R) = C + R - CR - LCR$ over $\mathbb{Z}/7\mathbb{Z}$
(Object~0: no $f_{\rm MDL}$ orbit-position filtering).

\begin{theorem}[\lstinline|p_poly_invariant_subsets_classification|, zero \texttt{sorry}]
The only subsets $S \subseteq \mathbb{Z}_7$ closed under $p$ (i.e.\ $p(S\times S\times S)\subseteq S$)
are $\emptyset$, $\{0\}$, $\{0,1\}$, and $\mathbb{Z}_7$ itself.
Proved by \lstinline|native_decide| over all $\binom{7}{k}$ candidate subsets.
\end{theorem}

\begin{theorem}[\lstinline|rule110_unique_proper_invariant_subca|, zero \texttt{sorry}]
Among non-empty proper subsets of $\mathbb{Z}_7$ closed under $p$, only $\{0\}$ and
$\{0,1\}$ remain.  The binary sublayer $\{0,1\}$ is the unique maximal proper
invariant sub-cellular automaton, matching Rule~110 on the binary sector.
\end{theorem}

\begin{theorem}[QNR binary floor, zero \texttt{sorry}]
$N_{\rm fam} = 5$ is a quadratic non-residue mod~7
(\lstinline|five_is_qnr_mod7|); the fixed-point equation $k^2+k-1\equiv 0\pmod 7$
has no solution (\lstinline|kink_fixed_point_eq_no_solution|,
\lstinline|no_intermediate_fixed_point|).
The master certificate \lstinline|nfam_qnr_explains_binary_floor| combines these
arithmetical facts with \lstinline|p_poly_invariant_subsets_classification|,
supplying an algebraic explanation for why $\{0,1\}$ is the unique non-trivial
invariant sub-CA of the unrestricted GTE polynomial.
\end{theorem}

\subsection{GTE Polynomial Explorations (\texttt{Polynomial.PolyExplorations})}\label{subsec:poly-explorations}

The \texttt{Polynomial/PolyExplorations.lean} module (35 theorems, zero \lstinline|sorry|)
formalizes the core mathematical properties of $p(L,C,R)=C+R-CR-LCR$ over $\mathbb{F}_7$,
its relationship to the integer update map $T$, period-475 certificates for the 5-cell ring,
and the GF($7^3$) number-theoretic structure underlying the factorization of the period.

\paragraph{Universal algebraic identities.}
\begin{theorem}[\lstinline|poly_p_at_111_eq_zero|, zero \texttt{sorry}]
Over any commutative ring $R$, $p(1,1,1)=1+1-1\cdot1-1\cdot1\cdot1=0$.
Proved by \lstinline|ring|.
\end{theorem}

\begin{theorem}[\lstinline|gf5_has_fixed_point|, \lstinline|gf5_second_ether|, zero \texttt{sorry}]
In $\mathrm{GF}(5)$: $k=2$ satisfies $k^2+k-1=0$ (\lstinline|decide|), and $p(2,2,2)=2$
(\lstinline|decide|), confirming a second stable ether at $w=2$.  This is the algebraic
mechanism by which $\mathrm{GF}(5)$ supports a spurious invariant sector that PSC rejects.
\end{theorem}

\paragraph{Ground-state characterization and diagonal factorization.}
\begin{theorem}[\lstinline|poly_p_uniform_gs_roots|, zero \texttt{sorry}]
In $\mathbb{Z}/7\mathbb{Z}$, $p(x,x,x)=-x(x-1)(x-5)$, and the ground states of the
uniform spin-7 configuration are exactly $\{0,1,5\}$ --- the roots of this diagonal cubic.
Proved by \lstinline|decide|.
\end{theorem}

This machine-certifies the $\mathbb{Z}_3$-symmetric ordered phase of the spin-7 lattice model.
Companion theorems \lstinline|poly_p_diagonal_factorization| and
\lstinline|poly_p_diagonal_plus_factor_eq_zero| certify the equivalent form
$p(x,x,x)=-x(x-1)(x+2)$ and the underlying ring identity.

\paragraph{T map: non-polynomial over $\mathrm{GF}(7)$.}
\begin{theorem}[\lstinline|T_b_output_not_determined_by_mod7|, \lstinline|T_computable_not_polynomial_GF7|, zero \texttt{sorry}]
The integer update map $T$ is computable but not polynomial over $\mathrm{GF}(7)$:
the counterexample $b=73$, $c=144$ vs $c=151$ (with $144\equiv 151\pmod7$) gives
distinct outputs mod~7 from identical inputs mod~7.  Proved by \lstinline|native_decide|.
\end{theorem}

\paragraph{Vacuum basin cardinality.}
\begin{theorem}[\lstinline|poly_p_vacuum_basin_card_eq_52|, zero \texttt{sorry}]
Exactly 52 of the $7^5=16807$ states of the 5-cell ring $(\mathbb{Z}/7)^5$ converge
to the vacuum fixed point under iteration of $p$.
Proved by \lstinline|native_decide| with convergence bound 8.
\end{theorem}

\paragraph{Period-475 certificates.}
\begin{theorem}[\lstinline|period_475_returns|, \lstinline|period_475_is_minimal|, zero \texttt{sorry}]
The orbit of the canonical initial state under $p$ on the 5-cell ring returns after
exactly 475 steps, and no proper divisor of 475 is a period.
Both proofs use \lstinline|native_decide|.
\end{theorem}

\begin{theorem}[\lstinline|phi25_order_19_on_cycle|, zero \texttt{sorry}]
The map $\phi^{25}$ has order exactly 19 on the 475-cycle (\lstinline|native_decide|).
\end{theorem}

\paragraph{GF($7^3$) arithmetic: the 19-factor.}
Six theorems certify the number-theoretic structure of the 19-factor in $475=5^2\times19$:
\lstinline|nineteen_divides_7cube_minus_1| ($19\mid 7^3-1=342$, \lstinline|decide|);
\lstinline|ord_19_seven_equals_3| ($\mathrm{ord}_{19}(7)=3$, \lstinline|decide|);
\lstinline|nineteen_not_divides_smaller_extensions| ($19\nmid 7^k-1$ for $k=1,2,4,5$);
\lstinline|nineteen_not_divides_linear_period| ($19\nmid 240$, the linearized period);
\lstinline|nineteen_unique_prime_in_7cube_minus_1|; and
\lstinline|gf73_norm_of_19th_root_is_one| (norm of 19th roots equals 1 in GF($7^3$)).
These collectively identify 19 as the unique prime $q\mid 7^3-1$ with $\mathrm{ord}_q(7)=3$,
connecting the period-475 to the multiplicative structure of $\mathrm{GF}(7^3)$.

\paragraph{GF(7) arithmetic coprocessor.}
\begin{theorem}[\lstinline|p_addition_via_L6|, zero \texttt{sorry}]
Setting $L=6\equiv -1\pmod{7}$ makes $p$ a one-gate addition:
$p(6,a,b) = a+b \pmod{7}$ for all $a,b\in\mathbb{Z}/7\mathbb{Z}$.
Proved by \lstinline|decide|.
\end{theorem}

\begin{theorem}[\lstinline|p_multiplication_from_two_gates|, zero \texttt{sorry}]
Multiplication in $\mathbb{Z}/7\mathbb{Z}$ is recoverable in two $p$-gate evaluations:
$a\cdot b = p(6,a,b) - p(0,a,b) \pmod{7}$.
Proved by \lstinline|decide|, exhaustively verified over all 49 input pairs.
\end{theorem}

\noindent Together these certify a complete $\mathrm{GF}(7)$ arithmetic coprocessor built from $p$-gates alone:
addition in one gate (Lean-certified), multiplication in two gates (Lean-certified).
The Walsh nonlinearity $\mathrm{NL}(p)=36/49\approx0.7347$ (\lstinline|thm:walshNL|, P49)
exceeds AES S-box nonlinearity and is determined by the GTE Higgs branch capacity $c_H=13$.
These properties make $p$ a cryptographically characterized S-box with provably derived nonlinearity.

\paragraph{KL divergence and PSC-projection bundle.}
\begin{theorem}[\lstinline|kl_divergence_fmdl_p_nonzero|, zero \texttt{sorry}]
There exists a triple $(L,C,R)$ such that $p(L,C,R)\neq f_{\mathrm{MDL}}(L,C,R)$, certifying
strictly positive KL divergence between Objects~0 ($p$) and~1 ($f_{\mathrm{MDL}}$).
Witness: $(1,1,5)$ with $p(1,1,5)=3\neq 2=f_{\mathrm{MDL}}(1,1,5)$. Proved by \lstinline|decide|.
\end{theorem}

\noindent The \lstinline|psc_projection_gives_fmdl| bundle certifies the full PSC-projection
characterisation of $f_{\mathrm{MDL}}$: (1)~$f_{\mathrm{MDL}}(L,C,R)=0$ for every free
(non-fixed-point) PSC neighbourhood, and (2)~$f_{\mathrm{MDL}}=p$ on the binary sublayer
$\{0,1\}^3$.  Together these two parts state that $f_{\mathrm{MDL}}$ is the unique map
obtained by zeroing $p$ outside the PSC-orbit sector (Objects~0 and~1 coincide on binary;
diverge only on the non-binary PSC interior).

\subsection{GTE WolframModel Causal Tree (\texttt{Polynomial.GTECausalTree})}\label{subsec:gte-causal-tree}

The \texttt{Polynomial/GTECausalTree.lean} module (8 theorems, zero \lstinline|sorry|)
machine-certifies the perfect-binary-tree structure of the ruleGTE WolframModel causal
graph and the Horton branching ratio relevant to the botanical correspondence with
\emph{Daucus carota} (Paper~P49 \S5.3--\S5.4).

The ruleGTE WolframModel encoding matches two overlapping neighbourhood hyperedges
$\{a,b,c\},\{c,d,e\}$ and produces four new overlapping pairs, making it a 1$\to$2
binary rule.  The 1$\to$2 structure implies that $n$ generations produce exactly
$2^n - 1$ events.

\paragraph{Node count and tree structure.}
\begin{theorem}[\lstinline|perfectTree_numNodes|, \lstinline|perfectTree_height|, \lstinline|perfectTree_numLeaves|, zero \texttt{sorry}]
The perfect binary tree of depth $n$ has $2^n-1$ nodes (induction + omega),
height $n$ (induction + simp), and $2^n$ leaf nodes (induction + omega).
For depth 10: $1023$ nodes, height $10$, $1024$ leaves.
\end{theorem}

\paragraph{Event-count certification.}
\begin{theorem}[\lstinline|gte_rulegte_event_count|, \lstinline|gte_rulegte_ten_generations|, zero \texttt{sorry}]
A rewriting rule that produces exactly two successor events per event (a 1$\to$2
binary rule) accumulates $\sum_{k=0}^{n-1} 2^k = 2^n - 1$ total events after
$n$ generations (induction + omega).
At depth 10: $\sum_{k=0}^{9} 2^k = 1023$ (\lstinline|decide|).
This directly certifies the 1023-event count reported in P49 \S5.3.
\end{theorem}

\paragraph{Horton branching ratio.}
\begin{theorem}[\lstinline|perfectTree_horton_ratio|, \lstinline|gte_causal_tree_horton_ratio_eq_two|, zero \texttt{sorry}]
The node count at depth $d+1$ is twice the node count at depth $d$ (Horton ratio
$r_B = 2$), for all depths $0 \le d < n-1$ (induction).
Verified for all consecutive level pairs at depth 10 (\lstinline|decide|).
The Horton ratio $r_B = 2$ matches the Lindenmayer system $F \to F[+F][-F]$ of
\emph{Daucus carota} (Apiaceae) at depth 10 (P49 \S5.4).
\end{theorem}

\begin{theorem}[\lstinline|gte_causal_tree_summary|, zero \texttt{sorry}]
The GTE WolframModel causal graph at depth 10 has exactly 1023 nodes,
height 10, and 1024 leaves.
\end{theorem}

\subsection{MDL Three-Level Unification (\texttt{Polynomial.MDLThreeLevelUnification})}\label{subsec:mdl-three-level}

The \texttt{Polynomial/MDLThreeLevelUnification.lean} module (2 theorems, zero
\lstinline|sorry|) is a cross-module bundle certifying that the MDL principle
$P^\top(X) = \arg\min K(x \mid \mathrm{PSC\ constraints})$ operates at three
nested physical levels, with the GTE polynomial $p$ as the connecting object
(Paper~P51): theory selection (the rule $p$ from the $\mathbb{Z}_N \times
\mathbb{Z}_M$ CA rule space), field dynamics (the SRRG self-referential fixed
point), and quantum event adjudication (the PSC-projected measurement orbit).

\begin{theorem}[\lstinline|mdl_three_level_unification|, zero \texttt{sorry}]
The three-level bundle holds:
\begin{enumerate}[nosep]
\item \textbf{Theory selection.} MDL selects the
  $\mathbb{Z}_7\times\mathbb{Z}_3$ CA substrate over
  $\mathbb{Z}_5\times\mathbb{Z}_3$: there is a data-penalty assignment
  $K_{\mathrm{data}}$ with
  $\mathrm{structureSpecCost}(7,3) + K_{\mathrm{data}}(7,3) <
   \mathrm{structureSpecCost}(5,3) + K_{\mathrm{data}}(5,3)$
  (\lstinline|mdl_ca_rule_coding_closed|).
\item \textbf{Field dynamics.} The diagonal self-referential fixed point
  satisfies the master quadratic $x^2 + x = 1$ at
  $x = (\sqrt{5}-1)/2$ (\lstinline|gte_poly_srrg_bridge|).
\item \textbf{PSC cross-link.} The PSC-projection of $p$ defines
  $f_{\mathrm{MDL}}$: non-fixed neighbourhoods project to $0$, and $p$
  agrees with $f_{\mathrm{MDL}}$ on the binary sublayer
  (\lstinline|psc_projection_gives_fmdl|).
\item \textbf{Measurement orbit.} The PSC-projection orbit
  ${\rm gen}_1 \to {\rm gen}_2 \to {\rm gen}_3 \to {\rm vacuum}$ is the
  unique PSC trajectory under \lstinline|fmdl_step5|
  (\lstinline|fmdl_orbit_is_unique_psc_trajectory|).
\end{enumerate}
\end{theorem}

\begin{theorem}[\lstinline|mdl_level23_closed_choice_forces_transputation|, zero \texttt{sorry}]
Under PSC closure and record-divergent choice, an internal adjudicator
(transputation) is forced.  This is a direct wrapper of the
\texttt{transputation-lean} theorem
\lstinline|closed_choice_forces_transputation|, re-exported in the
\texttt{ugp-lean} context so that the measurement-level transition of the
three-level unification carries a machine-checked adjudication-forcing
certificate (hypothesis-bearing via the imported premise bundle).
\end{theorem}
...
Module: \texttt{Universality/EWScalePrediction.lean} (zero
...
Module: \texttt{Universality/TwoRoleTheorem.lean} (zero
\lstinline|sorry|, zero custom axioms).
...
Module: \texttt{Universality/DynamicalCouplingBridge.lean} (zero
\lstinline|sorry|, \textbf{one named axiom}
...
Module: \texttt{Universality/LorentzInvariance.lean} (zero
\lstinline|sorry|, zero custom axioms).
...
Module: \texttt{Universality/BornRuleMDL.lean} (zero
\lstinline|sorry|, zero custom axioms).
...
from second quantization rather than assuming it---remains open.
...
Module: \texttt{Universality/FockSpaceKink.lean} (zero
\lstinline|sorry|, zero custom axioms).
...
Module: \texttt{Substrate/CogwheelDynamicsG21.lean}.  Certifies the Level~1
...
Module: \texttt{Universality/ExcitationCoupling.lean} (zero
\lstinline|sorry|, zero custom axioms).
...
Module: \texttt{Universality/ThooftEffectMeasureBridge.lean} (zero
\lstinline|sorry|, zero custom axioms; \textbf{partial CatAL}).
...
\S\ref{subsec:psc-effect-measure}.
...
Module: \texttt{Universality/PSCEffectMeasure.lean} (
...
discharged via \texttt{PSCEffectMeasureGeneric.lean}).
...
\texttt{Fin n} in \texttt{PSCEffectMeasureGeneric.lean}.
...
\subsection{MDL Algebraic Derivability Criterion and CA-Level Closure
  (\texttt{Universality/MDLDerivabilityCriterion.lean})}\label{subsec:mdl-derivability}

Module: \texttt{Universality/MDLDerivabilityCriterion.lean}
(\textbf{CLOSED CatAL}, zero \lstinline|sorry| throughout).

Formalizes the K-complexity description-length penalty when a cyclic substructure
$\mathbb{Z}_M$ is \emph{not} embeddable in the multiplicative group
$(\mathbb{Z}/p\mathbb{Z})^\times$ of a prime field---the algebraic core of the
non-circular MDL derivation showing MDL alone selects $\mathbb{Z}_7\times\mathbb{Z}_3$
over all $\mathbb{Z}_N\times\mathbb{Z}_M$ alternatives.

\begin{theorem}[\lstinline|multiplicative_substructure_embeddable_iff|, CatAL, zero \texttt{sorry}]
$\mathbb{Z}_M$ embeds in $(\mathbb{Z}/p\mathbb{Z})^\times$ if and only if $M \mid p-1$
(Lagrange for the cyclic units group).
\end{theorem}

\begin{theorem}[\lstinline|z5_z3_structure_cost_exceeds_z7_z3|, CatAL, zero \texttt{sorry}]
Quantitative $\mathbb{Z}_5\times\mathbb{Z}_3$ vs.\ $\mathbb{Z}_7\times\mathbb{Z}_3$
MDL cost gap: $\mathtt{structureSpecCost}(5,3) > \mathtt{structureSpecCost}(7,3)$
because $\mathbb{Z}_3 \not\hookrightarrow \mathrm{GF}(5)^\times$ (no Sylow-3) whereas
$\mathbb{Z}_3 \hookrightarrow \mathrm{GF}(7)^\times$ (Sylow-3 subgroup exists).
\end{theorem}

\begin{theorem}[\lstinline|mdl_z7z3_beats_z7z2|, CatAL, zero \texttt{sorry}]
CA-orbit competitor elimination:
$\mathbb{Z}_7\times\mathbb{Z}_3$ beats $\mathbb{Z}_7\times\mathbb{Z}_2$ on
CA-orbit data grounds---orbit class B ($Q_\chi=2$) is misclassified as vacuum
under the $\mathbb{Z}_2$ projection, incurring $\geq 7$ bits data penalty with
zero model-cost difference.
\end{theorem}

\paragraph{MDL CA-level closure.}
The following three theorems close the final gap (CA-level K-theory bridge):

\begin{theorem}[\lstinline|z5_fmdl_no_psc_kink_orbits|, CatAL, \lstinline|native_decide|]
The MDL-minimal $\mathbb{Z}_5$ CA---the GTE polynomial $p(L,C,R)=C+R-CR-LCR$
evaluated over $\mathrm{GF}(5)$ with winding-based PSC kink predicate
$\mathtt{isPSCKinkZ5}$---has \textbf{zero PSC kink orbit classes} on the 5-cell
ring ($5^5=3125$ states enumerated by \lstinline|native_decide|).
The constant state $(2,2,2,2,2)$ is a fixed point (since $x=2$ solves $x^2+x-1\equiv0\pmod5$)
but has winding number $0$ and is correctly classified as a displaced vacuum, not a kink.
\end{theorem}

\begin{theorem}[\lstinline|mdl_total_z7z3_strictly_beats_z5z3|, CatAL, \lstinline|decide|]
Total MDL cost (theory $+$ data terms) satisfies:
$\mathtt{structureSpecCost}(7,3)+0 < \mathtt{structureSpecCost}(5,3)+6$,
i.e., $3 < 11$. The data penalty 6 bits reflects 0 intrinsic PSC orbit types
in the $\mathbb{Z}_5$ CA requiring external specification of 3 generations.
\end{theorem}

\begin{theorem}[\lstinline|mdl_ca_rule_coding_closed|, CatAL, zero \texttt{sorry}]
MDL CA-level closure: there exists an explicit $K_{\mathrm{data}}$
function such that $K_{\mathrm{data}}(7,3)=0$, $K_{\mathrm{data}}(5,3)=6$, and
$\mathtt{structureSpecCost}(7,3)+K_{\mathrm{data}}(7,3) <
\mathtt{structureSpecCost}(5,3)+K_{\mathrm{data}}(5,3)$.
Supersedes the open placeholder \lstinline|mdl_ca_rule_coding_open|.
\end{theorem}

Physical meaning: the non-circular MDL derivation chain
binary-universality $\to$ non-binary minimality $\to$ derivability criterion
$\to$ $\mathrm{GF}(7)$ $\to$ $\mathbb{Z}_7\times\mathbb{Z}_3$ $\to$ SM content
is now \textbf{fully machine-certified} at both the algebraic parameter level (existing
theorems) and the CA orbit data level (new $\S5$ theorems above), with no SM input
at any step.
...
\textbf{Module \texttt{Spectral/ThermodynamicLimit.lean}} (canonical library).
...
Module: \texttt{Spacetime/CausalInvariance.lean} (canonical library, zero sorry).
...
Module: \texttt{Spacetime/ChiralMirrorSpeedSymmetry.lean} (CatAL,
...
Module: \texttt{Spacetime/OrbitDepthEtherPeriod.lean} (CatAL,
zero \lstinline|sorry|, zero custom axioms).
...
Module: \texttt{Spacetime/LiftingTheorem.lean} (canonical library, zero sorry).
...
\subsection{\texorpdfstring{$\Phi_{\rm MDL}$}{Phi\_MDL} Fock-Space Particle Realization
  (\texttt{Universality/\allowbreak PhiMDLFockSpaceParticles.lean})}\label{subsec:phimdl-fock-space-particles}

Module: \texttt{Universality/PhiMDLFockSpaceParticles.lean} (canonical library,
zero \lstinline|sorry|, zero new axioms). Connects the algebraic Fock-space
layer (\texttt{Universality/FockSpaceKink.lean},
\texttt{Universality/BeableWindingPartitionInstance.lean},
\texttt{Substrate/CMCAHilbertFockBridge.lean}) to the Algebraic Lifting Theorem
(\S\ref{subsec:lifting-theorem}), certifying the physical realization the
Lifting Theorem promises without requiring any spatially-compact classical
field profile.

\begin{keybox}{\texorpdfstring{$\Phi_{\rm MDL}$}{Phi\_MDL} particles are Fock-space excitations}
\textbf{Theorem \lstinline|phimdl_fock_particle_master_bundle|.}
Every PSC-admissible $\mathbb{Z}_7$ winding sector in
$\{0,2,3,4,6\}$ admits a normalizable one-particle Fock-sector state (or a
unit-weight normalized sector amplitude on the certified beable partition).
Every $[\mathbf{D}]$-weighted physical beable inherits this Fock lift via the
Algebraic Lifting Theorem. The construction uses only the algebraic Fock
layer --- no spatially-compact classical $\Phi_{\rm MDL}$ soliton profile
enters any step. Mass positivity, Garden-of-Eden stability, and the
three-generation mass ordering are bundled in on the same certified
orbit Fock states.
\end{keybox}

Physical meaning: $\Phi_{\rm MDL}$ particles are second-quantized Fock
excitations within topological superselection sectors. The classical
background certifying a sector's winding, colour charge, and generation index
may be spatially extended (e.g.\ an infinite domain wall); a compact
finite-energy topological soliton is never required and is in fact
independently excluded on general grounds (any continuous field
configuration with a direction-independent vacuum limit at spatial infinity
has zero winding, and no shape retains both finite energy and topological
protection). This module closes the gap between that exclusion and the
Algebraic Lifting Theorem's promise of a physical realization: the
realization is the normalizable Fock-sector state constructed here, not a
compact classical lump.

Key theorems:
\begin{itemize}[nosep]
\item \lstinline|psc_admissible_sector_has_normalizable_fock_state| --- every
  PSC-admissible winding sector $w \in \{0,2,3,4,6\}$ admits a normalizable
  one-particle Fock-sector state or a unit-weight normalized sector amplitude.
\item \lstinline|fock_state_realizes_algebraic_lifting| --- every
  $[\mathbf{D}]$-weighted physical beable admits a normalized Fock-sector lift
  in its winding superselection sector, composing
  \lstinline|algebraic_lifting_theorem| with the sector-level existence result.
\item \lstinline|kink_mode_fock_carries_certified_quantum_numbers| --- each
  GTE kink-mode Fock one-particle state carries the certified
  $(Q_\Phi, Q_\chi)$ winding/colour pair.
\item \lstinline|psc_admissible_beable_has_normalized_fock_lift| --- the four
  canonical PSC-orbit beables (vacuum, gen$_1$, gen$_2$, gen$_3$) each have a
  normalized Fock-sector lift, exhausting \lstinline|PSCAdmissible| via
  \lstinline|psc_admissible_iff_orbit|.
\item \lstinline|gen1_fock_layer_mass_and_stability|,
  \lstinline|fock_orbit_mass_hierarchy| --- gen$_1$ positive mass and
  Garden-of-Eden stability, and the three-generation mass ordering
  $m_{\rm gen_3} > m_{\rm gen_2} > m_{\rm gen_1} > 0$, all survive on the
  Fock/orbit layer with no classical-lump dependency.
\end{itemize}
...
measure and Ollivier--Ricci computation (open; Ollivier--Ricci formalization pending).
...
\item \lstinline|d2_orbit_closed_under_step| --- PSC orbit closed under
  $f_{\mathrm{MDL}}$ step; $\mathtt{DWeight}$ preserved (CatAL).
...
\item \lstinline|dweight_centroid_follows_orbit| --- discrete Ehrenfest:
  ...
  QEC projector (\texttt{QECStabilizer}; CatAL).
...
measure remain open pending Ollivier--Ricci formalization.
...
Module: \texttt{Universality/Z3SubOrbitDisjointness.lean} (zero \lstinline|sorry|).
...
Module: \texttt{Universality/Z3InvariantEntropy.lean} (zero \lstinline|sorry|).
...
Module: \texttt{Universality/PSCOrbitWindows.lean} (zero \lstinline|sorry|).
...
\item \lstinline|two_equal_orbits_not_psc_selected| --- two-equal $F_{21}$ orbit
  representatives are disjoint from PSC generic windows (open question: LEAN formalization of generic-window separation).
\end{itemize}

\subsection{Graviton Fock Space
  (\texttt{Spacetime/\allowbreak GravitonFockSpace.lean})}\label{subsec:graviton-fock}

Module: \texttt{Spacetime/GravitonFockSpace.lean} (zero \lstinline|sorry|).
...
(canonical library; \texttt{lake build UgpLean.Universality.SylowIndexCouplingHierarchy}
...
\subsection{$F_{21}$ Physics, Hadron Spectroscopy,
  and Mass Gap — §§5i–9}\label{subsec:f21-physics}

Module: \texttt{Universality/SylowIndexCouplingHierarchy.lean}
(canonical library; all theorems zero sorry; standard axioms only:
...
\caption{New theorems in \texttt{SylowIndexCouplingHierarchy.lean} §5i–§7
  (all zero sorry, canonical library).
...
(canonical library; \texttt{lake build UgpLean.Universality.GaugeInvariance}).
Zero sorry.
...
Module: \texttt{Substrate/LExtended.lean} (canonical library, zero sorry).
...
Module: \texttt{QFT/GaugedMassGap.lean} (canonical library, zero sorry,
...
\subsection{Unconditional theorems}\label{subsec:qft-mass-gap-unconditional}
...
A full audit of the library discharged previously declared
axioms in \texttt{VEVProof/GoldstoneEntropyCorrection.lean} as proved theorems.
Additional named axioms proved as theorems include five \lstinline|: True|-type
and existential axioms across \texttt{GaugeMDL}, \texttt{CCImpossibilityBundle},
\texttt{GUTStructure}, and \texttt{PMDLGravityTheorems}, and
\lstinline|adj_mem_periodicCausalNeighbors| in \texttt{SpectralDimensionDegree}
(proved via complete case analysis).
The verified global named-axiom total is \textbf{98}.

The following named axioms are proved theorems (zero sorry):
\begin{itemize}[nosep]
...
Additionally proved as theorems (zero sorry):
\begin{itemize}[nosep]
\item \lstinline|g_s_from_sigma_gte_2loop_rge| (\texttt{GaugeMDL}) — type \lstinline|: True|; trivially proved.
\item \lstinline|phimdl_no_susy_degenerate_spectrum_placeholder| (\texttt{CCImpossibilityBundle}) — type \lstinline|: True|; trivially proved; placeholder pending a real SUSY/fermion-spectrum formalization (none exists in this library).
\item \lstinline|phimdl_no_antigravitating_sector_placeholder| (\texttt{CCImpossibilityBundle}) — type \lstinline|: True|; trivially proved; placeholder — the unconstrained statement \lstinline|∀ cfg, T00 ≥ 0| is actually false under this library's own \texttt{StressEnergyTensor.lean} definitions (see module docstring for the explicit counterexample), so a correct restricted version is not yet formalized.
\item \lstinline|g_inter_structural_catA| (\texttt{GaugeMDL}) — type \lstinline|: True|; trivially proved.
\item \lstinline|eta_B_normalization_axiom| (\texttt{GUTStructure}) — type \lstinline|: ∃ κ : ℝ, 0 < κ ∧ κ = 1|; proved by \lstinline|⟨1, one_pos, rfl⟩|.
\item \lstinline|adj_mem_periodicCausalNeighbors| (\texttt{SpectralDimensionDegree}) — proved via
  \lstinline|finPeriodicPred_succ| helper theorem and exhaustive case analysis
  over all 20 adjacency branches of \lstinline|CausalGraphPeriodic|.
\end{itemize}

\begin{table}[h]
\centering\small
\begin{tabular}{@{}p{6.2cm}p{4.5cm}r@{}}
\toprule
Module & Scope & Custom axioms \\
\midrule
\texttt{MassRelations.TranscendentalMassBounds} & Transcendental exponential mass-bound numerics (cascade anchors) & 19 \\
\texttt{Universality.CUP3DPhysicalIncompleteness} & CUP3D physical-incompleteness bridge & 6 \\
\texttt{Substrate.WightmanAxioms}     & Wightman QFT axioms & 6 \\
\texttt{Gravity.PageWoottersZ7}       & Page--Wootters timeless-formalism bridge & 6 \\
\texttt{Gravity.PMDLGravityTheorems}  & PMDL gravity theorems & 5 \\
\texttt{Algebra.F21SU3Embedding}      & $F_{21}\hookrightarrow SU(3)$ embedding & 4 \\
\texttt{Universality.EWChiralBridge}  & Electroweak chirality bridge & 3 \\
\texttt{Universality.BellViolationGTE} & Bell-violation GTE bridge & 3 \\
Rule110-lean (cross-repo)             & ChiralGliderDynamics & (cross-repo) \\
\hline
Other modules (37 files, $\le 2$ axioms each) & Bridge, structural, and auxiliary axioms, including \texttt{Universality.RegisterMachine} (A5) and \texttt{Universality.CookComputableBridge} (A6) & 46 \\
\hline
\textbf{Global named custom axioms}   & \textbf{total} & \textbf{98} \\
\bottomrule
\end{tabular}
\caption{Custom named-axiom inventory. Each row lists the module, physical scope,
  and axiom count. Counted via
  \lstinline!grep -rhoE "^axiom [a-zA-Z0-9_']+" UgpLean --include="*.lean" | sort -u | wc -l!
  (top-level \texttt{axiom} declarations, deduplicated by name; excludes the standard
  Lean/Mathlib logical axioms \texttt{propext}, \texttt{Classical.choice},
  \texttt{Quot.sound}). The global verified total is \textbf{98}; the core physics
  proof path (charged leptons, couplings, generation count) uses only standard
  Lean/Mathlib axioms. \lstinline|causal\_path\_exists| is proved as a theorem
  (forward-causal pairs only) and is no longer a module-local axiom.
  ``Other modules'' includes the two Turing-universality bridge axioms discussed below.}
\label{tab:axiom-inventory}
\end{table}

\paragraph{Universality-layer axiom update.}
The Turing-universality closure work (\S\ref{subsec:phimdl-universality},
\S\ref{sec:universality}) replaced one unsound axiom with two justified classical
axioms, a net change of zero to the global total:
\begin{itemize}[nosep]
\item \textbf{Removed (unsound):} \lstinline|z7_boolean_completeness_implies_turing_universal|
  --- a purported Shannon-style bridge from finite Boolean completeness to Turing
  universality; the finite-completeness result it was attached to does not entail
  Turing universality, so the bridge was retired rather than repaired.
\item \textbf{Added, justified:} \lstinline|cook_rule110_simulates_computable|
  (\texttt{Universality.CookComputableBridge}) --- packages Cook's (2004) proved
  operational universality theorem (\lstinline|cook_operational_stage3_tm_microstep_readback|
  in \texttt{rule110-lean}, formerly \lstinline|rule110_turing_universal_from_cook|)
  composed with universal Turing-machine/partial-recursive
  compilation; replaces the former directly-declared axiom
  \lstinline|GTEComputability.rule110_simulates_computable| (now a zero-sorry theorem).
\item \textbf{Added, justified:} \lstinline|minsky_counter_machine_turing_complete_1967|
  (\texttt{Universality.RegisterMachine}) --- Minsky's (1967) classical theorem that
  two-counter machines are equivalent to Turing machines / partial-recursive functions;
  Mathlib has no counter-machine formalization, so this textbook-standard result is
  taken as a named axiom rather than re-derived.
\end{itemize}
Four new modules support this closure:
\texttt{Universality.RegisterMachine} (Minsky counter-machine semantics, zero sorry
apart from the axiom above), \texttt{Universality.UWCARegisterUniversality} (UWCA
register-file simulates counter machines, zero sorry),
\texttt{Universality.UWCARegisterSweep} ($k$ UWCA sweeps $=$ $k$ Rule-110 steps, zero
sorry, zero axioms), and \texttt{Universality.CookComputableBridge} (Cook composition
axiom and re-export of \texttt{rule110-lean}'s operational universality theorem).
...
Module: \texttt{Spacetime/AnomalyRenormalizability.lean} (canonical library, zero sorry).
...
Module: \texttt{Universality/AlgebraicDescentTheorem.lean} (canonical library, zero sorry).
...
Module: \texttt{Universality/AlgebraicNecessityTheorem.lean} (zero
\lstinline|sorry|, zero custom axioms).
...
Module: \texttt{Spacetime/StressEnergyTensor.lean} (zero \lstinline|sorry|).
...
Module: \texttt{Spacetime/AsyncLiftingTheorem.lean} (zero \lstinline|sorry|).
...
Module: \texttt{Spacetime/Spectral/HeatKernelFourier.lean}.
...
Module: \texttt{Substrate/WindingCoinDecoupling.lean} (canonical library, zero \lstinline|sorry|).
...
Module: \texttt{Substrate/CoherenceMeasureUniqueness.lean} (zero \lstinline|sorry|
...
\item \lstinline|c2_thermal_closure_bundle| --- Gibbs free-energy gap uniquely
  selects the canonical sector distribution (CatAL).
...
\subsection{Gromov--Hausdorff convergence of CMCA spatial graphs
  (\texttt{Spacetime/VacuumGHConvergence.lean},
   \texttt{Spacetime/MatterGHPrecompactness.lean})}\label{subsec:gh-convergence}

Two zero-\lstinline|sorry| modules establish that the vacuum CMCA spatial causal
graph converges to flat Euclidean space in the Gromov--Hausdorff topology,
and that finite-kink matter families are pre-compact in the same topology.

\begin{keybox}{\lstinline|vacuum_cmca_gh_converges_to_flat_space| + \lstinline|fin_grid_gh_dist_bound| (zero \texttt{sorry})}
The vacuum \CMCA{} spatial causal graph $\mathrm{FinGrid}_L$ equipped with the
scaled $1/L$ metric converges in Gromov--Hausdorff distance to the unit cube
$[0,1]^3$ (flat three-dimensional Euclidean space) as $L\to\infty$.
Quantitative bound: $d_{\mathrm{GH}}(\mathrm{FinGrid}_L,\,[0,1]^3)\leq 1/L$.
Module: \texttt{Spacetime/VacuumGHConvergence.lean} (zero \lstinline|sorry|).
\end{keybox}

\begin{keybox}{\lstinline|finGrid_family_totally_bounded| + \lstinline|single_kink_gh_converges_to_flat| (zero \texttt{sorry})}
The family of vacuum \CMCA{} spatial graphs is \emph{totally bounded} in the
Gromov--Hausdorff topology (every infinite subsequence has a convergent
subsequence).  A single isolated kink of width~$W$ satisfies
$d_{\mathrm{GH}}(\mathrm{KinkGrid}_L,\,[0,1]^3)\leq (W/2{+}1)/L\to 0$:
an isolated kink is measure-zero at large~$L$ and does not curve the
Gromov--Hausdorff limit.  Finite kink density is required for a curved GH limit
(OQ-QG-1b full; Cheeger--Colding regularity, open).
Module: \texttt{Spacetime/MatterGHPrecompactness.lean} (zero \lstinline|sorry|).
\end{keybox}

\subsection{Theorem inventory: quantum mechanics, Born rule, and gravity sector}\label{subsec:new-theorem-inventory}

The following table lists zero-sorry theorems spanning quantum mechanics,
Born-rule derivation, 3D substrate structure, and the GR\,/\,gravity sector,
...
\texttt{two\_\allowbreak equal\_\allowbreak orbits\_\allowbreak not\_\allowbreak psc\_\allowbreak selected}
  ...
  & Two-equal $F_{21}$ orbits not PSC-selected \\[1pt]
...
\bottomrule
\end{tabular}
\caption{Zero-sorry theorems in the quantum mechanics, Born-rule, and gravity sector,
  grouped by module. ``CatAL'' denotes physically calibrated category-A level;
  all others are unconditional unless noted.}
\label{tab:new-theorem-inventory}
\end{table}

\subsection{Theorem inventory: quantum-gravity completion}\label{subsec:new-theorem-inventory-078}

The following table records zero-sorry theorems from the quantum-gravity completion,
covering the minimal-coupling / Wald chain
...
\texttt{incompleteness\_\allowbreak implies\_\allowbreak nonzero\_\allowbreak omega\_\allowbreak lambda}
  & Gravity.PSCEpochSelection
  & PSC $\to$ $D_{\rm res}>0$ $\to$ $\Omega_\Lambda>0$; full incompleteness-cosmology chain \\[1pt]
...
\texttt{rs\_\allowbreak area\_\allowbreak unit\_\allowbreak log7}
  & Substrate.RSCodeOrbit
  & $\log(7)/\log(7) = 1$; trivial normalization, one ingredient in the
    paper's conditional area-unit construction \\[1pt]
...
\bottomrule
\end{tabular}
\caption{Zero-sorry CatAL theorems: quantum-gravity completion (LC1--LC11, PSP, fermionic statistics, Galois structure, RS codes, GF(7) vacuum fixed point).}
...
The following table records zero-sorry CatAL theorems from the three-tape CMCA
formalization, covering holographic state counting, the single-polynomial
...
space and spinor exchange now lives in \texttt{ugp-lean} as
\lstinline|UgpLean.Gravity| (\lstinline|MinkowskiSpace|, \lstinline|SpinorRep|,
\lstinline|WaldEntropy|); these modules were moved from \texttt{ugp-physics-lean}
to eliminate the cyclic Lake dependency between the two repositories.
...
\texttt{su2l\_\allowbreak weak\_\allowbreak force\_\allowbreak derivation}
  & Algebra.GaugeMDL
  & SU(2)$_L$ from PMDL gauging; \texttt{phimdl\_\allowbreak potential\_su2l\_\allowbreak invariant}, \texttt{su2l\_\allowbreak wpm\_\allowbreak generator\_\allowbreak algebra} (CatAL) \\[1pt]
...
\subsection{Theorem inventory: gravity force law and entanglement}\label{subsec:new-theorem-inventory-oq1-oq3}

The following modules and theorems establish the
Newtonian gravity force law (CatAD) and the partial formalization of
the Page-Wootters Born rule and Bell inequality (CatAD/CatA).
...
\midrule
\multicolumn{3}{@{}l}{\textit{L1$\to$L2 Bridge Theorems (Gap closure G1–G4)}} \\[2pt]
...
\texttt{l1\_l2\_born\_rule\_bridge\_placeholder} (axiom)
  & Gravity.PageWoottersZ7
  & Placeholder for: PW Born $P(k|\tau_c)$ converges to field Born $|\partial_x\Phi|^2/Z$ at rate $O(1/M^2)$; not yet formalized beyond \texttt{True} \\[1pt]
\texttt{born\_bridge\_informal\_argument\_placeholder}
  & Gravity.PageWoottersZ7
  & Documented informal argument that the Born bridge requires no axiom beyond \texttt{cmca\_continuum\_limit\_is\_phimdl}; not yet formalized beyond \texttt{True} \\[1pt]
\texttt{l1\_chsh\_and\_\allowbreak l2\_epr\_are\_\allowbreak distinct\_layers}
  & Universality.BellViolationGTE
  & Level-1 CHSH ($S=2.4459$) state-space index type and Level-2 P43 EPR Fock-space type have different cardinality (9 vs.\ 16); machine-checked \\[1pt]
\texttt{alt\_does\_not\_lift\_chsh\_value\_placeholder}
  & Universality.BellViolationGTE
  & Documented informal argument that ALT lifts algebraic structure, not dynamical Hamiltonians or CHSH values; not yet formalized beyond \texttt{True} \\[1pt]
\texttt{gap\_g1\_\allowbreak polynomial\_\allowbreak continuum\_\allowbreak taxonomy}
  & Algebra.\allowbreak PolynomialContinuumBridge
...
\bottomrule
\end{tabular}
\caption{Theorems and named axioms covering the gravity force law, the
Page-Wootters Born rule and Bell inequality (CatAD/CatA), and the L1$\to$L2 bridge.
...
\begin{remark}[\lstinline|born_bridge_informal_argument_placeholder|]
The Born bridge $\psi=\partial_x\Phi/\sqrt{Z}$ is intended to require no axiom
beyond \lstinline|cmca_continuum_limit_is_phimdl|: the identification is
informally motivated by the BPS saturation condition
$\partial_x\Phi=\sqrt{2V_{\ZS}(\Phi)}$ together with the $M\to\infty$
continuum limit theorem.  This connection is currently a documented informal
argument (Lean statement \lstinline|True|), not a machine-checked theorem;
the identifier is named \lstinline|_placeholder| to prevent it from being
cited as a zero-\texttt{sorry} formal result.
\end{remark}

\begin{theorem}[\lstinline|l1_chsh_and_l2_epr_are_distinct_layers|, zero \texttt{sorry}]
The finite index type underlying the Level-1 CHSH state space
($\mathrm{Fin}\,3\times\mathrm{Fin}\,3$, cardinality~9; the qutrit-pair setup
of \lstinline|gte_bell_violation_at_half_coupling|, $S=2.4459$ on
$\mathbb{C}^7\otimes\mathbb{C}^7$) has different cardinality from the Level-2
$\Phi_{\mathrm{MDL}}$ kink Fock space
\lstinline|FockSpaceKink.KinkFockState| (cardinality~16).  This machine-checked
type-level separation is the formal core of the broader claim that the Level-1
CHSH violation and the Level-2 $[\mathbf{D}]$-record EPR correlations of P43
(transputation~D3, non-computable) are structurally distinct phenomena at
different levels of the theory; the full dynamical argument for that broader
claim remains at $\CatAD$.
\end{theorem}

\begin{remark}[\lstinline|alt_does_not_lift_chsh_value_placeholder|]
The intended claim is that the Algebraic Lifting Theorem carries algebraic
and structural content ($\mathbb{Z}_7$ winding, $N_{\mathrm{gen}}$, GoE,
confinement, gauge vertices, V--A fraction) from Level~1 to Level~2, but does
\emph{not} lift dynamical Hamiltonians or specific CHSH values.  The Bell
parameter $S=2.4459$ is a Level-1 dynamical certificate; what lifts is the
structural statement that $\Phi_{\mathrm{MDL}}$ supports genuine quantum
entanglement.  This is a documented physical argument, not yet a
machine-checked theorem (Lean statement \lstinline|True|); the identifier is
named \lstinline|_placeholder| accordingly.
\end{remark}

\subsection{Theorem inventory: phantom-reference resolution}\label{subsec:new-theorem-inventory-audit}

The following theorems resolve phantom theorem name references in physics papers. All are zero sorry.
...
\texttt{frobenius\_\allowbreak f21\_\allowbreak is\_\allowbreak z7\_\allowbreak sdp\_\allowbreak z3}
  & Universality.\allowbreak SylowIndex\allowbreak CouplingHierarchy
...
\texttt{srrg\_\allowbreak higgs\_\allowbreak vev\_\allowbreak from\_\allowbreak fixed\_\allowbreak point}
  & srrg-lean: VEVProof.\allowbreak EWVacuumBridge
  & \texttt{ew\_\allowbreak vacuum\_\allowbreak bridge\_\allowbreak grade\_\allowbreak certificate}; grade~[A$^-$]; $v_{\rm PSC}=246.16$\,GeV \\[1pt]
...
\bottomrule
\end{tabular}
\caption{Zero-sorry theorems resolving phantom theorem name references in physics papers.}
\label{tab:new-theorem-inventory-audit}
\end{table}

\subsection{Theorem inventory: GTP-3 uniqueness, winding superselection, and
  cyclotomic Galois certificates}\label{subsec:new-theorem-inventory-gtp3-galois}

The following theorems span four modules, all zero \lstinline|sorry| and zero
new axioms.
\texttt{Universality/GTP3Uniqueness.lean} completes the GTP-3 classification
begun in \texttt{GoEStabilityHierarchy.lean}: where the existing
\lstinline|sm_orbit_is_gtp3| certifies the \emph{existence} direction (the SM
orbit gen$_1{\to}$gen$_2{\to}$gen$_3{\to}$vacuum is a Garden-of-Eden-rooted
terminating path of length~3) and \lstinline|fmdl_no_gtp4| the no-GTP-4 bound,
the new module certifies \emph{uniqueness} and the exact count by exhaustive
\lstinline|native_decide| over all $7^5 = 16{,}807$ ring states (the decision
procedure evaluates a short-circuiting boolean form, ordered so the $O(1)$
depth-3 conditions filter out almost every state before any $O(7^5)$
predecessor scan runs).
\texttt{Universality/WindingSectorSuperselection.lean} certifies the
winding-sector superselection statement underlying the black-hole information
argument, grounded in the certified $\mathbb{Z}_7$ winding conservation law
(\lstinline|gte_winding_sm_vertex_conserved|,
\lstinline|gte_seven_vertices_z7_winding|).
\texttt{Algebra/CyclotomicZ7GaloisGroup.lean} upgrades the arithmetic
$(\mathbb{Z}/7)^\times$ certificates of \texttt{CyclotomicZ7Galois.lean} to
statements about the \emph{actual} Galois group
$\mathrm{Gal}(\mathbb{Q}(\zeta_7)/\mathbb{Q})$ via Mathlib's
\lstinline|IsCyclotomicExtension.autEquivPow| (using irreducibility of
$\Phi_7$ over $\mathbb{Q}$).
\texttt{Algebra/CyclotomicFieldDisjointness.lean} proves the field-theoretic
content of the $\mathbb{Z}_7$/$\mathbb{Z}_{120}$ sector-disjointness claim by
a degree obstruction.
...
Theorem & Module & Physical content \\
\midrule
\multicolumn{3}{@{}l}{\textit{GTP-3 uniqueness and count (\texttt{Universality/GTP3Uniqueness.lean})}} \\[2pt]
\texttt{sm\_\allowbreak orbit\_\allowbreak unique\_\allowbreak gtp3}
  & Universality.GTP3Uniqueness
  & A state starts a GTP-3 chain \textbf{iff} it is a cyclic rotation of
    gen$_1$; exhaustive over $16{,}807$ states (\CatAL) \\[1pt]
\texttt{sm\_\allowbreak orbit\_\allowbreak gtp3\_\allowbreak count}
  & Universality.GTP3Uniqueness
  & Exactly 5 GTP-3 start states exist in $\mathbb{Z}_7^5$ — the five
    rotations of gen$_1$ (\CatAL) \\[1pt]
\texttt{gen1\_\allowbreak is\_\allowbreak gtp3\_\allowbreak start}
  & Universality.GTP3Uniqueness
  & gen$_1$ itself is a GTP-3 start (connects to \lstinline|sm_orbit_is_gtp3|) \\[1pt]
\midrule
\multicolumn{3}{@{}l}{\textit{Winding-sector superselection (\texttt{Universality/WindingSectorSuperselection.lean})}} \\[2pt]
\texttt{distinct\_\allowbreak sector\_\allowbreak support\_\allowbreak forces\_\allowbreak zero}
  & Universality.\allowbreak WindingSector\allowbreak Superselection
  & A wavefunction supported in two distinct winding sectors is identically
    zero \\[1pt]
\texttt{winding\_\allowbreak sector\_\allowbreak superselection}
  & Universality.\allowbreak WindingSector\allowbreak Superselection
  & A winding-preserving injective evolution never maps a nonzero sector-$w$
    state into sector $w' \neq w$ (\CatAL) \\[1pt]
\texttt{topological\_\allowbreak kink\_\allowbreak stability}
  & Universality.\allowbreak WindingSector\allowbreak Superselection
  & Specialization to $w'{=}0$: nonzero states of non-trivial winding cannot
    evolve into the vacuum sector (\CatAL) \\[1pt]
\midrule
\multicolumn{3}{@{}l}{\textit{Galois group of $\mathbb{Q}(\zeta_7)/\mathbb{Q}$ (\texttt{Algebra/CyclotomicZ7GaloisGroup.lean})}} \\[2pt]
\texttt{galois\_\allowbreak z7\_\allowbreak cyclic\_\allowbreak order\_\allowbreak 6}
  & Algebra.\allowbreak CyclotomicZ7GaloisGroup
  & $\mathrm{Gal}(\mathbb{Q}(\zeta_7)/\mathbb{Q})$ has order 6 and is cyclic
    — the full $\cong \mathbb{Z}_6$ statement (\CatAL) \\[1pt]
\texttt{galois\_\allowbreak z7\_\allowbreak cpt\_\allowbreak generator}
  & Algebra.\allowbreak CyclotomicZ7GaloisGroup
  & The automorphism $\sigma_6 : \zeta_7 \mapsto \zeta_7^{-1}$ (complex
    conjugation; CPT under the $\Phi_{\rm MDL}$ kink identification) has
    order exactly 2 (\CatAL) \\[1pt]
\texttt{galois\_\allowbreak z7\_\allowbreak generation\_\allowbreak subgroup\_\allowbreak order\_\allowbreak 3}
  & Algebra.\allowbreak CyclotomicZ7GaloisGroup
  & The Frobenius $\sigma_2$ (generation symmetry) has order exactly 3, so
    $\mathbb{Z}_6 = \mathbb{Z}_2 \times \mathbb{Z}_3 = {\rm CPT} \times
    {\rm generation}$ (\CatAL) \\[1pt]
\midrule
\multicolumn{3}{@{}l}{\textit{Cyclotomic field disjointness (\texttt{Algebra/CyclotomicFieldDisjointness.lean})}} \\[2pt]
\texttt{finrank\_\allowbreak cyclotomic\_\allowbreak seven\_\allowbreak eq\_\allowbreak six}
  & Algebra.\allowbreak CyclotomicField\allowbreak Disjointness
  & $[\mathbb{Q}(\zeta_7):\mathbb{Q}] = \varphi(7) = 6$ \\[1pt]
\texttt{finrank\_\allowbreak cyclotomic\_\allowbreak onetwenty\_\allowbreak eq\_\allowbreak thirtytwo}
  & Algebra.\allowbreak CyclotomicField\allowbreak Disjointness
  & $[\mathbb{Q}(\zeta_{120}):\mathbb{Q}] = \varphi(120) = 32$ \\[1pt]
\texttt{cyclotomic\_\allowbreak z7\_\allowbreak not\_\allowbreak embeddable\_\allowbreak in\_\allowbreak z120}
  & Algebra.\allowbreak CyclotomicField\allowbreak Disjointness
  & No $\mathbb{Q}$-algebra embedding
    $\mathbb{Q}(\zeta_7) \hookrightarrow \mathbb{Q}(\zeta_{120})$ exists
    (degree obstruction $6 \nmid 32$): the $\mathbb{Z}_7$ vacuum sector's
    cyclotomic field is arithmetically independent of the
    $\mathbb{Z}_{120}$ mass/Yukawa sector's (\CatAL) \\
\bottomrule
\end{tabular}
\caption{GTP-3 uniqueness, winding-sector superselection, and cyclotomic
Galois certificates. All zero \lstinline|sorry|, zero new axioms.
\texttt{GTP3Uniqueness} decides by exhaustive \lstinline|native_decide|;
\texttt{WindingSectorSuperselection} states its unitarity residue
(injectivity, $U0=0$) as explicit hypotheses;
the two \texttt{Algebra} modules work through Mathlib's cyclotomic field
theory rather than arithmetic proxies.}
\label{tab:new-theorem-inventory-gtp3-galois}
\end{table}

\subsection{Theorem inventory: L1--L2 bridge and BPS kink dynamics}\label{subsec:new-theorem-inventory-080}

The following modules and theorems cover the L1$\to$L2 bridge and span ten modules:
...
\bottomrule
\end{tabular}
\caption{Theorems covering BPS kink mass and
...
\subsection{Theorem inventory: neutrino vacuum sector and EW precision}\label{subsec:new-theorem-inventory-g28-g29}

The following theorems were added in two modules: \texttt{MassRelations/\allowbreak NeutrinoVacuumSectorL2.lean} (G28) formalises the neutrino Level-2 vacuum-sector constraints and dark-ring counting; \texttt{Algebra/GaugeMDL.lean} (G10/G29) certifies EW coupling constants and the one-loop oblique EW precision gap closure.
...
\texttt{neutrino\_\allowbreak winding\_\allowbreak is\_\allowbreak vacuum}
  & MassRelations.\allowbreak NeutrinoVacuumSectorL2
...
\midrule
\multicolumn{3}{@{}l}{\textit{G30 --- CC cancellation impossibility, placeholder (\texttt{Gravity/CCImpossibilityBundle.lean})}} \\[2pt]
\texttt{cc\_\allowbreak cancellation\_\allowbreak impossible\_\allowbreak in\_\allowbreak gte\_\allowbreak placeholder}
  & Gravity.CCImpossibilityBundle
  & Type \lstinline|True ∧ True ∧ True|; intended target is no SUSY-degenerate fermion sector and no antigravitating $T_{00}<0$ sector, but neither component is Lean-certified (zero \texttt{axiom} declarations in this module; see module docstring) \\[1pt]
\texttt{voxel\_\allowbreak cc\_\allowbreak coefficient}
  & Gravity.TemporalVoxelCC
  & GTE voxel CC coefficient $9/112 = 9/(7{\times}16)$ (zero \texttt{sorry}) \\[1pt]
\texttt{omega\_\allowbreak lambda\_\allowbreak from\_\allowbreak temporal\_\allowbreak voxel}
  & Gravity.TemporalVoxelCC
  & $\Omega_\Lambda = 3\pi/14$ from three-tape temporal voxel (structural, zero \texttt{sorry}) \\[1pt]
\midrule
\multicolumn{3}{@{}l}{\textit{G15 --- Proton mass structural (\texttt{Gravity/PMDLGravityTheorems.lean})}} \\[2pt]
\texttt{proton\_\allowbreak winding\_\allowbreak norm}
  & Gravity.PMDLGravityTheorems
...
\bottomrule
\end{tabular}
\caption{EW coupling constants, neutrino vacuum sector winding, dark-ring state counting, and one-loop EW oblique precision. All zero \texttt{sorry}.}
\label{tab:new-theorem-inventory-g28-g29}
\end{table}

\subsection{Temporal voxel cosmological constant (G30,
  \texttt{Gravity/TemporalVoxelCC.lean})}\label{subsec:temporal-voxel-cc}

Module: \texttt{Gravity/TemporalVoxelCC.lean}.  Certifies the three-tape CMCA
temporal-voxel derivation of the dark-energy fraction from GTE inputs
$N_{\rm spatial}=3$, $\tau_{\rm proper}/\tau_{\rm lab}=3/7$, and $D=4$:
\[
\rho_{\rm CC} = \frac{9\,M_{\rm Pl}^2 H_0^2}{112},
\qquad
\Omega_\Lambda = \frac{3\pi}{14} \approx 0.6732
\]
(within $2.4\%$ of $D_{\rm res}$ and $2.3\%$ of Planck 2018).

\begin{theorem}[\lstinline|voxel_cc_coefficient|, zero \texttt{sorry} ($\CatB$ structural)]
The GTE voxel CC coefficient is $9/112 = 9/(7{\times}16)$.
\end{theorem}

\begin{theorem}[\lstinline|omega_lambda_from_temporal_voxel|, zero \texttt{sorry} ($\CatB$ structural)]
The implied dark-energy fraction is $\Omega_\Lambda = 3\pi/14$.
\end{theorem}

The proper time rate $\tau = N_{\rm spatial}/|\mathbb{Z}_7| = 3/7$ is certified as a
GTE structural identity via \lstinline|tau_structural_identity| (zero \texttt{sorry}).
The complete counting formula \lstinline|gte_cc_counting_formula| expresses
$\rho_{\rm CC} = N_{\rm spatial}^2/(D^2 \cdot |\mathbb{Z}_7|) \times M_{\rm Pl}^2 H_0^2$
(zero \texttt{sorry}).

\begin{theorem}[\lstinline|su2l_within_tape_l2_from_phimdl|, zero \texttt{sorry} ($\CatAL$)]
The within-tape doublet $(\Phi_+,\Phi_-)$ gains SU(2)$_L$ covariant kinetic term
$|D_\mu\Psi|^2$ via the MDL principle (zero \texttt{sorry}; LC5 orbit geometry
pending for full continuum $\CatAL$).
\end{theorem}

\begin{theorem}[\lstinline|phimdl_potential_su2l_invariant|, zero \texttt{sorry} ($\CatAL$)]
The within-tape doublet L2 norm is preserved under orthogonal $2\times2$ rotations:
$\|U\Psi\|_2=\|\Psi\|_2$ when $U^\top U=1$, so the $\Phi_{\mathrm{MDL}}$ potential
$V(|\Psi|)$ is SU(2)$_L$ invariant.
\end{theorem}

\begin{theorem}[\lstinline|su2l_covariant_derivative_minimal|, zero \texttt{sorry} ($\CatAL$)]
MDL minimality on the weak-isospin doublet orbit forces the covariant kinetic term:
$K_{\mathrm{gauged}}<K_{\mathrm{global}}$ for the finite doublet proxy.
\end{theorem}

\begin{theorem}[\lstinline|tPlus_tMinus_commutator|, zero \texttt{sorry} ($\CatAL$)]
SU(2)$_L$ raising/lowering commutator on the rational $2\times2$ proxy:
$[T_+,T_-]=2T_0$.
\end{theorem}

\begin{theorem}[\lstinline|tZero_tPlus_commutator|, zero \texttt{sorry} ($\CatAL$)]
$[T_0,T_+]=T_+$ on the rational $2\times2$ proxy ($2T_0$ convention).
\end{theorem}

\begin{theorem}[\lstinline|tZero_tMinus_commutator|, zero \texttt{sorry} ($\CatAL$)]
$[T_0,T_-]=-T_-$ on the rational $2\times2$ proxy ($2T_0$ convention).
\end{theorem}

\begin{theorem}[\lstinline|su2l_wpm_generator_algebra|, zero \texttt{sorry} ($\CatAL$)]
SU(2)$_L$ W-boson generator algebra bundled: all three Lie algebra relations
(\lstinline|tPlus_tMinus_commutator|, \lstinline|tZero_tPlus_commutator|,
\lstinline|tZero_tMinus_commutator|).
\end{theorem}

\begin{theorem}[\lstinline|su2l_full_gauging_catad|, zero \texttt{sorry} ($\CatAL$)]
...
(\lstinline|PhimdlWeakChargedCurrentCert|), SU(2)$_L$ potential invariance
(\lstinline|PhimdlPotentialSu2lInvariant|), and the W$^\pm$ generator algebra
(\lstinline|su2l_wpm_generator_algebra|). All components zero \texttt{sorry}.
\end{theorem}

\subsection{SU(2)$_L$ doublet Hilbert space
  (\texttt{Substrate/SU2LDoubletHilbert.lean})}\label{subsec:su2l-doublet-hilbert}

Module: \texttt{Substrate/SU2LDoubletHilbert.lean} (zero \lstinline|sorry|).

\begin{keybox}{\lstinline|su2l_doublet_hilbert_certified| (zero \texttt{sorry})}
The $\mathrm{SU}(2)_L$ doublet Hilbert space $(\nu_L,\,e_L)$ is explicitly
constructed from the \GTE{} discrete data: $T_3$ eigenvalues $+\tfrac{1}{2}$
(neutrino component) and $-\tfrac{1}{2}$ (charged-lepton component), $W^\pm$
raising/lowering operators, and the $\mathfrak{su}(2)$ algebra relations
$[T_+,T_-]=2T_3$, $[T_3,T_\pm]=\pm T_\pm$ are all machine-certified with
zero \lstinline|sorry| from the certified $W_B$ arithmetic.
This is the master bundle for the electroweak doublet Hilbert structure.\\
Individual components: \lstinline|t3_eigenvalue| ($T_3 = \pm\tfrac{1}{2}$ on the
standard basis) and \lstinline|su2l_algebra_closes| (all three Lie algebra
commutation relations).
\end{keybox}

\subsection{Holographic mode-count identification (\texttt{Gravity/TemporalVoxelCC.lean}, G30 closure)}\label{subsec:holographic-mode-count}

The following theorems, added in the G30 closure, certify the holographic
identification that converts the Planck-scale vacuum-energy catastrophe into the
observed $H_0^2 M_{\rm Pl}^2$ scale without any metric input.

\begin{theorem}[\lstinline|holographic_mode_count_ratio|, zero \texttt{sorry} ($\CatAD$)]
The three-tape mode count $3L$ divided by the naive 3D count $L^3$ is exactly $3/L^2$:
\[ (3L)/L^3 = 3/L^2. \]
\end{theorem}

\begin{theorem}[\lstinline|holographic_voxel_scaling|, zero \texttt{sorry} ($\CatAD$)]
With the Hubble radius in Planck units $L = M_{\rm Pl}/H_0$, the naive
vacuum-energy density $M_{\rm Pl}^4$ reduced by the $3/L^2$ holographic factor
gives the temporal voxel scale:
\[ (3/L^2) \cdot M_{\rm Pl}^4 = 3\,H_0^2 M_{\rm Pl}^2. \]
\end{theorem}

\begin{theorem}[\lstinline|holographic_identification_voxel_cc|, zero \texttt{sorry} ($\CatAD$)]
Full holographic identification assembled from certified components: the
$3/L^2$ mode-count suppression (\lstinline|holographic_mode_count_ratio|) gives the
$H_0^2 M_{\rm Pl}^2$ scale (\lstinline|holographic_voxel_scaling|); multiplied by the
structural coefficient $9/112$ (\lstinline|gte_cc_counting_formula|) this yields the
temporal voxel CC $\rho_{\rm CC} = (9/112)\,M_{\rm Pl}^2 H_0^2$,
$\Omega_\Lambda = 3\pi/14$.  Zero axioms; the $3L$ mode count is $\CatAD$ via
\lstinline|cmca_continuum_limit_is_phimdl| $\circ$ \lstinline|three_tape_state_card| $\circ$
\lstinline|algebraic_lifting_theorem|.
\end{theorem}

\begin{theorem}[\lstinline|holographic_nrt_loop_tree_ratio_h0_independent|, zero \texttt{sorry} ($\CatAD$, metric-free)]
The holographic one-loop CC correction as a fraction of the tree-level CC is
independent of the Hubble scale $H_0$:
\[ \frac{\delta\rho}{\rho_{\rm tree}}
   = \frac{g^2}{16\pi^2}\cdot\frac{112}{3}\cdot\left(\frac{m_{\rm kink}}{M_{\rm Pl}}\right)^2. \]
The $H_0^2$ factors cancel exactly.  This is the metric-free core of the
holographic non-renormalization theorem (G22+G42); the absolute-magnitude
identification $L = M_{\rm Pl}/H_0$ remains an open question pending quantum gravity input.
\end{theorem}

\subsection{Proper-time rate from Rule~110 ether (\texttt{Gravity/EtherProperTimeRate.lean})}\label{subsec:ether-proper-time-rate}

Module: \texttt{Gravity/EtherProperTimeRate.lean}.  Derives $\tau = 3/7$ — the
ratio of proper time to coordinate time for an odd-parity cell in the Rule~110
vacuum background (ether) — without any axioms or sorry.  The derivation proceeds
directly from the Rule~110 update rule and the canonical period-14 ether pattern.

\begin{theorem}[\lstinline|ether_is_rule110_orbit|, zero \texttt{sorry}, zero axioms ($\CatAL$)]
The canonical period-14 ether pattern \texttt{[1,1,1,1,1,0,0,0,1,0,0,1,1,0]} is a
genuine Rule~110 orbit: one update equals a left-shift by exactly 4 cells.
Proved by \texttt{decide} over the explicit 14-cell Boolean state.
\end{theorem}

\begin{theorem}[\lstinline|ether_period_seven|, \lstinline|ether_period_minimal|, zero \texttt{sorry} ($\CatAL$)]
The intrinsic temporal period at any fixed ether cell is exactly~7: seven applications
of the Rule~110 update return the pattern to itself ($4 \times 7 = 28 \equiv 0 \pmod{14}$),
and no smaller positive $k$ does so.  The period formula $7 = 14/\gcd(4,14)$ is
machine-certified (\lstinline|ether_period_formula|, zero \texttt{sorry}).
\end{theorem}

\begin{theorem}[\lstinline|tau_proper_rate|, \lstinline|tau_three_sevenths_from_ether|, zero \texttt{sorry}, zero axioms ($\CatAL$)]
\label{thm:tau-three-sevenths}
The proper-time rate of an odd-parity ether cell equals $\tau = 3/7$.
Of the seven odd-index positions in the period-14 ether, exactly three carry state~1
(\lstinline|ether_odd_fire_count|).  Since the gate (proper-time tick) fires iff the
ether cell is in state~1, the fractional fire rate over one full period is $3/7$.
The denominator~7 is forced by ether drift~4 and spatial period~14; the numerator~3 is
the odd-parity 1-count, both forced by the ether pattern with no free parameters.
The master bundle \lstinline|tau_three_sevenths_from_ether| packages all steps (zero
axioms, zero \texttt{sorry}).
\end{theorem}

\begin{remark}[Relation to $\tau_{\rm proper}/\tau_{\rm lab}$ in P45 and P47]
The result $\tau = 3/7$ certified here is the \emph{derivation} of the structural
input used in \texttt{TemporalVoxelCC.lean}.  The temporal voxel CC formula
$\Omega_\Lambda = 3\pi/14$ takes $\tau = 3/7$ as input; the present module proves
it from first principles, closing the derivation chain without any assumption about
$N_{\rm spatial}/|\mathbb{Z}_7|$.
\end{remark}

\subsection{L1\,\textrightarrow\,L2 proper-time bridge (\texttt{Gravity/PhiMDLProperTimeBridge.lean})}\label{subsec:phimdl-proper-time-bridge}

Module: \texttt{Gravity/PhiMDLProperTimeBridge.lean}.  Closes the explicit
Level-1 $\to$ Level-2 connection for the proper-time rate $\tau = 3/7$: the
ether-derived $\tau$ of \texttt{EtherProperTimeRate.lean} is formally identified
with the $\tau$ parameter entering the temporal voxel CC formula in
\texttt{TemporalVoxelCC.lean}.  Zero \texttt{sorry}, zero axioms (\CatAD).

\begin{theorem}[\lstinline|phimdl_tau_from_ether|, zero \texttt{sorry}, zero axioms ($\CatAD$)]
The $\Phi_{\rm MDL}$ proper-time rate equals the Rule~110 ether-derived value:
$\tau_{\Phi{\rm MDL}} = \texttt{EtherProperTimeRate.tauProper} = 3/7$.
Proved by unfolding \lstinline|EtherProperTimeRate.tau_proper_rate|.
\end{theorem}

\begin{theorem}[\lstinline|phimdl_voxel_coeff_from_ether_tau|, zero \texttt{sorry}, zero axioms ($\CatAD$)]
The temporal voxel CC coefficient satisfies
$2 \times C_{\rm Gorard} \times \tau_{\rm ether} = 2 \times (3/32) \times (3/7) = 9/112$,
where $\tau_{\rm ether} = \texttt{EtherProperTimeRate.tauProper}$ (L1, \CatAD) and
$C_{\rm Gorard} = 3/32$ (\CatAL).  Proved by \lstinline|norm_num| after rewriting
with \lstinline|tau_proper_rate|.
\end{theorem}

\begin{theorem}[\lstinline|l1_to_l2_tau_bridge|, zero \texttt{sorry}, zero axioms ($\CatAD$)]
\label{thm:l1-l2-tau-bridge}
\textbf{Master L1\,\textrightarrow\,L2 $\tau$ bundle.}  The full derivation chain is certified:
\begin{enumerate}
  \item Ether-derived $\tau = 3/7$ (\lstinline|phimdl_tau_from_ether|, \CatAD)
  \item Voxel coefficient $9/112 = 2C_{\rm Gorard}\tau_{\rm ether}$ (\lstinline|phimdl_voxel_coeff_from_ether_tau|, \CatAD)
  \item $\Omega_\Lambda = (9/112)\times(8\pi/3) = 3\pi/14$ from \lstinline|TemporalVoxelCC.omega_lambda_from_temporal_voxel|
\end{enumerate}
The $\Phi_{\rm MDL}$ vacuum energy density inherits its proper-time suppression factor from
Rule~110 CMCA ether dynamics via the Algebraic Lifting Theorem.
\end{theorem}

\begin{theorem}[\lstinline|tau_action_coupling_catad|, zero \texttt{sorry}, zero axioms ($\CatAD$)]
\label{thm:tau-action-coupling-catad}
$\tau=3/7$ as $\Phi_{\rm MDL}$ proper-time action coupling: in lab-time coordinates the
temporal kinetic term acquires $\tau^2 = (3/7)^2 = 9/49$
(\lstinline|phimdl_tau_action_coupling_sq|,
\lstinline|phimdl_tau_sq_from_ether|); the CC coefficient factorizes as
$2\,C_{\rm Gorard}\,\tau = 9/112$
(\lstinline|phimdl_cc_coefficient_factored|).  \CatAD\ structural consequence of ether dynamics.
\end{theorem}
...
\subsection{$\Phi_{\rm MDL}$ propagator, quartic vertex, and ZZ exact S-matrix}\label{subsec:phimdl-propagator-g9}
...
\item \textbf{\texttt{Spacetime/OrbitMassHierarchy.lean}} (zero sorry):
...
\begin{keybox}{\lstinline|alpha_em_inverse_structural_identity| (zero sorry)}
The inverse fine-structure constant at zero momentum from GTE orbit arithmetic:
\[
  \frac{1}{\alpha_{\rm em}(0)} = 2^{|Z_7|} + N_{\rm gen}^2 = 2^7 + 3^2 = 128 + 9 = 137.
\]
Module: \texttt{Universality/AlphaEMStructuralIdentity.lean}.
\end{keybox}

\begin{keybox}{\lstinline|b0_casimir_relation| (zero sorry)}
The one-loop QCD coefficient matches orbit-family counting:
\[
  b_0 = N_{\rm fam} + N_{\rm gen} - 1 = 5 + 3 - 1 = 7 = |Z_7|.
\]
Consequence: $C(b_0,N_{\rm gen})/C(b_0,N_{\rm fam}) = 5/3 = N_{\rm fam}/N_{\rm gen}$.
Module: \texttt{Universality/CasimirB0Relation.lean}.
\end{keybox}

\begin{keybox}{\lstinline|top_bottom_mass_ratio_approx| (zero sorry)}
Top-to-bottom quark $N_{\rm eff}$ ratio:
\[
  \frac{b_{\rm top}}{b_b} = \frac{337920}{8191} \approx 41.255,
\]
tracking $m_t/m_b$ to $0.072\%$ (PDG). Module: \texttt{Universality/GUTStructure.lean}.
\end{keybox}

\begin{keybox}{\lstinline|m_omega_ksrf_formula| (zero sorry)}
$\omega$-meson mass from KSRF with GTE coupling $g_\omega^2 = N_{\rm gen}\,c_Z = 36$:
\[
  m_\omega = \sqrt{2\,g_\omega^2}\,f_\pi = \sqrt{72}\,f_\pi \approx 783.55\,\mathrm{MeV}
  \quad (+0.11\%\text{ vs.\ PDG }782.65\,\mathrm{MeV}).
\]
Module: \texttt{MassRelations/MesonMasses.lean}; uses \lstinline|fpi_from_scc|.
\end{keybox}

\begin{keybox}{\lstinline|g_rho_sq_eq_casimir_orbit| (zero sorry)}
$\rho$-meson isospin coupling squared from GTE orbit counting:
\[
  g_\rho^2 = C(b_0,N_{\rm gen}) = \binom{7}{3} = 35.
\]
Module: \texttt{MassRelations/MesonMasses.lean}; uses \lstinline|Z7_order|, \lstinline|n_gen|.
\end{keybox}

\begin{keybox}{\lstinline|g_omega_rho_coupling_gap| (zero sorry)}
Coupling gap between $\omega$ and $\rho$ vector mesons:
\[
  g_\omega^2 - g_\rho^2 = N_{\rm gen}\,c_Z - C(b_0,N_{\rm gen}) = 36 - 35 = 1.
\]
Module: \texttt{MassRelations/MesonMasses.lean}.
\end{keybox}

\begin{keybox}{\lstinline|m_rho_ksrf_formula| (zero sorry)}
$\rho$-meson mass from KSRF with $g_\rho^2 = 35$:
\[
  m_\rho = \sqrt{2\,g_\rho^2}\,f_\pi = \sqrt{70}\,f_\pi \approx 772.57\,\mathrm{MeV}
  \quad (\text{GTE SCC; PDG }775.26\,\mathrm{MeV}).
\]
Numerical certificate \lstinline|m_rho_ksrf_scc_value| bounds the prediction within
$0.1\,\mathrm{MeV}$ of $772.57\,\mathrm{MeV}$.
Module: \texttt{MassRelations/MesonMasses.lean}; uses \lstinline|fpi_from_scc|.
\end{keybox}

\begin{keybox}{\lstinline|pion_mass_from_gor| (zero sorry)}
GOR pion mass with GTE chiral condensate $\langle\bar\psi\psi\rangle_{\rm GTE}=-M_{\rm kink}^3$
(\lstinline|chiral_condensate_kink_identification|) and $f_\pi=M_{\rm kink}/\pi$
(\lstinline|fpi_from_scc|):
\[
  m_\pi = \pi\sqrt{(m_u+m_d)\,M_{\rm kink}}.
\]
Numerical certificate \lstinline|pion_mass_numerical_certificate| bounds the GTE
quark sum $m_u+m_d=6.8040\,\mathrm{MeV}$ and $M_{\rm kink}=290.10\,\mathrm{MeV}$
prediction $139.57\,\mathrm{MeV}$ vs.\ PDG $139.5703\,\mathrm{MeV}$.
Module: \texttt{MassRelations/PionMassFromGOR.lean}.
\end{keybox}

\begin{keybox}{\lstinline|m_u_pdg_interval| (zero sorry, CatAD)}
Up-type $m_u$ from the TT cascade at PDG $(m_e,m_\mu)$ anchors lies in the
PDG 2024 interval $[2.09,2.23]\,\mathrm{MeV}$.
Uses \lstinline|tt_exp_g0_bounds| from \texttt{TranscendentalMassBounds.lean}.
Module: \texttt{MassRelations/QuarkMassNumericalCerts.lean}.
\end{keybox}

\begin{keybox}{\lstinline|m_d_pdg_interval|, \lstinline|m_s_pdg_interval| (zero sorry, CatAD)}
Down-type $m_d$ and $m_s$ from the VV cascade at PDG anchors lie in certified
PDG bands (uses \lstinline|m_d_predicted_interval|,
\lstinline|m_s_predicted_interval|).
Module: \texttt{MassRelations/QuarkMassNumericalCerts.lean}.
\end{keybox}

\begin{keybox}{\lstinline|m_c_pdg_interval|, \lstinline|m_t_pdg_interval| (zero sorry, CatAD)}
Up-type $m_c$ and $m_t$ PDG intervals (uses \lstinline|m_c_predicted_interval|,
\lstinline|m_t_predicted_interval|, \lstinline|tt_exp_g1_bounds|,
\lstinline|tt_exp_g2_bounds|).
Module: \texttt{MassRelations/QuarkMassNumericalCerts.lean}.
\end{keybox}

\begin{keybox}{\lstinline|m_b_pdg_interval| (zero sorry, CatAD)}
Bottom quark mass in $[4175,4191]\,\mathrm{MeV}$ at PDG anchors
(uses \lstinline|m_b_predicted_interval|, \lstinline|vv_gamma_exp_bounds|,
\lstinline|koide_tau_mass_bounds|).
Certified in \texttt{QuarkMassNumericalCerts.lean} and
\texttt{MassRelations/DownRational.lean}.
\end{keybox}

\begin{keybox}{\lstinline|m_W_pdg_interval| (zero sorry, CatAL)}
GTE electroweak scale $M_W=80364\,\mathrm{MeV}$ lies in the PDG 2024 interval
$(80351,80377)\,\mathrm{MeV}$.
Module: \texttt{Universality/EWBosonNumericalCerts.lean}.
\end{keybox}

\begin{keybox}{\lstinline|m_Z_gte_from_weinberg| (zero sorry, CatAL)}
$M_Z = M_W/\sqrt{1-\sin^2\theta_W} = M_W\sqrt{13/10}$ (definitional from GTE
Weinberg $\cos^2\theta_W=3/13$).
Module: \texttt{Universality/EWBosonNumericalCerts.lean}.
\end{keybox}

\begin{keybox}{\lstinline|m_Z_pdg_interval| (zero sorry, CatAD)}
GTE $M_Z = M_W\sqrt{13/10}$ lies in $(91600,91660)\,\mathrm{MeV}$
(uses \lstinline|sqrt_thirteen_ten_bounds|).
Module: \texttt{Universality/EWBosonNumericalCerts.lean}.
\end{keybox}

\begin{keybox}{\lstinline|m_Z_rho_corrected_interval| (zero sorry, CatAD)}
GTE $\hat\rho$-corrected $Z$ boson mass $M_Z^{\rm phys} \in (91190,91220)\,\mathrm{MeV}$,
using GTE-internal top-quark pole mass $m_t=172610\,\mathrm{MeV}$ (TT+VV cascade) and
leading top self-energy correction $\hat\rho=1+3G_F m_t^2/(8\pi^2\sqrt{2})$
(uses \lstinline|sqrt_rho_hat_GTE_bounds|, \lstinline|sqrt_thirteen_ten_bounds|).
Module: \texttt{Universality/EWRhoCorrectedMZ.lean}.
\end{keybox}

\begin{keybox}{\lstinline|sin2_theta_W_threshold_interval| (zero sorry, CatAD)}
Threshold-corrected $\sin^2\theta_W$ in $[0.23128,0.23131]$ at GTE inputs
(uses \lstinline|higgs_threshold_log_bound|,
\lstinline|sin2_threshold_value_bounds|).
Module: \texttt{Universality/EWBosonNumericalCerts.lean}.
\end{keybox}

\begin{keybox}{\lstinline|fn_washout_sum_bounds| (zero sorry, CatAD)}
Three-generation FN washout sum $S=1+\exp(-2\pi/3)+\exp(-2\pi)\in[1.124,1.126]$
(uses \lstinline|exp_neg_two_pi_over_three_bounds|,
\lstinline|exp_neg_two_pi_bounds|).
Module: \texttt{MassRelations/SeesawNumericalCerts.lean}.
\end{keybox}

\begin{keybox}{\lstinline|b5_seesaw_weight_bounds|, \lstinline|b11_seesaw_weight_bounds|, \lstinline|b19_seesaw_weight_bounds| (zero sorry, CatAL)}
Integer 9th-power certificates $178<5^{29/9}<179$, $2267<11^{29/9}<2268$,
$13195<19^{29/9}<13196$.
Module: \texttt{MassRelations/SeesawNumericalCerts.lean}.
\end{keybox}

\begin{keybox}{\lstinline|gte_delta_m21_sq_bounds|, \lstinline|gte_delta_m31_sq_bounds| (zero sorry, CatAD)}
GTE neutrino splittings $\Delta m^2_{21}\in[7.36,7.38]\times10^{-5}\,\mathrm{eV}^2$,
$\Delta m^2_{31}\in[2.509,2.513]\times10^{-3}\,\mathrm{eV}^2$ at $\Sigma m_\nu=59.4\,\mathrm{meV}$
(uses \lstinline|gte_delta_m21_sq_interval|,
\lstinline|gte_delta_m31_sq_interval|).
Module: \texttt{MassRelations/SeesawNumericalCerts.lean}.
\end{keybox}

\begin{keybox}{\lstinline|mr_gut_geV_bounds| (zero sorry, CatAD)}
Common seesaw scale $M_{R,\mathrm{GUT}}\in[1.89,1.91]\times10^{16}\,\mathrm{GeV}$
from $E_D=v_H/29$ and $C=\Sigma m_\nu/\sum b_i^{29/9}$
(uses \lstinline|mr_gut_geV_interval|).
Module: \texttt{MassRelations/SeesawNumericalCerts.lean}.
\end{keybox}

\begin{keybox}{\lstinline|mr1_mr_gut_ratio_bounds| (zero sorry, CatAD)}
Pati--Salam ratio $M_{R1}/M_{R,\mathrm{GUT}}=\exp(-\pi)/(19/5)^{29/9}\in[5.828,5.863]\times10^{-4}$
(uses \lstinline|exp_neg_pi_bounds|,
\lstinline|mr1_mr_gut_ratio_interval|).
Module: \texttt{MassRelations/SeesawNumericalCerts.lean}.
\end{keybox}

\begin{keybox}{\lstinline|bps_kink_is_half_instanton|, \lstinline|bps_per_tape_action| (zero sorry, CatAL)}
BPS half-instanton $S_{\rm kink}=\tfrac12(2\pi/|Z_7|)$ and per-tape action
$S_1=\pi/N_c=\pi/3$ from $|Z_7|$ cancellation.
$T_{11}=0$ is definitionally certified (\lstinline|phi_mdl_kink_bps_saturation := rfl|; zero axioms).
Module: \texttt{Physics/FKTTCoupling.lean}.
\end{keybox}

\begin{keybox}{\lstinline|per_tape_bps_action_eq_pi_over_Nc|, \lstinline|per_tape_bps_action_bound| (zero sorry, CatAL)}
Per-tape BPS action $S_1=\pi/3$ and $S_1\in[1.047,1.048]$.
Module: \texttt{Physics/FKTTCoupling.lean}.
\end{keybox}

\begin{keybox}{\lstinline|kink_top_coupling_eq_eps_FN|, \lstinline|kink_top_coupling_bounds| (zero sorry, CatAL)}
Kink--top Yukawa $g_{\rm kink\text{-}top}=\exp(-S_1)=\varepsilon_{\rm FN}=\exp(-\pi/3)$
with $g_{\rm kink\text{-}top}\in[0.3508,0.3510]$ (zero axioms; unconditional CatAL).
Module: \texttt{Physics/FKTTCoupling.lean}.
\end{keybox}
...
Module: \texttt{Spacetime/QECStabilizer.lean} (canonical library,
...
Module: \texttt{Spacetime/DWeightSRFormula.lean} (zero
...
(\texttt{QECStabilizer.lean}) with the SR proper-time ratio
identity (\texttt{CausalInvariance.lean}).
...
correction.  In the continuum limit $M\to\infty$ this gives
...
Computational evidence (CatA): true AFCA at $M=7$, $L=300$,
...
continuum companion (Klein--Gordon substrate) achieves
...
Module: \texttt{Substrate/Substrate.lean} (zero
\lstinline|sorry|, zero custom axioms).
...
Module: \texttt{Substrate/PSCPreservingTransformation.lean} (zero
\lstinline|sorry|, zero custom axioms).
...
Conservative stubs are refined with real RC content on
...
Module: \texttt{Substrate/CoherenceMeasure.lean} (zero
\lstinline|sorry|, \textbf{one named axiom} \lstinline|d2_universal|).
...
Module: \texttt{Substrate/LagrangianLorentzScalar.lean} (zero
\lstinline|sorry|, zero custom axioms).
...
Module: \texttt{Substrate/PSCStructureLorentzPreserved.lean} (zero
\lstinline|sorry|, zero custom axioms).
RC is proved; NM$^*$/TV/SA remain formal stubs (\texttt{True}).
...
Module: \texttt{Substrate/NoetherAngularMomentum.lean} (zero
\lstinline|sorry|, \textbf{one PDE axiom}
...
Module: \texttt{Substrate/PSCPILorentzMain.lean} (canonical library, zero
...
Module: \texttt{Spacetime/MultiParticleHilbert.lean} (zero
...
\bottomrule
\end{tabular}
\caption{Cross-repository bridge theorems (canonical library).}
...
\subsection{Theorem inventory: foundational completions}\label{subsec:new-theorem-inventory-083}

The following theorems span four modules:
\texttt{Gravity/Z7AnomalyFree.lean} formalises the $\mathbb{Z}_7$ global
scalar anomaly-free argument at the Lebesgue-measure level;
\texttt{ContinuumLimit/GorardVacuumW1Bridge.lean} closes the Gorard ORIC
axiom for all adjacent vacuum edges via an explicit Wasserstein-1 calculation;
\texttt{srrg-lean}: \texttt{Bridges/ToMDL.lean} provides the OP9 MDL equivalence
proof at CatAL and CatAD levels; and \texttt{Core/CMCALanguage.lean} formalises
the CMCA encoding-language scaffold.
...
Theorem & Module & Physical content \\
\midrule
\multicolumn{3}{@{}l}{\textit{G08 --- $\mathbb{Z}_7$ global scalar anomaly-free (\texttt{Gravity/Z7AnomalyFree.lean})}} \\[2pt]
\texttt{lebesgue\_\allowbreak shift\_\allowbreak invariant}
  & Gravity.Z7AnomalyFree
  & Lebesgue measure is shift-invariant: $\lambda(A+c)=\lambda(A)$ (zero \texttt{sorry}) \\[1pt]
\texttt{z7\_\allowbreak shift\_\allowbreak measure\_\allowbreak preserving}
  & Gravity.Z7AnomalyFree
  & $\mathbb{Z}_7$ shift $\phi\mapsto\phi+2\pi/7$ is Lebesgue-measure-preserving (zero \texttt{sorry}) \\[1pt]
\texttt{z7\_\allowbreak jacobian\_\allowbreak eq\_\allowbreak one}
  & Gravity.Z7AnomalyFree
  & Jacobian of $\mathbb{Z}_7$ shift $= 1$ a.e.\ (zero \texttt{sorry}) \\[1pt]
\texttt{z7\_\allowbreak global\_\allowbreak scalar\_\allowbreak anomaly\_\allowbreak free}
  & Gravity.Z7AnomalyFree
  & $\mathbb{Z}_7$ global scalar shift symmetry is anomaly-free (Fujikawa argument; zero \texttt{sorry}) \\[1pt]
\texttt{z7\_\allowbreak vacuum\_\allowbreak sectors\_\allowbreak equiprobable}
  & Gravity.Z7AnomalyFree
  & All seven $\mathbb{Z}_7$ vacuum sectors have equal integration weight (1 \texttt{sorry}: path-integral bridge) \\[1pt]
\midrule
\multicolumn{3}{@{}l}{\textit{Gorard ORIC adjacent-edge $\kappa=0$ (\texttt{ContinuumLimit/GorardVacuumW1Bridge.lean})}} \\[2pt]
\texttt{vacuum\_\allowbreak coupling\_\allowbreak cost\_\allowbreak eq\_\allowbreak one}
  & ContinuumLimit.\allowbreak GorardVacuumW1Bridge
  & Explicit transport cost between adjacent vacuum walk measures $= 1$ (zero \texttt{sorry}) \\[1pt]
\texttt{vacuum\_\allowbreak w1\_\allowbreak le\_\allowbreak one}
  & ContinuumLimit.\allowbreak GorardVacuumW1Bridge
  & $W_1 \le 1$ upper bound via explicit coupling (zero \texttt{sorry}) \\[1pt]
\texttt{vacuum\_\allowbreak w1\_\allowbreak eq\_\allowbreak one}
  & ContinuumLimit.\allowbreak GorardVacuumW1Bridge
  & $W_1 = 1$ for adjacent vacuum walk measures (zero \texttt{sorry}) \\[1pt]
\texttt{gorard\_\allowbreak vacuum\_\allowbreak oric\_\allowbreak zero\_\allowbreak adjacent}
  & ContinuumLimit.\allowbreak GorardVacuumW1Bridge
  & $\kappa_{\rm OR}=0$ for all adjacent vacuum edges; scoped path (zero \texttt{sorry}) \\[1pt]
\texttt{gorard\_\allowbreak vacuum\_\allowbreak oric\_\allowbreak zero\_\allowbreak scoped}
  & ContinuumLimit.\allowbreak GorardVacuumW1Bridge
  & $\kappa_{\rm OR}=0$ for \emph{all} adjacent vacuum edges $\forall n$ (\CatAL; zero \texttt{sorry}) \\[1pt]
\midrule
\multicolumn{3}{@{}l}{\textit{OP9 MDL equivalence (\texttt{srrg-lean/Bridges/ToMDL.lean})}} \\[2pt]
\texttt{argmax\_\allowbreak viability\_\allowbreak iff\_\allowbreak argmin\_\allowbreak descLen}
  & SrrgLean.Bridges.ToMDL
  & $\mathop{\rm argmax}_S F[S] = \mathop{\rm argmin}_S K[S]$; OP9 algebraic core (\CatAD; zero \texttt{sorry}) \\[1pt]
\texttt{srrg\_\allowbreak fp\_\allowbreak is\_\allowbreak mdl\_\allowbreak minimizer}
  & SrrgLean.Bridges.ToMDL
  & SRRG fixed point $g^*=1/\varphi$ is the MDL minimizer (\CatAD; zero \texttt{sorry}) \\[1pt]
\texttt{srrg\_\allowbreak mdl\_\allowbreak functional\_\allowbreak identity}
  & SrrgLean.Bridges.ToMDL
  & $K[S] = B - F[S]$ functional identity (\CatAD; zero \texttt{sorry}) \\[1pt]
\texttt{srrg\_\allowbreak op9\_\allowbreak k\_\allowbreak alg\_\allowbreak biconditional}
  & SrrgLean.Bridges.ToMDL
  & OP9 biconditional for algebraic MDL profile (\CatAD; zero \texttt{sorry}; unconditional) \\[1pt]
\texttt{op9\_\allowbreak catal\_\allowbreak unconditional}
  & SrrgLean.Bridges.ToMDL
  & OP9 \CatAL\ at canonical SRRG instance: $\mathop{\rm argmax} F = \mathop{\rm argmin} K$ (zero \texttt{sorry}; no hypotheses) \\[1pt]
\midrule
\multicolumn{3}{@{}l}{\textit{$W_1$ Wasserstein distance scaffold (\texttt{ContinuumLimit/WassersteinDistance.lean})}} \\[2pt]
\texttt{W1\_\allowbreak ge\_\allowbreak of\_\allowbreak lipschitz}
  & ContinuumLimit.\allowbreak WassersteinDistance
  & Kantorovich dual lower bound: $W_1 \ge |E_\mu[f] - E_\nu[f]|$ for 1-Lip $f$ (zero \texttt{sorry}) \\[1pt]
\texttt{W1\_\allowbreak triangle}
  & ContinuumLimit.\allowbreak WassersteinDistance
  & Triangle inequality $W_1(\mu,\rho) \le W_1(\mu,\nu) + W_1(\nu,\rho)$ via glued coupling (zero \texttt{sorry}) \\[1pt]
\texttt{W1\_\allowbreak eq\_\allowbreak zero\_\allowbreak iff}
  & ContinuumLimit.\allowbreak WassersteinDistance
  & $W_1(\mu,\nu) = 0 \Leftrightarrow \mu = \nu$ (zero \texttt{sorry}) \\[1pt]
\texttt{gluedCoupling\_\allowbreak cost\_\allowbreak le}
  & ContinuumLimit.\allowbreak WassersteinDistance
  & Gluing lemma: cost of glued coupling $\le$ sum of marginal costs (zero \texttt{sorry}) \\[1pt]
\texttt{W1\_\allowbreak attained}
  & ContinuumLimit.\allowbreak WassersteinDistance
  & Infimum attained: $\exists\,\gamma$ s.t.\ $W_1 = \text{cost}(\gamma)$ (finite coupling polytope; zero \texttt{sorry}) \\[1pt]
\texttt{couplingCostSet\_\allowbreak isCompact}
  & ContinuumLimit.\allowbreak WassersteinDistance
  & Achievable transport costs form a compact set (finite polytope compactness; zero \texttt{sorry}) \\[1pt]
\midrule
\multicolumn{3}{@{}l}{\textit{CMCA encoding language scaffold (\texttt{srrg-lean/Core/CMCALanguage.lean})}} \\[2pt]
\texttt{mdl\_\allowbreak self\_\allowbreak desc\_\allowbreak eq}
  & SrrgLean.Core.CMCALanguage
  & $L[S] = B - R[S]$ (self-description length; zero \texttt{sorry}) \\[1pt]
\texttt{mdl\_\allowbreak conditional\_\allowbreak eq}
  & SrrgLean.Core.CMCALanguage
  & $L[{\rm data}|S] = C_\Lambda[S]$ (conditional description length; zero \texttt{sorry}) \\[1pt]
\texttt{cmca\_\allowbreak k\_\allowbreak eq\_\allowbreak barrier\_\allowbreak minus\_\allowbreak viability}
  & SrrgLean.Core.CMCALanguage
  & $K_{\rm CMCA}(g) = B - V(g)$ (CMCA Kolmogorov description length; zero \texttt{sorry}) \\[1pt]
\bottomrule
\end{tabular}
\caption{Foundational completion theorems: $\mathbb{Z}_7$ global scalar anomaly-free argument;
Gorard ORIC adjacent-edge vacuum flatness ($W_1=1$, $\kappa=0$ $\forall n$);
$W_1$ Wasserstein distance scaffold fully \CatAL\ (triangle inequality, Kantorovich bound,
attainment, compactness — all zero \texttt{sorry});
OP9 MDL equivalence at \CatAD/\CatAL\ level (unconditional);
and CMCA encoding-language scaffold.
Modules \texttt{Z7AnomalyFree}, \texttt{GorardVacuumW1Bridge}, and \texttt{WassersteinDistance}
are in \texttt{ugp-lean};
\texttt{ToMDL} and \texttt{CMCALanguage} are in \texttt{srrg-lean}.}
\label{tab:new-theorem-inventory-083}
\end{table}

\subsection{Frontier-closure theorems}\label{subsec:frontier-closures}

This section covers \textbf{221} net-new zero-\texttt{sorry} theorems across
twenty-two modules, closing the Yukawa vertex, PMNS orbit-ratio sector, SRRG Higgs
quartic, full nine-coefficient UCL master certification, FGCI uniqueness at
$N_c=3$, Pöschl--Teller fluctuation/QFT integral infrastructure, and the
$D_{\rm top}=\exp(-1/N_c)$ topological dilution chain.
Highlights are summarized in \cref{tab:frontier-closures}; the full catalog is in
\texttt{docs/THEOREMS.md}.

\begin{table}[h]
\centering\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}p{5.0cm}p{4.2cm}p{4.6cm}@{}}
\toprule
Theorem & Module & Physical content \\
\midrule
\multicolumn{3}{@{}l}{\textit{UCL master certification --- all nine EK coefficients (\CatAL)}} \\[2pt]
\texttt{elegant\_\allowbreak kernel\_\allowbreak full\_\allowbreak certification}
  & ElegantKernel.\allowbreak Unconditional.\allowbreak MasterCertification
  & $k_{L^2},k_{\rm gen},k_{\rm gen2},k_L,k_{\rm const},k_M,k_{a,b,c}$ bundled; links to FullClosure, KGen/KL/KConst, MuTriple, QuarterLock \\[1pt]
\texttt{thm\_\allowbreak quarter\_\allowbreak lock\_\allowbreak catal}
  & QuarterLock
  & $k_M = -\varphi/2 + 7/2048$ (Quarter-Lock, \CatAL) \\[1pt]
\texttt{ucl\_\allowbreak fermion\_\allowbreak mass\_\allowbreak ordering}
  & ElegantKernel.\allowbreak Unconditional.\allowbreak UCLMassOrdering
  & gen$_1<$gen$_2<$gen$_3$ per sector under UCL + $4^{g-1}$ scaling (interval margin certs) \\[1pt]
\texttt{ucl\_\allowbreak lepton\_\allowbreak sector\_\allowbreak koide\_\allowbreak identity}
  & ElegantKernel.\allowbreak Unconditional.\allowbreak UCLKoide
  & Koide $Q=2/3$ from GTE-pinned phase and amplitude chain (\CatAL) \\[1pt]
\midrule
\multicolumn{3}{@{}l}{\textit{PMNS orbit ratios and CP (\CatAL)}} \\[2pt]
\texttt{pmns\_\allowbreak solar\_\allowbreak sin\_\allowbreak sq}
  & MassRelations.NeutrinoSector
  & $\sin^2\theta_{12} = 4/13$ \\[1pt]
\texttt{pmns\_\allowbreak atm\_\allowbreak sin\_\allowbreak sq}
  & MassRelations.NeutrinoSector
  & $\sin^2\theta_{23} = 19/42$ \\[1pt]
\texttt{pmns\_\allowbreak reactor\_\allowbreak sin\_\allowbreak val}
  & MassRelations.NeutrinoSector
  & $\sin\theta_{13} = 11/73$ \\[1pt]
\texttt{pmns\_\allowbreak cp\_\allowbreak phase\_\allowbreak from\_\allowbreak z7\_\allowbreak winding}
  & MassRelations.NeutrinoSector
  & $\delta_{\rm CP} = 8\pi/7$ from $W_L=4$ \\[1pt]
\texttt{gte\_\allowbreak jarlskog\_\allowbreak is\_\allowbreak negative}
  & MassRelations.NeutrinoSector
  & GTE Jarlskog invariant $J<0$ \\[1pt]
\midrule
\multicolumn{3}{@{}l}{\textit{Higgs quartic from SRRG (\CatAL)}} \\[2pt]
\texttt{higgs\_\allowbreak quartic\_\allowbreak ch\_\allowbreak correction}
  & MassRelations.HiggsQuartic
  & $\lambda = \dfrac{\varphi}{4\pi}\bigl(1+\dfrac{{\rm IPT}-1}{27}\bigr)$ with $2c_H+1=N_{\rm gen}^3$ \\[1pt]
\texttt{higgs\_\allowbreak quartic\_\allowbreak numerical\_\allowbreak approx}
  & MassRelations.HiggsQuartic
  & $0.12 < \lambda < 0.14$ (PDG central $\approx 0.129$) \\[1pt]
\midrule
\multicolumn{3}{@{}l}{\textit{$\Phi_{\rm MDL}$ Yukawa vertex and sech integrals}} \\[2pt]
\texttt{phimdl\_\allowbreak fluctuation\_\allowbreak is\_\allowbreak poschl\_\allowbreak teller}
  & Substrate.\allowbreak PhiMDLFluctuationSpectrum
  & $V_{\rm fl}(x)=m_\phi^2[1-2\,\mathrm{sech}^2(m_\phi x)]$ (\CatAL) \\[1pt]
\texttt{integral\_\allowbreak sech} / \texttt{integral\_\allowbreak sech\_\allowbreak cubed}
  & Substrate.\allowbreak PhiMDLFluctuationSpectrum
  & $\int\mathrm{sech}\,dx=\pi$, $\int\mathrm{sech}^3\,dx=\pi/2$ (\CatAL; Mathlib FTC) \\[1pt]
\texttt{sech\_\allowbreak overlap\_\allowbreak asymptotic}
  & Substrate.\allowbreak PhiMDLFluctuationSpectrum
  & $r\cdot I(r)\to\pi$ as $r\to\infty$ (\CatAL) \\[1pt]
\texttt{sech\_\allowbreak overlap\_\allowbreak at\_\allowbreak five\_\allowbreak ge\_\allowbreak pi}
  & Substrate.\allowbreak SechOverlapIntegralBounds
  & $I(5)\ge 0.95\pi/5$ (zero \texttt{sorry}, zero axiom; imports bridge) \\[1pt]
\texttt{sech\_\allowbreak overlap\_\allowbreak at\_\allowbreak eleven\_\allowbreak ge\_\allowbreak pi}
  & Substrate.\allowbreak SechOverlapIntegralBounds
  & $I(11)\ge 0.99\pi/11$ (zero \texttt{sorry}, zero axiom) \\[1pt]
\texttt{SechOverlapIntegralBounds\_\allowbreak cosh} / \texttt{\_r5bins}
  & Substrate.\allowbreak SechOverlapIntegralBounds
  & Certified $\cosh$ and per-bin sech-product rational lower bounds (\CatAL) \\[1pt]
\texttt{SechOverlapIntegralBounds\_\allowbreak bridge}
  & Substrate.\allowbreak SechOverlap\allowbreak IntegralBounds\_bridge
  & 2 \CatA\ axioms: mesh sum $\le$ integral for $r\in\{5,11\}$ \\[1pt]
\texttt{phimdl\_\allowbreak yukawa\_\allowbreak vertex\_\allowbreak catad}
  & Substrate.\allowbreak PhiMDLFluctuationSpectrum
  & Three-kink overlap amplitude assembly (\CatAD) \\[1pt]
\texttt{yukawa\_\allowbreak overlap\_\allowbreak exponent\_\allowbreak catad}
  & Gravity.YukawaOverlapExponent
  & Dirac scaling $\alpha=N_c-1=2$ from three-tape DPP counting (\CatAD) \\[1pt]
\texttt{leptogenesis\_\allowbreak kink\_\allowbreak overlap\_\allowbreak catad}
  & Gravity.YukawaOverlapExponent
  & $f_1\times f_2 = 1/3025 = 1/(5^2\cdot 11^2)$ (\CatAD; \CatAL\ conditional on sech bridge) \\[1pt]
\texttt{eta\_\allowbreak B\_\allowbreak PDG\_\allowbreak in\_\allowbreak GTE\_\allowbreak bracket}
  & Gravity.YukawaOverlapExponent
  & Planck $\eta_B$ in GTE sech--asymptotic interval $[5.62,6.36]\times 10^{-10}$ (\CatAL\ conditional) \\[1pt]
\texttt{eta\_\allowbreak B\_\allowbreak loop\_\allowbreak bracket}
  & Gravity.YukawaOverlapExponent
  & Planck $\eta_B$ in loop-function interval $[6.06,6.36]\times 10^{-10}$ (\CatAL\ conditional) \\[1pt]
\texttt{pmns\_\allowbreak atm\_\allowbreak nlo\_\allowbreak formula}
  & MassRelations.PMNSNLOCorrection
  & $\sin^2\theta_{23}^{\mathrm{NLO}} = b_{R2}b_{R3}/|F_{21}|^2 = 209/441$ (\CatAL) \\[1pt]
\texttt{two\_\allowbreak b\_\allowbreak R2\_\allowbreak eq\_\allowbreak F21\_\allowbreak succ}
  & MassRelations.PMNSNLOCorrection
  & $2 b_{R2} = |F_{21}|+1 = 22$ (\CatAL) \\[1pt]
\texttt{ngen\_\allowbreak eq\_\allowbreak nfam\_\allowbreak minus\_\allowbreak two}
  & MassRelations.PMNSNLOCorrection
  & $N_{\mathrm{gen}} = N_{\mathrm{fam}}-2 = 3$ (\CatAL) \\[1pt]
\midrule
\multicolumn{3}{@{}l}{\textit{FGCI and Frobenius chain (\CatAL)}} \\[2pt]
\texttt{fgci\_\allowbreak unique\_\allowbreak at\_\allowbreak nc}
  & Universality.FrobeniusChain
  & $F(3)=G(3)=73$ unique at $N_c=3$ \\[1pt]
\texttt{frobenius\_\allowbreak chain\_\allowbreak two\_\allowbreak primes}
  & Universality.FrobeniusChain
  & Cascade $\{7,73\}$ complete; level-3 non-prime termination \\[1pt]
\texttt{b\_\allowbreak L2\_\allowbreak eq\_\allowbreak two\_\allowbreak Nc\_\allowbreak Z7}
  & Universality.FrobeniusChain
  & $b_{L2}=2N_c|Z_7|=42$ \\[1pt]
\midrule
\multicolumn{3}{@{}l}{\textit{$\mathbb{Z}_7$ dark-baryon correction and $D_{\rm top}$ (\CatAL, Lean-certified)}} \\[2pt]
\texttt{z7\_\allowbreak dark\_\allowbreak baryon\_\allowbreak correction\_\allowbreak identity}
  & Universality.GUTStructure
  & $q_{\rm dark}/(|Z_7|-1)=1/N_c$ \\[1pt]
\texttt{z7\_\allowbreak topological\_\allowbreak dilution\_\allowbreak formula\_\allowbreak rational}
  & Universality.GUTStructure
  & $2/6=1/3=1/N_c$ \\[1pt]
\texttt{z7\_\allowbreak star\_\allowbreak transitivity\_\allowbreak under\_\allowbreak addition}
  & Universality.GUTStructure
  & $\mathbb{Z}_7$ acts transitively on $\mathbb{Z}_7^*=\{1,\ldots,6\}$ \\[1pt]
\texttt{z7\_\allowbreak symmetry\_\allowbreak forces\_\allowbreak equal\_\allowbreak sector\_\allowbreak action}
  & Universality.GUTStructure
  & $S_{\rm per}=q_{\rm dark}/|\mathbb{Z}_7^*|=2/6=1/3=1/N_c$ \\[1pt]
\texttt{d\_\allowbreak top\_\allowbreak derivation\_\allowbreak chain\_\allowbreak catal}
  & Universality.GUTStructure
  & $D_{\rm top}=\exp(-1/N_c)$ from BPS $S_0/T=1$, $q_{\rm dark}=2$, $|\mathbb{Z}_7^*|=6$, transitivity \\[1pt]
\midrule
\multicolumn{3}{@{}l}{\textit{GH convergence and SU(2)$_L$ doublet Hilbert space (\CatAL)}} \\[2pt]
\texttt{vacuum\_\allowbreak cmca\_\allowbreak gh\_\allowbreak converges\_\allowbreak to\_\allowbreak flat\_\allowbreak space}
  & Spacetime.\allowbreak VacuumGH\allowbreak Convergence
  & Vacuum CMCA spatial graph $\to$ flat $[0,1]^3$ in GH; bound $d_{\rm GH}\leq 1/L$ \\[1pt]
\texttt{finGrid\_\allowbreak family\_\allowbreak totally\_\allowbreak bounded}
  & Spacetime.\allowbreak MatterGH\allowbreak Precompactness
  & FinGrid GH family totally bounded (pre-compact) \\[1pt]
\texttt{single\_\allowbreak kink\_\allowbreak gh\_\allowbreak converges\_\allowbreak to\_\allowbreak flat}
  & Spacetime.\allowbreak MatterGH\allowbreak Precompactness
  & Isolated kink (width $W$): $d_{\rm GH}\leq (W/2{+}1)/L\to 0$ \\[1pt]
\texttt{su2l\_\allowbreak doublet\_\allowbreak hilbert\_\allowbreak certified}
  & Substrate.\allowbreak SU2LDoublet\allowbreak Hilbert
  & $(\nu_L,e_L)$: $T_3=\pm\tfrac{1}{2}$, $W^\pm$, $\mathfrak{su}(2)$ algebra closed \\
\bottomrule
\end{tabular}
\caption{Frontier-closure representative theorems: UCL nine-coefficient master
certification, PMNS orbit-ratio closure, SRRG Higgs quartic, Pöschl--Teller and sech
integrals, Yukawa vertex (\CatAD), FGCI at $N_c=3$, $\mathbb{Z}_7$ dark-baryon
identities including $D_{\rm top}=\exp(-1/N_c)$ (\CatAL, Lean-certified),
and GH convergence / SU(2)$_L$ doublet Hilbert space (\CatAL).}
\label{tab:frontier-closures}
\end{table}

\paragraph{UCL master certification.}
\cref{subsec:ucl-unconditional} documents the unconditional closure modules;
\texttt{elegant\_kernel\_full\_certification} (\cref{tab:frontier-closures}) packages all
nine Elegant Kernel coefficients with their derivation sources in a single
\CatAL\ bundle, including the Quarter-Lock identity
$k_M = k_{\rm gen2} + k_{L^2}/4 = -\varphi/2 + 7/2048$.

\paragraph{PMNS and neutrino sector.}
The \texttt{MassRelations.NeutrinoSector} module certifies LH
FN charge neutrality ($q_L=0$), the Type-I seesaw algebra, rank-1 Dirac Yukawa
from positive FN charges, and the orbit-ratio PMNS sines and CP phase listed in
\cref{tab:frontier-closures}.

\paragraph{QFT fluctuation spectrum.}
\cref{sec:qft-modules} is extended by \texttt{Substrate.PhiMDLFluctuationSpectrum}:
the fluctuation potential is a Pöschl--Teller well, the sech and sech$^3$
integrals are Mathlib-closed, and the three-kink Yukawa overlap feeds the
\texttt{Gravity.YukawaOverlapExponent} DPP tape-counting derivation of
$\alpha=N_c-1=2$.
Finite-$r$ overlap lower bounds are certified in
\texttt{Substrate.SechOverlapIntegralBounds} (main module: zero \texttt{sorry},
zero custom axioms) with supporting \CatAL\ rational certifications in
\texttt{SechOverlapIntegralBounds\_cosh} and \texttt{\_r5bins}; the mesh-to-integral
bridge is disclosed as two \CatA\ axioms in \texttt{SechOverlapIntegralBounds\_bridge}
(see \cref{subsec:phimdl-fluctuation-083c}).
The leptogenesis suppression \lstinline|leptogenesis_kink_overlap_catad| is
\CatAD\ (\CatAL\ conditional on those bridge axioms).

\paragraph{$D_{\rm top}$ topological dilution (\CatAL, Lean-certified).}
The dark-matter relic density uses topological dilution
$D_{\rm top}=\exp(-1/N_c)$.
Machine-certified in Lean~4 with zero \texttt{sorry}, the derivation chain
\texttt{d\_top\_derivation\_chain\_catal} assembles BPS kink action $S_0/T=1$,
dark baryon charge $q_{\rm dark}=2$, sector count $|\mathbb{Z}_7^*|=6$, and
equal sector distribution from $\mathbb{Z}_7$ group transitivity
(\texttt{z7\_star\_transitivity\_under\_addition},
\texttt{z7\_symmetry\_forces\_equal\_sector\_action}).
The equal-sector-distribution step follows from $\mathbb{Z}_7$ group transitivity
alone --- no dilute instanton gas approximation is required.

% ═══════════════════════════════════════════════════════════════
\subsection{New algebraic certificates}\label{subsec:new-algebraic-certificates}
% ═══════════════════════════════════════════════════════════════

These certificates span GTE polynomial structure, the gauge-coupling sector,
cosmological constant structure, and the $N_{\mathrm{gen}}$
bracket-orientation theorem.
The first group covers \textbf{69} net-new zero-\texttt{sorry} theorems across twelve
modules, plus one module with partial certifications awaiting kernel-budget
resolution: the diagonal fixed-point structure of the GTE polynomial,
the Eisenstein and cyclotomic arithmetic of the GTE integer constants, the full
dynamical-zeta classification of the seven-ring orbit, the $\mathbb{Z}_7$
vacuum-selection mechanism with its gauge-coupling hierarchy, the algebraic
cores of the kink-sector physical observables, and the biquadratic-compositum
unification of the two master rings.
A second group --- the spin-7 wall spectroscopy and spectral-amplitude package,
the CMCA physical-point dictionary with the Tape Saturation Theorem, the
variational carrier pricing chain, the direct-interpolation lift of the GTE
polynomial from UGP orbit data, the parity-projection forcing batteries, and
the GTE register-machine trajectory containment --- adds over
\textbf{130} further zero-\texttt{sorry} theorems across ten modules and is
documented in the closing paragraphs of this subsection.

\paragraph{Golden quadratic and diagonal fixed points
  (\texttt{Polynomial/\allowbreak GoldenQuadratic}).}
Nine \CatAL\ theorems certify the diagonal self-consistency structure
$p(x,x,x)-x = -x(x^2+x-1)$ over any commutative ring
(\texttt{gte\_diagonal\_quadratic\_factorization}),
the rootlessness of $m(x)=x^2+x-1$ in $\mathbb{F}_7$ via the
$N_{\mathrm{fam}}=5$ quadratic-non-residue condition
(\texttt{disc5\_dichotomy\_gf7}),
the SRRG--floor duality bundle unifying $1/\varphi$ (archimedean fiber) with
the Rule~110 binary floor (\texttt{golden\_floor\_duality\_bundle}),
7-adic inertness through all $7^k$
(\texttt{master\_quadratic\_no\_root\_mod\_seven\_pow}),
the all-prime dichotomy via quadratic reciprocity
(\texttt{master\_quadratic\_split\_iff\_qr5}),
singleton-invariance classification
(\texttt{golden\_singleton\_invariant\_iff\_diag\_fixed}),
the $q{=}5$ ramification deriving the $\mathbb{F}_5$ ballistic pathology
(\texttt{second\_floor\_iff\_ramified}),
Frobenius swap of the two $\mathbb{F}_{49}$ roots
(\texttt{gf49\_golden\_roots\_frobenius\_swap}),
and the Pisano period $\pi(7)=16$
(\texttt{pisano\_seven\_eq\_sixteen}).

\paragraph{Eisenstein arithmetic and $F_{21}$ residue field
  (\texttt{Polynomial/\allowbreak EisensteinIdentities}).}
Seven \CatAL\ theorems certify $c_H = \Phi_3(N_{\mathrm{gen}}) =
\Phi_6(N_{\mathrm{gen}}{+}1) = 13$
(\texttt{c\_H\_eq\_phi3\_ngen}),
the Eisenstein norm-product $|F_{21}|=\Phi_6(2)\Phi_6(3)$
(\texttt{f21\_order\_eisenstein\_norm\_product}),
the residue-field isomorphism $F_{21}\cong(\mathbb{Z}[\omega]/(3{+}\omega))^+
\rtimes\mu_3$ via an explicit \texttt{decide}-verified group model
(\texttt{f21\_eisenstein\_residue\_model}),
$p$-torus semi-invariance
(\texttt{poly\_p\_torus\_equivariance}),
the $\Phi_6(q)$-count orbit decomposition $|V(p)\cap\mathbb{F}_7^3|=43$
with unique ether point (\texttt{poly\_p\_variety\_orbit\_decomposition\_gf7}),
and the four-identity $\Phi_6$ ladder web including the cyclotomic uniqueness
$\Phi_3(n)=(n^3{-}1)/2\iff n=3$ at $N_{\mathrm{gen}}=3$
(\texttt{phi6\_identity\_bundle}, \texttt{cH\_phi3\_unique\_at\_ngen}).

\paragraph{Dynamical zeta and period-475 orbit
  (\texttt{Polynomial/\allowbreak DynamicalZeta}).}
Nineteen \CatAL\ theorems certify the vacuum-uniqueness chain
$\mathrm{Fix}(T_n)=\{0^n\}$ for all $n\ge 1$ via local factorisation, the
golden Möbius 8-cycle on $\mathbb{P}^1(\mathbb{F}_7)$, and pole-within-6
refutation (\texttt{vacuum\_unique\_temporal\_fixed\_point\_ring} together with
the supporting \texttt{fixed\_point\_local\_factorization},
\texttt{golden\_moebius\_single\_eight\_cycle\_gf7},
\texttt{fibonacci\_matrix\_order\_sixteen\_gf7}, and related theorems);
the prime-ring shift-equivariance dichotomy
(\texttt{prime\_ring\_cycle\_dichotomy\_bundle});
the period-475 spectral facts $T^{95}=\sigma^3$ exact, return-map order~19,
zero per-cell mean, and $\sigma$-invariant gauge-observable period~95
(\texttt{t95\_eq\_sigma3\_on\_period475\_cycle},
\texttt{period475\_drift\_cancelled\_return\_order\_nineteen},
\texttt{period475\_gauge\_observable\_e1\_period\_95} and companions).
Five named results carry \textbf{PARTIAL} status pending kernel-budget
resolution: the de~Bruijn trace identity, spatial zeta, full $e_{2..5}$ gauge
observables, and linearisation charpoly fold.

\paragraph{$\mathbb{Z}_7$ vacuum selection and gauge-coupling hierarchy
  (\texttt{Physics/\allowbreak ZSevenVacuumSelection},
  \texttt{Universality/\allowbreak SylowIndexCouplingHierarchy}).}
Fourteen \CatAL\ theorems certify the vacuum-selection mechanism and
coupling-hierarchy arithmetic.
\texttt{ZSevenVacuumSelection} formalises: $V_{\mathrm{coupling}}$
$\mathbb{Z}_7$-shift-breaking (\texttt{vcoupling\_breaks\_z7\_shift}),
strict $\chi$-kinetic inequivalence across all seven vacua
(\texttt{z7\_vacua\_chi\_kinetic\_inequivalent}),
unique bias minimum at $k{=}0$ (\texttt{wall\_bias\_minimum\_unique}),
ground-state $\mathbb{Z}_3$-orbit transversal
(\texttt{gs\_set\_is\_z3\_orbit\_transversal}),
BPS-window discriminator (\texttt{vcoup\_periodic\_profile\_fails\_bps\_window}),
compact-completion minimum at $k{=}0$
(\texttt{compact\_completion\_minimum\_at\_k0}),
inward Coleman--Weinberg direction (\texttt{scalar\_cw\_step\_inward}),
seam-discontinuity witness, and boundary-sensitivity positivity.
\texttt{SylowIndexCouplingHierarchy} certifies the Sylow-family
neighbor-null establishing $e^2=7/2$ as uniquely closest
(\texttt{sylow\_family\_pdg\_match\_unique\_closest}) and the thermal
leading-order dominance $3e^2>g^2$
(\texttt{thermal\_leading\_inward\_dominance}).

\paragraph{Kink physics algebraic cores
  (\texttt{Physics/\allowbreak KinkVacuumPolarization},
  \texttt{KinkFormFactor}, \texttt{BurnsideCosetCharges},
  \texttt{Universality/\allowbreak HeatKernelContactTerms},
  \texttt{LambdaGTEThreshold}).}
Eleven \CatAL\ theorems certify the arithmetic and inequality skeletons of
five kink-sector physical observables.
\texttt{KinkVacuumPolarization}: Cartan-weight $t_{\mathrm{kink}}=3$ computation,
continuity identity $\hat{b}=-4=7-11$ linking to the $\mathbb{Z}_7$ order,
rational-log constant $c^{S^1,\mathrm{tree}}=8\ln(8/7)$, and positivity
inequality $m_\phi<e^{\gamma/2}\Lambda_{\mathrm{GTE}}$
(\texttt{kink\_vacuum\_polarization\_algebraic\_core} bundle).
\texttt{KinkFormFactor}: sech-profile moment bundle including the $r_{\mathrm{top}}/r_{\mathrm{born}}=\sqrt{3}$ ratio and BA-SHAPE autocorrelation, plus the
dissolution Born floor $b\le 2.51/2.53$ (tree/pole readings)
(\texttt{sech\_form\_factor\_moment\_bundle}, \texttt{kink\_dissolution\_born\_floor}).
\texttt{BurnsideCosetCharges}: coset-charge spectrum $t_V=3$,
$c_{\mathrm{coset}}=-1$ rational root-system skeleton.
\texttt{HeatKernelContactTerms}: contact-term difference $(N^2{-}3)/(24N)=1/12$
at $N=3$, $\mathfrak{su}(N)$ trace and Killing identities.
\texttt{LambdaGTEThreshold}: $\Lambda_{\mathrm{GTE}}=(8/7)m_\tau$, multiplier
minimality (7 is the least $\mathbb{Z}_7$-neutral chain length), $\mathbb{Z}_7$
coset residues $=\{\pm1,\pm2,\pm3\}$, and slope-continuity $7=11-4$.
One module carries \textbf{PARTIAL} status: \texttt{KinkPoleMassSpectralCore}
certifies the Pöschl--Teller autocorrelation at $s{=}0$, Levinson identity,
spectral sum rule, dim-reg channel value, and $\Gamma(-3/2)=4\sqrt{\pi}/3$
as definitional values; full Mathlib integral proofs are deferred.

\paragraph{Ground-space rigidity for arbitrary ring length
  (\texttt{Polynomial/\allowbreak SpinSevenGroundSpace}).}
Eight \CatAL\ theorems (zero sorry, zero custom axioms) certify the spin-7
ring ground-space rigidity theorem for \emph{every} ring length $n \ge 3$:
the cyclic zero-energy configurations of the spin-7 chain --- assignments
$(s_0,\dots,s_{n-1}) \in \mathbb{F}_7^n$ with $p(s_{i-1},s_i,s_{i+1}) = 0$
at every site --- are exactly the three uniform assemblies
$\{0^n, 1^n, 5^n\}$
(\texttt{gte\_ring\_ground\_states\_uniform\_general}).
The zero-energy condition $p=0$ is distinct from the temporal fixed-point
condition $p = s_i$ classified in \texttt{DynamicalZeta}; the two conditions
induce different pair digraphs and different rigidity sets
($\{0^n,1^n,5^n\}$ versus the vacuum-only $\{0^n\}$).
The proof is structural rather than per-length: a zero-energy window
determines its right cell from the left pair, so zero-energy triples define
a deterministic successor function on the $43$ active pair states of
$\mathbb{F}_7\times\mathbb{F}_7$; \texttt{native\_decide} certifies that the
only directed cycles of this pair digraph are the three uniform self-loops
$(0,0)$, $(1,1)$, $(5,5)$
(\texttt{zero\_energy\_active\_pair\_only\_uniform\_cycles}).
A cyclic length-$n$ configuration induces a closed walk of length $n$; the
minimal-period bound $\mathrm{minimalPeriod} \le |\mathbb{F}_7^2| = 49$
pigeonholes the return into the certified window, forcing one of the three
uniform loops, and any zero cell propagates around the ring to the vacuum
$0^n$ (\texttt{zero\_energy\_any\_zero\_forces\_all\_zero}).
The converse --- each uniform ground assembly has zero energy at every
site --- and the per-length corollaries for $3 \le n \le 7$ are packaged in
\texttt{gte\_ring\_ground\_states\_uniform\_bundle} and
\texttt{gte\_ring\_ground\_states\_uniform}, both delegating to the general
theorem.

\paragraph{Retrospective closures: biquadratic compositum and AGL chiral
  $\mathbb{Z}_2$
  (\texttt{Polynomial/\allowbreak BiquadraticCompositum},
   \texttt{Polynomial/\allowbreak AGL17ChiralZ2}).}
Two additional modules certify synthesis results unifying the two master
rings.
\texttt{BiquadraticCompositum} (15~\CatAL): establishes
$\mathrm{cond}(K)=15=N_{\mathrm{gen}}\cdot N_{\mathrm{fam}}$ for
$K=\mathbb{Q}(\sqrt{-3},\sqrt{5})$ (\texttt{biquadratic\_conductor\_identity})
and the Artin-class characterisation --- $q\equiv 7,13\pmod{15}$ iff $q$
splits in $\mathbb{Z}[\omega]$ (color-sector condition) and is inert in
$\mathbb{Z}[\varphi]$ (floor-protection condition) --- with minimal class
prime $7$ and second class prime $c_H=13$ certified by \texttt{decide}.
\texttt{AGL17ChiralZ2} (7~\CatAL): certifies $|{\rm AGL}(1,7)|=42$
(\texttt{agl17\_order}), the spatial reflection $s=(6,0)$ of order~2 that
swaps Rule~110 and Rule~124 on all binary inputs with zero mismatches
(\texttt{reflection\_swaps\_rules\_verified}), the commuting $\mathbb{Z}_3$
color action, and the free-orbit cardinality $42=|{\rm AGL}(1,7)|$ as the
arithmetic mechanism for the unique torus partition of $V(p)$.

Cross-certifications from companion libraries are documented in
\cref{subsec:cross-repo-certs}: the \texttt{nems-lean} and
\texttt{transputation-lean} diagonal completeness, no-convergence-modulus,
and many-one degree theorems (\CatAL) are incorporated there.

\paragraph{CC one-jump residual and $N_{\mathrm{gen}}$ bracket-orientation
  (\texttt{Physics/\allowbreak CCOneJumpResidual},
   \texttt{Universality/\allowbreak NgenBracketOrientation}).}
Two further modules certify the cosmological-constant residual structure
and the generation-count orientation theorem.
\texttt{CCOneJumpResidual} (6~\CatAL\ + 1~\textbf{PARTIAL}):
certifies the census annotation of the realised-ledger residual
$D_{\mathrm{res}} := (\ln 2/\pi)\cdot L_{\mathrm{PSC}}$ as the left-c.e.\
computable upper bracket endpoint
(\texttt{d\_res\_census\_annotation}),
the strict bracket non-emptiness
$\Omega_{\mathrm{holo}} < \Omega_{\mathrm{census}}$ with the Planck~2018
central value strictly in the interior
(\texttt{omega\_lambda\_bracket\_strict},
 \texttt{bracket\_strict\_interiority}),
the no-computable-modulus theorem via reduction to the RT non-computability
core
(\texttt{no\_computable\_gte\_rt\_convergence\_modulus}),
and the falsifiability horizon
$\sigma^* \in (2.9{\times}10^{-4},\, 4.3{\times}10^{-4})$ requiring
factor-five improvement over current Planck precision
(\texttt{sigma\_star\_falsifiability\_horizon}).
The halting-weighted residual form
(\texttt{d\_res\_halting\_ledger\_factorization\_core})
carries \textbf{PARTIAL} status: a finite \texttt{toyLedger} witness is
certified; the full infinite Form Theorem and $\deg_T = 0'$
identification are deferred.
\texttt{NgenBracketOrientation} (5~\CatAL\ + 2~\CatAL~$|$~H1):
the orientation-flip theorem certifies that the admissible ledger interval
$[\mathrm{floor}(N),\, \mathrm{census}(N)]$ is empty for all $N \ge 4$
and non-empty for $N \le 3$
(\texttt{ngen\_bracket\_orientation\_flip},
 \texttt{ngen\_bracket\_orientation\_excluded\_from\_four});
the GTE-atom inequality $14\ln(2000/3) > 9\pi^2$ provides the bracket
non-emptiness corollary in the atom form
$2|\mathbb{Z}_7|\ln(D^2 N_{\mathrm{fam}}^3 / N_{\mathrm{gen}}) >
N_{\mathrm{gen}}^2\pi^2$ at $D{=}4$, $N_{\mathrm{fam}}{=}5$,
$N_{\mathrm{gen}}{=}3$
(\texttt{cc\_floor\_orientation\_atom\_inequality},
 \texttt{cc\_floor\_orientation\_gte\_atom\_form},
 \texttt{cc\_floor\_orientation\_bracket\_non\_empty});
and the PI-free pincer combines the PSC Layer~I lower bound
$N_{\mathrm{gen}} \ge 3$ with the bracket-orientation upper bound
$N_{\mathrm{gen}} \le 3$ to force $N_{\mathrm{gen}} = 3$ without invoking
Presentation Invariance, conditional on named hypothesis H1
(\texttt{ngen\_pincer\_from\_layer1\_and\_orientation},
 \texttt{ngen\_pincer\_pi\_free\_bundle},
 \CatAL~$|$~H1).
The orientation hypothesis H1 is analytically discharged: the floor half
follows from the Floor Orientation Theorem (the realised ledger cannot
undercut the MDL-minimal carrier price) and the census half from capacity
bounds, so the pincer holds unconditionally within the record-ledger
premise bundle of the One-Jump Residual Theorem.

\paragraph{Spin-7 wall spectroscopy and spectral-amplitude package
  (\texttt{Polynomial/\allowbreak SpinSevenWallSpectroscopy},
   \texttt{SpinSevenSpectatorAmplitude},
   \texttt{SpinSevenGapAmplitude},
   \texttt{SpinSevenTransferPrimitivity}).}
Four modules (44~\CatAL\ theorems, zero sorry, zero custom axioms) certify
the interface thermodynamics of the spin-7 chain.
\texttt{SpinSevenWallSpectroscopy} (14 theorems) establishes the directed
minimal-interface energies of the 49-node pair digraph --- edge
$(a,b)\to(b,c)$ weighted by the local triple energy $p(a,b,c)$ --- by
bounded Bellman--Ford search closed with \texttt{native\_decide}:
$E_w(1{\to}0)=1$, $E_w(0{\to}1)=2$, $E_w(5{\to}0)=2$,
$E_w(0{\to}5)=E_w(1{\to}5)=E_w(5{\to}1)=4$, with the composite hub identity
$E_w(5{\to}1)=E_w(5{\to}0)+E_w(0{\to}1)$ (sector~0 is the hub of the wall
algebra), the bump table $E_{\rm loop}(0,1,5)=(2,3,4)$, and the half-integer
gap exponent $(E_w(0{\to}1)+E_w(1{\to}0))/2 = 3/2$
(\texttt{spin7\_directed\_wall\_energies},
 \texttt{directed\_wall\_half\_integer\_gap}).
\texttt{SpinSevenSpectatorAmplitude} (10 theorems) certifies the spectator
mechanism behind the gap-law amplitude: the zero-energy adjacency matrix has
spectral radius exactly~1 --- its powers stabilise at $Z^{22}=Z^{23}$ with
$\mathrm{tr}(Z^k)=3$ for all $k\ge1$, the three uniform self-loops being the
only recurrent structure --- and the three-level cluster perturbation
eigensystem has ordered gaps $\{T, 2T\}$
(\texttt{zero\_energy\_closed\_walk\_count\_three},
 \texttt{cluster\_gap\_pair}, \texttt{spin7\_spectator\_amplitude}).
\texttt{SpinSevenGapAmplitude} (13 theorems) computes the through-walk counts
between ground loops in the weight-truncation semiring
$\mathbb{N}[t]/(t^5)$ --- $c_{10}=c_{01}=b_0=b_1=1$, $b_5=2$ --- with a
frontier-death certificate making the counts total over all walk lengths,
yielding $A^2 = c_{10}c_{01} = 1$ and the leading correction $b_0/2 = 1/2$
(\texttt{spin7\_gap\_amplitude\_certificate}, \texttt{wall\_frontier\_death},
 \texttt{gap\_amplitude\_sq\_unit}).
\texttt{SpinSevenTransferPrimitivity} (7 theorems) certifies the complete
Perron--Frobenius hypothesis package for the thermal transfer matrix,
uniform in $\beta:\mathbb{R}$: entrywise non-negativity, primitivity with
explicit power $k=2$, irreducibility, and positive uniform diagonal
(\texttt{spin7\_transfer\_pf\_hypotheses}); the leading-eigenvalue
simplicity conclusion awaits a Perron--Frobenius eigenvalue theorem in
Mathlib.

\paragraph{CMCA physical point: dictionary, tape saturation, blocking ratio,
  and boundary equivalence (\texttt{Physics/\allowbreak CMCAPhysicalPoint}).}
The module certifies the arithmetic web of the CMCA physical point.
Given the seven-kink threshold $\Lambda = 7M$, the SCC kink mass
$M = (8/49)\,m_\varphi$, and the spacing identification $a\cdot\Lambda = 1$
carried by the named premise structure
\texttt{MDLSaturationSpacingHypothesis}, the dictionary follows:
$aM = 1/7$, $a m_\varphi = 7/8$, and the correlation signature
$\xi^* = 1/(aM) = 7 = |\mathbb{Z}_7|$
(\texttt{cmca\_physical\_point\_dictionary}, \CatAL~$|$~H --- conditional on
the named hypothesis, never bare).
The Tape Saturation Theorem then upgrades the spacing hypothesis from a bare
assumption to a theorem-with-named-premise: under the
\texttt{ComptonSupportCriterion} (the $\kappa = 1$ hosting boundary: a tape
of spacing $a$ faithfully hosts masses $m$ with $m\cdot a \le \hbar c$)
together with strict MDL pricing
(\texttt{StrictTapePricingHypothesis}) and extremization at the hosting
boundary, the optimal tape satisfies $a\cdot\Lambda = 1$
(\texttt{tape\_saturation\_theorem},
 \texttt{compton\_support\_derives\_mdl\_saturation},
 \texttt{tape\_saturation\_physical\_point\_dictionary}; all
\CatAD~$|$~CSC --- conditional on the named Compton-support premise).
The Boundary Equivalence Theorem establishes that this premise is minimal:
the Compton-support criterion, register-window readability
(\texttt{RegisterWindowReadability} --- the $\mathbb{Z}_7$ winding-sector
label, entropy $\ln 7$, recoverable within one correlation window), and the
$\kappa = 1$ hosting boundary are mutually entailed at saturation, and the
seven winding sectors biject onto the seven register values
(\texttt{hosting\_boundary\_csc\_equivalence},
 \texttt{saturation\_alphabet\_bijection}).
Finally, the blocking-ratio web binds the two tape readings unconditionally:
$M_{\rm Pl}/\Lambda_{\rm GTE} = 3^{10}\,7^{18}/2^4$ with the monomial
identity $3^{10}7^{18} = 7^8\,21^{10}$ and the fine-end kink ratio
$M_{\rm Pl}/M_{\rm kink} = 21^{10}\,7^9/2^4$
(\texttt{planck\_eft\_blocking\_ratio}, \CatAL).

\paragraph{Variational carrier pricing
  (\texttt{Gravity/\allowbreak TemporalVoxelCC} extension).}
Three further \CatAL\ theorems extend the temporal-voxel module
(\cref{subsec:temporal-voxel-cc}) with the arithmetic spine of the
variational carrier analysis: the three-form coefficient chain
$N\tau\,(2C_{\rm Gorard}/N) = 2C_{\rm Gorard}\tau = N^2/(D^2|\mathbb{Z}_7|)
= 9/112$
(\texttt{pmdl\_carrier\_coefficient\_identities}), the pricing-gap closed
forms for the half-quantum continuum and lattice pricings, and the bracket
exclusion of the half-quantum pricing ($2\pi^2$ exceeds the census endpoint;
\texttt{half\_quantum\_pricing\_excluded}), bundled in
\texttt{pmdl\_carrier\_pricing\_chain}.

\paragraph{Direct-interpolation lift and chirality census
  (\texttt{Universality/\allowbreak TriangleLiftTheorem},
   \texttt{TriangleLiftStructural};
   \texttt{FrobeniusChain} extension).}
Two modules (21~\CatAL\ theorems, zero sorry, zero custom axioms) certify
that the GTE polynomial $p$ is recoverable from UGP orbit data alone, by two
independent routes.
\texttt{gte\_orbit\_parity\_provenance} closes the provenance link: the
total parities of the fifteen canonical cascade triples reproduce the
certified generation vectors of \texttt{CUP4TotalParity}.
The census route
(\texttt{ugp\_orbit\_interpolation\_lift},
 \texttt{orbit\_interpolation\_vandermonde\_full\_rank}) is an exhaustive
$7^8$ \texttt{native\_decide} census over all multilinear $\mathrm{GF}(7)$
rules: the parity-orbit ring evaluations alone leave seven survivors, and
adding vacuum transparency leaves exactly one --- $p = C + R - CR - LCR$.
The structural route
(\texttt{multilinear\_binary\_determination},
 \texttt{orbit\_vt\_forces\_interpolant}) proves the same uniqueness with no
enumeration, by explicit M\"obius inversion of the binary Vandermonde
system: the orbit covers seven of the eight binary neighbourhoods and vacuum
transparency supplies the eighth.
The binary restriction of the interpolant is Rule~110
(\texttt{interpolation\_lift\_binary\_corollary}) --- the anchor direction
is inverted: Rule~110 becomes an output of the orbit data rather than an
input.
The MDL sparsity floor holds in its strongest form: \emph{any}
canonical-form $\mathrm{GF}(7)$ polynomial (per-variable degree $\le 6$)
whose binary restriction is Rule~110 flattens exactly to $p$ and has at
least four monomials, so multilinearity is MDL-forced rather than declared
(\texttt{gf7\_rule110\_sparsity\_floor},
 \texttt{rule110\_lift\_sparsity\_floor}).
The chirality census (\texttt{orbit\_chirality\_census}) runs the
construction over all $120$ orderings of the five families: exactly $20$
admit a vacuum-transparent orbit rule, the survivor union is exactly
$\{110, 124\}$, and ordering reversal bijects the two $10$-sets --- the
ordering convention is the chirality gauge, and the unordered orbit data
force the CMCA chiral pair.
The \texttt{FrobeniusChain} extension adds the bounded consilience
certificate \texttt{delta\_qmin\_coincidence\_at\_three}: for
$n \in \{2,\dots,30\}$ the ridge offset $\delta(n) = n + (n^2-1)/2$ equals
the minimal admissible prime $q$ iff $n = 3$ (an over-determination of the
modulus, not an independent forcing route).

\paragraph{Parity-projection forcing batteries
  (\texttt{Universality/\allowbreak ParityProjectionBattery},
   \texttt{ParityProjectionForcing}).}
The forcing-scope theorems certify that the parity projection is forced over
the full admissible reduction space.
The additive battery runs all $777$ nonzero weighted forms
$(\alpha,\beta,\gamma) \bmod m$ for $2 \le m \le 7$ --- every additive
homomorphism $\mathbb{Z}^3 \to \mathbb{Z}_m$ --- against the canonical orbit
constraints: the coherently-forcing forms are exactly the parity-factoring
ones, with forced rules $g_1 = p$ and the unit conjugates $g_2, g_3$
(\texttt{parity\_projection\_additive\_forcing}; per-modulus
\texttt{native\_decide} censuses with aggregate totals $277/490/5/5$).
The recoding battery runs all $7^5 = 16{,}807$ vacuum-normalised
$\mathrm{GF}(7)$ assignments to the five mod-2 residue patterns: the
coherently-forcing assignments are exactly $t\cdot(\text{total parity})$ for
a unit $t$, and the product-parity family is excluded by the Rule-106
displaced-vacuum classification
(\texttt{parity\_projection\_mod2\_recoding\_forcing}; seven coordinate
chunks of $2{,}401$ assignments each, aggregate totals
$7392/8820/343/252$).
Together the two censuses prove the maximality of the forcing scope
constructively: no forcing theorem can exist over arbitrary recodings.

\paragraph{GTE register-machine trajectory containment
  (\texttt{Universality/\allowbreak GTECompilation} extension).}
The compilation module (\cref{subsec:gte-compilation}) is extended with the
finite-horizon trajectory-containment package: a register-machine evolution
over the UWCA substrate reproduces the GTE arithmetic trace exactly, for
every horizon $H$
(\texttt{gte\_trajectory\_is\_uwca\_trajectory}:
 \texttt{gte\_uwca\_run}~$n$~$H = $ \texttt{gteAfterSteps}~$n$~$H$).
The two-tile $\sigma_{\rm GTE}$ program --- the odd-step tile of
\cref{thm:gte-compilation} paired with an even-step tile carrying the
Fibonacci lift on $b$ and the Mersenne-boundary $c$-rule --- reproduces the
first two GTE macro-steps from the Lepton seed, matching the reference
evolution \texttt{gteAfterSteps}
(\texttt{gte\_two\_tile\_compilation\_theorem}), and the containment
corollary \texttt{gte\_trajectory\_two\_tile\_containment} packages the
direction of the statement: GTE trajectories are contained in UWCA
trajectories --- the UWCA is the general machine and GTE one lawful
program --- while the converse is false and is not claimed.
...
% ═══════════════════════════════════════════════════════════════
\section{Archival Identity and Structural Classification Modules}\label{sec:archival-identity}
% ═══════════════════════════════════════════════════════════════

Nine further modules certify arithmetic identities and structural classification
results that deepen the algebraic foundations of the GTE framework.

\subsection{\texttt{Universality/AlphaEMArchivalIdentities}}\label{subsec:alphaem-archival}

Three \CatAL\ theorems (zero \texttt{sorry}, zero custom axioms) certify the
$\varphi(7)$ structural identity for the lepton seed and fine-structure constant:
\begin{itemize}[nosep]
\item \lstinline|b1_totient7_ngen_identity|: $b_1 = 2^{\mathrm{Nat.totient}\,7} + N_{\rm gen}^2 = 73$.
  This provides a third independent arithmetic route to $b_1$, complementing the
  Frobenius-chain and cascade routes already in the library.
\item \lstinline|primorial7_alpha_b1_identity|: $b_1 + 1/\alpha_{\rm em} = 73 + 137 = 210 = 2\cdot3\cdot5\cdot7$.
\item \lstinline|alpha_inv_routes_equivalent|: $2\times73 - 9 = 2^7 + 9 = 137$
  (equivalence of the two known routes to $1/\alpha_{\rm em}$).
\end{itemize}
Module also certifies the URC scale identity: $200 = 2^3\times5^2$ with structural
identification $0.015 = N_{\rm gen}/(2^{N_{\rm gen}}\times N_{\rm fam}^{N_{\rm gen}-1})$
(\lstinline|urc_scale_200_gte_identity|, \CatAL).

\subsection{\texttt{Universality/Phi7UnificationTheorem}}\label{subsec:phi7-unification}

Five \CatAL\ theorems prove the $\varphi(7)$ structural unification:
\begin{itemize}[nosep]
\item \lstinline|ord7_2_eq_ngen|: $\mathrm{ord}_7(2) = N_{\rm gen} = 3$.
  The element $2 \in \mathrm{GF}(7)^*$ generates the Sylow-3 color subgroup and has order~3.
\item \lstinline|phi7_eq_two_ngen|: $\varphi(7) = 2 N_{\rm gen} = 6$.
\item \lstinline|phi7_structural_unification_theorem|: bundles the corollaries
  $b_1 = 2^{\varphi(7)}+N_{\rm gen}^2$, $b_2 = \varphi(7)\times7$, and
  $1/\alpha_{\rm em} = 2^{\varphi(7)+1}+N_{\rm gen}^2$ as consequences of
  $|\mathrm{GF}(7)^*| = 2 N_{\rm gen}$.
\item \lstinline|phi7_uniqueness_among_primes|: $\varphi(q) = 6$ holds only for
  $q = 7$ among all primes up to 20 (null test).
\end{itemize}

\subsection{\texttt{GTE/MobiusTripleClassification}}\label{subsec:mobius-triple}

Fourteen \CatAL\ theorems (zero \texttt{sorry}) classify the canonical GTE fermion triples
via Möbius arithmetic and divisibility:
\begin{itemize}[nosep]
\item \lstinline|electron_triple_mobius_unique_lepton|: the electron triple $(1,73,823)$
  is the unique lepton triple with Möbius product $\mu(a)\mu(b)\mu(c) = +1$.
\item \lstinline|lepton_c_values_mod8_seven|: the three lepton $c$-values $\{823,1023,65535\}$
  satisfy $c \equiv 7 \pmod 8$.
\item \lstinline|electron_all_squarefree|, \lstinline|bottom_all_squarefree|,
  \lstinline|fermion_triples_all_squarefree_pair|: the electron $(1,73,823)$ and bottom
  $(5,8191,65535)$ are the unique pair among all nine canonical fermion triples with all
  three components squarefree.
\item \lstinline|b2_equals_phi7_times_7|: $b_2 = \varphi(7)\times7 = 42$.
\item \lstinline|b2_phi7_divides_ridge|: $b_2 \mid R_{10}$.
\item \lstinline|c1_prime|, \lstinline|c1_squarefree|: $c_1 = 823$ is prime (decidable).
\end{itemize}

\subsection{\texttt{GTE/PolynomialLipschitz}}\label{subsec:poly-lipschitz}

Three \CatAL\ theorems (zero \texttt{sorry}, exhaustive \lstinline|decide| over
$7^3 = 343$ inputs) certify the Lipschitz constant of the GTE update polynomial
$p(L,C,R) = C+R-CR-LCR$ over $\mathrm{GF}(7)$ under the symmetric distance metric:
the Lipschitz constant equals~3, and the bound is achieved (tight).
(\lstinline|gte_lipschitz_constant_eq_3|, \lstinline|gte_lipschitz_bound_tight|.)

\subsection{\texttt{QM/BilinearNeutrinoNoGo}}\label{subsec:bduo}

Three \CatAL\ theorems prove the Bilinear-$D$ Unitarity Obstruction (BDUO):
for any unitary $3\times3$ mixing matrix $U$, the matrix of bilinear interference
coefficients $A_{jk} = 2\,\mathrm{Re}\!\left(\sum_j U^*_{\alpha j} U_{\beta j}\right)$
has $\det A = 0$.
(\lstinline|unitary_row_inner_product|, \lstinline|bduo_row_sum_zero|,
\lstinline|bduo_interference_sum_zero|.)
This establishes a structural no-go theorem ruling out any bilinear-coherence neutrino
mass mechanism with PMNS-derived mixing coefficients.

\subsection{\texttt{QM/KahlerStateManifold}}\label{subsec:kahler}

Five theorems certify the Kähler geometric foundation for the Born rule:
\begin{itemize}[nosep]
\item \lstinline|trace_invariant_under_unitary| (\CatAL): unitary conjugation preserves trace.
\item \lstinline|unitary_preserves_density_matrix| (\CatAL): the density-matrix condition
  (Hermitian, positive semi-definite, unit trace) is preserved under unitary conjugation.
\item \lstinline|gte_state_space_kaehler_property| (\CatAD): the GTE coarse-grained state
  manifold $\{\rho_\theta\}$ carries a Kähler triple (Fisher metric, symplectic form,
  compatible complex structure); blocked by Fisher–Rao / Bures metric infrastructure
  in Mathlib.
\end{itemize}

\subsection{\texttt{Spacetime/EntanglementAreaLaw}}\label{subsec:entanglement-area}

Five theorems certify the entanglement area law and Jacobson GR route:
\begin{itemize}[nosep]
\item \lstinline|pure_state_bipartite_rank_one| (\CatAL): a normalised amplitude yields
  a rank-one projector with unit trace.
\item \lstinline|bekenstein_hawking_wald_crossref| (\CatAD): cross-reference to
  \lstinline|phimdl_wald_entropy_is_area_over_4G| in \texttt{WaldEntropy.lean}.
\item \lstinline|gte_entanglement_area_law_scaffold|: names the intended claim
  $S(D) = \eta\cdot\mathrm{Area}(\partial D)$, but its current Lean statement is a
  tautological identity ($x = x+0$) with no entanglement, area, or entropy
  content --- a documentation scaffold, not a \CatAD\ bridge with formal
  content.  Genuine formalization is blocked by CMCA causal-graph entropy
  infrastructure not yet in this repository; the \lstinline|_scaffold| suffix
  marks it as pending that infrastructure, not as an established result at
  any Cat level.
\end{itemize}

\subsection{\texttt{Spacetime/LorentzianCausalityNecessity}}\label{subsec:lorentzian-causality}

Six theorems certify the information-causal derivation of Lorentzian signature:
\begin{itemize}[nosep]
\item \lstinline|minkowski_not_euclidean|, \lstinline|minkowski_has_indefinite_signature|,
  \lstinline|minkowski_not_positive_definite| (\CatAL): structural facts about Minkowski metric.
\item \lstinline|minkowski_supports_causal_cone| (\CatAL): timelike ($\eta(v,v)<0$) and
  spacelike ($\eta(w,w)>0$) vectors coexist, establishing the causal cone sign structure.
\item \lstinline|lorentzian_sig_from_causal_propagation| (\CatAD): the full
  PSC $\Rightarrow$ Lorentzian chain; blocked by PDE classification in Mathlib.
\end{itemize}

\subsection{\texttt{Universality/IMTStructuralPair}}\label{subsec:imt-structural}

Three \CatAL\ theorems (zero \texttt{sorry}) certify the IMT engine constant structural pairing:
\begin{itemize}[nosep]
\item \lstinline|imt_gen1_phase_structural_pair|: $g_1 + 2 w_{\rm phase} = 2^{N_{\rm fam}} - \pi/2^{n_{\rm ridge}}$
  with $N_{\rm fam} = 5$, $n_{\rm ridge} = 10$; the Euler constant $e$ cancels exactly by \lstinline|ring|.
\item \lstinline|imt_structural_pair_gte_params|: parameterised version with explicit hypotheses.
\item \lstinline|imt_binding_weight_structural|: $w_{\rm bind} = -1/(4\times11) - 1/2^{11}$
  where $q_1 = 11 = \lfloor c_1/b_1\rfloor$ is the first-step cascade quotient.
\end{itemize}

\subsection{\texttt{Spacetime/SpacelikeCutAreaScaling} and \texttt{Spacetime/ThreeTapeCutAreaScaling}}\label{subsec:cut-area-scaling}

These two modules supply an intrinsic (metric-free) combinatorial cut measure for a
spacelike coordinate hyperplane, first as a proof of concept on the certified 3D causal
graph and then ported to the actual three-tape $\mathbb{Z}_7$ CMCA substrate. Neither
\lstinline|HolographicScaling.three_tape_state_card| (a state-space cardinality count,
$7^{3L}$ vs.\ $7^{L^3}$, with no notion of a specific cut) nor
\lstinline|Gravity.GorardRicciFlatVacuum.three_tape_causal_diamond_t4| (a continuum
causal-diamond \emph{volume} $T^4/4$, not a boundary \emph{area}) establishes a literal
codimension-2 cut area; these modules supply that missing object.

\paragraph{\texttt{SpacelikeCutAreaScaling} (causal-graph proof of concept).}
On the certified causal graph \lstinline|GTE.Spacetime.CausalGraph| (spatial lattice
$\mathrm{Fin}\,L \times \mathrm{Fin}\,L \times \mathrm{Fin}\,L$, adjacency
\lstinline|SpacelikeAdj|), the spacelike causal edges crossing the coordinate cut
$x = c$ at a fixed time $t$ are parametrized by their shared $(y,z)$ position
(\lstinline|cutEdgeAt|). Three \CatAL\ theorems (zero \texttt{sorry}) establish that
this parametrization is injective (\lstinline|cutEdgeAt_injective|), that every
parametrized pair is a genuine \lstinline|SpacelikeAdj| edge
(\lstinline|cutEdgeAt_is_spacelike_adj|), and --- the completeness half needed to make
the count exact rather than a lower bound --- that every \lstinline|SpacelikeAdj| pair
straddling the cut arises from this parametrization
(\lstinline|cutEdgeAt_characterizes_x_crossing|). The main theorem,
\lstinline|cut_crossing_finset_card_eq_L_sq| (\CatAL), concludes that the crossing-edge
\lstinline|Finset| has cardinality exactly $L^2$: genuine quadratic, area-like scaling,
built entirely from \lstinline|Fin|-coordinate adjacency, with no metric, no continuum
geometry, and no dependence on the CA update rule. This is a positive existence result
for a GTE-style discrete causal graph, not yet the literal three-tape construction of
the following module.

\paragraph{\texttt{ThreeTapeCutAreaScaling} (three-tape $\mathbb{Z}_7$ CMCA port).}
This module ports the same construction to the actual three-tape CMCA configuration
space $(\mathrm{Fin}\,L \to \mathbb{Z}_7) \times (\mathrm{Fin}\,L \to \mathbb{Z}_7)
\times (\mathrm{Fin}\,L \to \mathbb{Z}_7)$ used by
\lstinline|HolographicScaling.three_tape_state_card|. P45's gravitational-coupling
source-density formula $\rho(x) = p(w_x(x), w_y(x), w_z(x))/6$ is ambiguous, read
literally, about whether each tape function $w_j$ depends on all three spatial
coordinates or only its own axis's coordinate; the module resolves this by an explicit,
checked choice rather than an unstated assumption
(\lstinline|three_tape_config_card_confirms_reading_A|): only the one-coordinate-per-tape
reading reproduces the certified cardinality $7^{3L}$, so this is the reading adopted.
Under that reading, the emergent spatial lattice \lstinline|SpatialPoint L| is shown to
be \emph{definitionally} (\lstinline|rfl|) the spatial part of
\lstinline|CausalGraph.CausalNode|, and the corresponding adjacency relation
\lstinline|SpatialAdj| is shown to agree exactly with \lstinline|SpacelikeAdj| restricted
to a shared time (\lstinline|spatialPoint_eq_causalNode_spatial_part|,
\lstinline|spatialAdj_eq_spacelikeAdj_at_shared_time|) --- confirming the port is a
genuine specialization, not an independent redefinition that could disagree with the
proof of concept above. The main theorem,
\lstinline|three_tape_cut_crossing_card_eq_L_sq| (\CatAL, zero \texttt{sorry}, zero new
\texttt{axiom}), concludes that the number of spacelike edges crossing a coordinate cut
in the three-tape substrate's emergent 3D lattice equals $L^2$ exactly --- the same
intrinsic quadratic area-scaling law, now established for the CMCA architecture itself
rather than only the proof-of-concept causal graph.

\paragraph{Scope.} These results establish an intrinsic, non-metric, area-scaling
($L^2$) count of spacelike causal edges crossing a coordinate cut in the three-tape CMCA
substrate's emergent spatial lattice. They do \emph{not} establish that this count is the
boundary entropy of a genuine causal diamond with a horizon/null-congruence structure, and
they do not compute an entropy $S$ or slope $\eta$; both remain open. No physical length
scale, $\ell_{\rm Pl}$, or $G$ appears anywhere in either module.
...
strict zero-sorry policy throughout, and zero custom axioms on the
arithmetic/algebraic core proof path; the Turing-universality results are
conditional on a small number of explicitly named, standard classical axioms
(Minsky's counter-machine theorem, Cook's collision-analysis bridge lemmas),
each disclosed at the point of use.
...
observables (\lstinline|algebraic_lifting_theorem|, zero sorry);
...
An audit discharged several \texttt{VEVProof} axioms as proved theorems;
the verified global named-axiom total is 98 (see Table~\ref{tab:axiom-inventory}).

\medskip
\noindent\textbf{PSL(2,7) unification, golden fiber, and algebraic extensions}
(27 new modules):
\texttt{Polynomial/PSL27Unification} certifies $F_{21} \cong \mathrm{PSL}(2,7)$
as the automorphism group of the Fano plane and establishes the Borel-measurability
of the $F_{21}$ action (\texttt{Polynomial/F21SU2Bridge},
\texttt{Algebra/FanoRegularAction});
\texttt{Polynomial/GoldenFiberTaxonomy} and \texttt{Polynomial/GoldenQuadratic}
classify golden-fiber states at $q=7$ and certify golden-quadratic arithmetic;
\texttt{Algebra/EisensteinFunctor} formalizes the Eisenstein functor and
derives the $A_4$ structure from inert-2 ramification
(\texttt{MassRelations/EisensteinIdentities});
\texttt{Polynomial/GaussianFaceLemma} and \texttt{Polynomial/AdmissiblePrimes}
certify the admissible prime set and Gaussian face arithmetic;
\texttt{Classification/NgenUniqueness} proves $N_{\mathrm{gen}} = 3$ uniqueness;
\texttt{Gravity/LambdaAllOrderZero} certifies vanishing of all cosmological constant
contributions at the algebraic level;
\texttt{Gravity/ClassicalLambda} certifies that the classical cosmological constant
vanishes (\texttt{classical\_lambda\_zero}, \CatAL) via the Z\textsubscript{7} vacuum
energy $V(0)=0$;
\texttt{Substrate/VAQuantBundle}, \texttt{Substrate/ChiralCurrentL2}, and
\texttt{Substrate/CoherenceMeasureUniqueness} extend the substrate layer
with VA quantization, chiral currents, and coherence-measure uniqueness;
\texttt{Universality/SolovayCompleteness}, \texttt{Universality/BiImmunity}, and
\texttt{Universality/ComplexAmplitudeForced} certify Solovay completeness of
the UWCA, bi-immunity of the orbit, and forcing of complex amplitudes;
\texttt{Cosmology/CCBracketHurwitz}, \texttt{Foundations/CMCARecordFiltration},
\texttt{Foundations/CMCAThermodynamicBridge}, \texttt{Framework/PR5Independence},
and \texttt{Framework/PSCMeasureUniformity} establish the cosmological constant bracket,
CMCA filtration and thermodynamic bridge, PR-5 independence, and PSC measure uniformity.
All new modules carry zero \texttt{sorry} and zero custom axioms beyond the library's
documented structural axioms.

\medskip
\noindent\textbf{Octonion certificate cluster}
(10 new modules, all zero \texttt{sorry}, zero custom axioms):
\texttt{Algebra/OctonionShadowInterface} proves that the UGP color sector is
generated by the quadratic residues $\mathrm{QR}(7) = \{1,2,4\} \subset \mathbb{Z}_7$,
which coincide with the $(7,3,1)$ difference set that seeds the Cayley octonions;
it certifies that the $F_{21}$ automorphism action preserves the oriented Fano lines,
establishing $F_{21} \hookrightarrow G_2$ at the index level, and that the pencil
count $|\mathrm{QR}(7)| = 3 = N_c$ is not an independent input but an intrinsic
property of the design.
\texttt{Algebra/HurwitzCosetCertificate} replays the Todd--Coxeter coset
enumeration for the $(2,3,7;4)$ presentation, certifying $|\langle a,b \mid
a^2, b^3, (ab)^7, [a,b]^4\rangle| = 168$ by an explicit 168-coset permutation
representation verified by \texttt{native\_decide}.
\texttt{Algebra/G2StabilizerCertificate} constructs the QR(7)-octonion derivation
Lie algebra by exact integer arithmetic: 14 derivation witnesses satisfying the
Leibniz rule on all 64 basis pairs, independence confirmed by a
$14 \times 14$ Bareiss determinant $\neq 0$, exact dimension verified by a
$50 \times 50$ rank minor, bracket closure for all 91 commutators, and
negative-definite Killing form by Sylvester's criterion; the same procedure
certifies the apex stabilizer as 8-dimensional.
The Cartan--Killing classification (cited, not formalized) then identifies the
derivation algebra as $\mathfrak{g}_2$ and the stabilizer as $\mathfrak{su}(3)$.
\texttt{Algebra/TrialityInterface} certifies six theorems (G1--G6) for the
Spin(8) triality skeleton: $Z(\mathrm{Spin}(8)) = V_4$, the permutation action of
the triality group $S_3 = \langle\rho,\sigma\rangle$ on the three vector/spinor
slots, inner vs.\ outer classification of the two $\mathbb{Z}_3$ actions, and the
faithful $S_3$ action on the set of three generation kink species.
\texttt{Algebra/OctonionColorFlavorDisambiguation} certifies that the $\mathbb{Z}_3$
subgroup of $F_{21}$ acts in two inequivalent representations: as \emph{color
rotation} ($\times 2$ on $\mathbb{Z}_7$, an inner $G_2$ automorphism) and as
\emph{flavor cycling} (phase shift $\theta \mapsto \theta + 2\pi/3$ on the Koide
cone, an outer Spin(8) triality action); neither is formally identified with the
other, making the inner/outer split manifest.
\texttt{Algebra/KinkSigmaParityAction} certifies that the outer triality involution
$\sigma$ acts on the four GTE orbit kink labels as $\mathbb{Z}_7$ charge conjugation,
swapping $Q_\varphi = 4$ and $Q_\varphi = 3$, and that this permutation is not
realized by any non-identity $\mathrm{Aut}(F_{21})$ element.
\texttt{Algebra/KinkSectorTrialityAction} and \texttt{Spacetime/PhiMDLZeroModeIndex}
complete the kink sector analysis: the former certifies that $S_3$ acts faithfully
on $\{\mathrm{gen}_1, \mathrm{gen}_2, \mathrm{gen}_3\}$ as a degree-3 permutation
representation (zero JR zero-mode analysis required); the latter certifies the
Callias boundary-value theorem for the $\mathbb{Z}_7$-invariant cosine Yukawa profile
$f(\varphi) = \cos(7\varphi)$, showing that the boundary Dirac masses agree at both
asymptotic kink ends (same sign), so the spectral-flow index vanishes, blocking JR
zero modes for Case~A; no normalizable zero modes arise and fermionic statistics
follow from triple exchange.
\texttt{Algebra/BraidAtlasPhaseEquivariance} certifies that the uniform-triple
exchange phase $P(g,g,g)$ is \emph{not} $S_3$-equivariant: the sector permutation
$\sigma$ acts as $\mathbb{Z}_7$ charge conjugation on windings, mapping the fermionic
charged-lepton sector ($Q_\varphi = 4$, phase $-1$) to the bosonic $W^+$ sector
($Q_\varphi = 3$, phase $+1$); the phase structure carries only a $\mathbb{Z}_2$
symmetry $\{e, \sigma\rho^2\}$.
\texttt{MassRelations/SeesawTrialityPinning} certifies: RH braid seeds
$b_{R,1} = 5$, $b_{R,2} = 11$, $b_{R,3} = 19$ (strict ordering $5 < 11 < 19$
by \texttt{decide}); the unique Eisenstein norm among the seeds is $b_{R,3} = 19 =
N(-5-3\omega)$, paralleling the Eisenstein selection of $b_{\rm gen1} = 73$ in the
charged-lepton sector; and strict monotonicity of $m(b) = C \cdot b^{29/9}$ implies
normal mass ordering $m_{\nu,1} < m_{\nu,2} < m_{\nu,3}$ conditional on the corpus
seesaw formula.
All ten modules carry zero \texttt{sorry} and zero custom axioms; the only cited
external input is the Cartan--Killing classification theorem
(see Assumption~C9 in the premise ledger).
...
% ── Computational Certificate Generators ──────────────────────
\clearpage
\section*{Appendix: Computational Certificate Generators}

Some theorems in \texttt{UgpLean} rely on rational bounds and \texttt{norm\_num}
witnesses computed by Python scripts in \texttt{scripts/}.  The table below lists
each generator, the \lean{} module(s) it produces, and its certificate type.

\begin{center}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{llp{3.2cm}p{3.5cm}}
\toprule
\textbf{Script} & \textbf{Lean module(s)} & \textbf{Certificate type} & \textbf{Regenerate} \\
\midrule
\texttt{scripts/gen\_ucl\_mass\_ordering\_sbounds.py}
  & \texttt{UCLMassOrderingSBounds.lean},\newline\texttt{UCLMassOrderingDelta.lean}
  & Rational bounds for UCL mass-ordering coupled-corner deltas; \texttt{norm\_num} witnesses
  & \texttt{python3 scripts/gen\_ucl\_mass\_ordering\_sbounds.py} \\[4pt]
\texttt{scripts/gen\_ucl\_mass\_ordering\_certs.py}
  & \texttt{UCLMassOrderingSBounds.lean},\newline\texttt{UCLMassOrderingCerts.lean}
  & Prior UCL iteration; superseded by \texttt{gen\_ucl\_mass\_ordering\_sbounds.py}
  & (retained for reference) \\[4pt]
\texttt{scripts/gen\_ucl\_mass\_ordering\_delta.py}
  & \texttt{UCLMassOrderingDelta.lean}
  & Log-delta bounds for UCL coefficients; \texttt{norm\_num}
  & \texttt{python3 scripts/gen\_ucl\_mass\_ordering\_delta.py} \\[4pt]
\texttt{scripts/gen\_ucl\_mass\_ordering\_interval.py}
  & \texttt{UCLMassOrderingInterval.lean}
  & Log-bound interval lemmas; \texttt{norm\_num}
  & \texttt{python3 scripts/gen\_ucl\_mass\_ordering\_interval.py} \\[4pt]
\texttt{scripts/gen\_sech\_overlap\_bounds.py}
  & \texttt{SechOverlapIntegralBounds\_gen.lean} (placeholder)
  & Placeholder scaffold for sech overlap; main bounds hand-certified in \texttt{SechOverlapIntegralBounds.lean}
  & (scaffold only; see hand-derived section) \\[4pt]
\texttt{scripts/gen\_sech5\_mesh\_proof.py}
  & \texttt{SechOverlapIntegralBounds\_r5mesh.lean} (removed 2026-09-13)
  & $r{=}5$ half-line mesh with per-point Taylor cosh proofs; never reached zero \texttt{linarith} failures (25{,}461 lines, \texttt{lake build} still fails at dozens of mesh points as of removal) — abandoned in favor of the \CatA{} axiom route in \texttt{SechOverlapIntegralBounds\_bridge.lean}; recoverable from git history if a future pass wants to complete the antitone-Riemann-sum-vs-integral bridge for real
  & (removed; see git history pre-2026-09-13) \\[4pt]
\texttt{scripts/gen\_sech5\_cosh\_bins.py}
  & \texttt{SechOverlapIntegralBounds\_r5bins.lean} (removed 2026-09-13)
  & 990 per-bin cosh bound lemmas for $r{=}5$; zero sorry, but its only consumer (\texttt{\_r5mesh.lean}) never compiled, so it was removed alongside it
  & (removed; see git history pre-2026-09-13) \\[4pt]
\texttt{scripts/gen\_sech11\_mesh\_proof.py}
  & \texttt{SechOverlapIntegralBounds\_r11cert.lean}
  & $r{=}11$ mesh certification ($N{=}40000$); not yet run to completion (axiom used instead)
  & \texttt{python3 scripts/gen\_sech11\_mesh\_proof.py} \\[4pt]
\texttt{scripts/gen\_inj.py}
  & \texttt{inj\_proof\_gen.lean} (intermediate)
  & Injectivity block for 20-neighbor periodic causal graph
  & Called by \texttt{gen\_degree\_block.py} \\[4pt]
\texttt{scripts/gen\_degree\_block.py}
  & \texttt{UgpLean/Spacetime/SpectralDimensionDegree.lean}
  & Periodic-successor degree proof block; calls \texttt{gen\_inj.py}
  & \texttt{python3 scripts/gen\_degree\_block.py} \\
\bottomrule
\end{tabular}%
}%
\end{center}

\subsection*{Hand-derived certificates (no Python generator)}

The following bounds were derived analytically and hand-written directly in \lean{}:

\begin{itemize}
\item \texttt{UCLLogBounds.lean} ---
  $\log\varphi$, $\log(2\pi)$, $\sqrt{5}$, and golden-ratio rational certificates
  following the Mathlib \texttt{Real.pi\_gt\_d*} / Taylor \texttt{exp\_bound'} pattern;
  hand-written, not script-generated.

\item \texttt{SechOverlapIntegralBounds\_cosh.lean} (removed 2026-09-13) ---
  \texttt{cosh\_two\_le} through \texttt{cosh\_five\_le} proved by hand using
  \texttt{exp\_bound'} and \texttt{exp\_one\_lt\_d9} combined with \texttt{cosh\_eq};
  not generated by any script; zero sorry, but removed alongside \texttt{\_r5mesh.lean}
  and \texttt{\_r5bins.lean} since its only consumer (\texttt{\_r5bins.lean}) had no
  remaining purpose once \texttt{\_r5mesh.lean} was removed. Recoverable from git
  history pre-2026-09-13.

\item \texttt{SechOverlapIntegralBounds\_bridge.lean} ---
  two \CatA{} axioms (\texttt{sech\_overlap\_at\_five\_ge\_certified},
  \texttt{sech\_overlap\_at\_eleven\_ge\_certified}) documenting the
  mesh\textrightarrow{}integral bridge; the aggregate bounds $596903/10^6$ and $282771/10^6$
  were confirmed numerically by \texttt{eta\_b\_exact\_sech\_overlap.py}.  The main
  \texttt{SechOverlapIntegralBounds.lean} and \texttt{PhiMDLFluctuationSpectrum.lean}
  are zero sorry; a full Riemann-sum interval certificate remains open (CatAL pending Mathlib interval arithmetic support).

\item \texttt{SechOverlapIntegralBounds.lean} (per-cell product lemmas) ---
  generated using \texttt{gen\_sech\_overlap\_bounds.py}; all bounds
  hand-certified in the main module.
\end{itemize}

\subsection*{Native-decide and norm-num only (no external script)}

The following theorems use \lean{}'s \texttt{native\_decide} or \texttt{norm\_num}
directly, with no external Python generator:

\begin{itemize}
\item \texttt{GUTStructure.lean} --- combinatorial scan of GUT-compatible charge
  assignments, CKM arithmetic, Mersenne structure (\texttt{native\_decide}).
\item \texttt{NeutrinoSector.lean} --- PMNS orbit ratio scan (\texttt{native\_decide}).
\item \texttt{TE22/ScanCertificate.lean} --- 34,560-universe PSC enumeration
  (\texttt{native\_decide}).
\item Core classification modules (\texttt{TheoremA.lean}, \texttt{TheoremB.lean},
  \texttt{Bounds.lean}, \texttt{FormalRSUC.lean}) --- all \texttt{native\_decide}.
\end{itemize}
