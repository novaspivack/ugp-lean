...
% Suite §2.8-style overfull tolerance: set AFTER \sloppy since \sloppy resets \hfuzz to 0.5pt.
\hfuzz=30pt
...
framework in \lean{}~4 against Mathlib, comprising 118 modules with a
...
A \textbf{MassRelations} layer (25 modules) formalizes the
...
and the seesaw mass-squared ratio arithmetic chain: FN texture $(q_1,q_2)=(3,2)$
determines exponent $29/9$, $M_R$ cancels algebraically, tight bound
$|R - 0.02936| < 0.0001$ and within $1\%$ of NuFIT~6.0 certified via
unit-width integer 9th-power comparisons (\texttt{NeutrinoMassRatio}, 2026-05-16,
...
\item \textbf{Mass relations and physical spectrum} (\S\ref{sec:massrelations}):
  The \texttt{MassRelations} layer (25 modules) covers
  ...
  (viii) the CDM Cabibbo derivation: effective Cabibbo FN charge
  ...
  and (ix) the seesaw mass-squared ratio arithmetic chain: FN texture
  $(q_1,q_2)=(3,2)$ determines exponent $29/9$, $M_R$ cancels algebraically,
  tight bound $|R - 0.02936| < 0.0001$ and within $1\%$ of NuFIT~6.0 certified
  zero sorry (5 theorems; \texttt{NeutrinoMassRatio}, 2026-05-16).
\end{itemize}
...
The artifact is organized into 118~\lean{} modules across twelve layers
...
\node[layer, fill=teal!8,   below=of struc] (mass)
  {\textbf{MassRelations}~(25)\quad \textit{MassRelations} [umbrella], KoideClosedForm, KoideNewtonFlow,
   ...
   CKMTheta23, KoideS3DiscreteIdentities, CKMMixing, \textbf{NeutrinoMassRatio}};
...
\texttt{Phase4.\allowbreak GaloisProtection} (one-loop non-renormalization),
\texttt{Phase4.\allowbreak TwoLoopCoefficient} (two-loop color coefficient $= 8/9$),
\texttt{GaloisStructure.\allowbreak CyclotomicLayers} (Galois-stability of UGP layers in
$\mathbb{Q}(\zeta_{120})$), \texttt{GaloisStructure.\allowbreak MinimalCyclotomic}
($\mathbb{Q}(\zeta_{120})$ minimality), and
\texttt{CyclotomicCompleteness.\allowbreak CoxeterBiconditional} (Coxeter-conductor
biconditional; see \S\ref{sec:coxeter_biconditional}), and
\texttt{CyclotomicCompleteness.\allowbreak CyclotomicContainment} (field-theoretic embedding
...
Two theorems in \texttt{GTE.\allowbreak AnalyticArchitecture} are declared as
...
\begin{sloppypar}
\medskip\noindent\textbf{One additional disclosed axiom (\texttt{BraidAtlas.\allowbreak ChargeTheorem}, 2026-05-08):}
...
\subsection[Mersenne ladder and Fibonacci uniqueness]{Mersenne ladder and Fibonacci uniqueness (\texttt{MersenneLadder})}\label{subsec:mersenne-ladder}
...
\subsection[Fiber bundle over GTE triples]{Fiber bundle over GTE triples (\texttt{FiberBundle})}\label{subsec:fiberbundle}
...
\subsection[Orbital stability and linear response]{Orbital stability and linear response (\texttt{LinearResponse})}\label{subsec:linear-response}
...
\subsection[Ridge-level scale connection]{Ridge-level scale connection (\texttt{ScaleConnection})}\label{subsec:scaleconnect}
...
% ═══════════════════════════════════════════════════════════════
\subsection[SM Winding Numbers from $N_c$]{SM Winding Numbers from $N_c$ (\texttt{BraidAtlas.ChargeDerivation})}
...
% ═══════════════════════════════════════════════════════════════
\subsection[Coxeter--Conductor Theorem]{Coxeter--Conductor Theorem (\texttt{BraidAtlas.\allowbreak CoxeterConductor}, \texttt{BraidAtlas.\allowbreak CoxeterConductorTowerLaw})}
\label{sec:coxeter_conductor}
% ═══════════════════════════════════════════════════════════════

\begin{sloppypar}
The pair of modules \texttt{BraidAtlas.\allowbreak CoxeterConductor} and
\texttt{BraidAtlas.\allowbreak CoxeterConductorTowerLaw} certify the arithmetic core
...
irreducibility from \texttt{Polynomial.\allowbreak irreducible\_of\_degree\_le\_three\_of\_not\_isRoot}.
...
% ═══════════════════════════════════════════════════════════════
\subsection[Coxeter--Conductor Biconditional]{Coxeter--Conductor Biconditional (\texttt{CoxeterBiconditional})}
...
\texttt{CyclotomicCompleteness.\allowbreak CyclotomicContainment}
(see \S\ref{sec:cyclotomic_containment}).
\end{sloppypar}
...
\texttt{BraidAtlas.\allowbreak CoxeterConductorTowerLaw}: divisibility fails at both the
...
% ═══════════════════════════════════════════════════════════════
\subsection[Cyclotomic field embedding]{Cyclotomic field embedding (\texttt{CyclotomicContainment})}
\label{sec:cyclotomic_containment}
% ═══════════════════════════════════════════════════════════════

\begin{sloppypar}
The module \texttt{CyclotomicCompleteness.\allowbreak CyclotomicContainment} (SPEC\_042\_CYX
...
% ═══════════════════════════════════════════════════════════════
\subsection[Electroweak boson c-values]{Electroweak boson c-values (\texttt{BraidAtlas.EWBosons})}
...
% ═══════════════════════════════════════════════════════════════
\subsection[Chirality squaring]{Chirality squaring (\texttt{BraidAtlas.ChiralitySquaring})}
...
% ═══════════════════════════════════════════════════════════════
\section[RCC Infinite Families]{RCC Infinite Families (\texttt{PSC.RCCInfiniteFamilies})}
...
% ═══════════════════════════════════════════════════════════════
\section[PSC Three-Route Forcing Capstone]{PSC Three-Route Forcing Capstone (\texttt{PSC.ThreeRouteForcing})}
...
% ═══════════════════════════════════════════════════════════════
\section[Neutrino FN Texture Selection]{Neutrino FN Texture Selection (\texttt{NeutrinoFroggattNielsen})}
...
% ═══════════════════════════════════════════════════════════════
\section[Galois-Protection Non-Renormalization]{Galois-Protection Non-Renormalization\newline
(\texttt{Phase4.\allowbreak GaloisProtection})}
\label{sec:galois_protection}
% ═══════════════════════════════════════════════════════════════

\begin{sloppypar}
The module \texttt{Phase4.\allowbreak GaloisProtection} formalises the
...
% ═══════════════════════════════════════════════════════════════
\section[Two-Loop Color Coefficient]{Two-Loop Color Coefficient (\texttt{Phase4.TwoLoopCoefficient})}
\label{sec:two_loop}
% ═══════════════════════════════════════════════════════════════
...
\subsection[Algebraic saturation barrier]{Algebraic saturation barrier (\texttt{SaturationBarrier})}\label{subsec:saturation}
...
\subsection[Theorem eligibility criterion]{Theorem eligibility criterion (\texttt{TheoremEligibility})}\label{subsec:tec}
...
\begin{align*}
  \frac{\ln 2}{N_{\rm univ}}
    &= \frac{\ln\varphi}{\ln\varphi + 2\ln(2\pi)}, \\
  1 - \frac{\ln 2}{N_{\rm univ}}
    &= \frac{2\ln(2\pi)}{\ln\varphi + 2\ln(2\pi)}, \\
  \frac{1}{1 - \ln 2/N_{\rm univ}}
    &= \frac{\ln\varphi + 2\ln(2\pi)}{2\ln(2\pi)}
     = 1 + \frac{\ln\varphi}{2\ln(2\pi)}
     = \mathrm{IPT}.\;\checkmark
\end{align*}
...
% ═══════════════════════════════════════════════════════════════
\subsection[Galois Layer Stability]{Galois Layer Stability (\texttt{GaloisStructure.CyclotomicLayers})}
\label{subsec:galois}
% ═══════════════════════════════════════════════════════════════
...
The \texttt{MassRelations} layer (25 modules, including the
...
\subsection[Colour-rank structural chain]{Colour-rank structural chain (\texttt{KoideAngle})}\label{subsec:koideangle}
...
\subsection[Seesaw index as GUT representation defect]{Seesaw index as GUT representation defect (\texttt{SeesawIndex})}\label{subsec:seesawindex}
...
\subsection[Mass ratio Z-independence]{Mass ratio Z-independence (\texttt{ScaleTransport})}\label{subsec:scaletransport}
...
\subsection[CKM $\theta_{23}$ Structural Ratio]{CKM \texorpdfstring{$\theta_{23}$}{theta23} Structural Ratio (\texttt{MassRelations.CKMTheta23})}
...
\subsection[Koide $S_3$ Discrete Arithmetic-Mean Identity]{Koide \texorpdfstring{$S_3$}{S3} Discrete Arithmetic-Mean Identity (\texttt{MassRelations.KoideS3DiscreteIdentities})}
\label{sec:koide_s3_discrete}

\begin{sloppypar}
The module \texttt{MassRelations.\allowbreak KoideS3DiscreteIdentities} certifies the discrete
...
% ═══════════════════════════════════════════════════════════════
\subsection[CDM Cabibbo Derivation]{CDM Cabibbo Derivation (\texttt{MassRelations.CKMMixing})}\label{sec:ckmmixing}
% ═══════════════════════════════════════════════════════════════
...
but not yet formalized in Lean.  The formal gap is the 2$\times$2 FN SVD diagonalization theorem
$|(U_{uL}^\dagger U_{dL})_{12}| = \varepsilon_1^{\alpha_d}(1 + O(\varepsilon_1^2))$;
this is left for a companion \texttt{CKMMixing.SVDDiagonalization} module.

% ═══════════════════════════════════════════════════════════════
\subsection[Neutrino Mass Ratio Arithmetic]{Neutrino Mass-Squared Ratio (\texttt{NeutrinoMassRatio})}\label{sec:neutrino_mass_ratio}
% ═══════════════════════════════════════════════════════════════

The module \texttt{MassRelations.NeutrinoMassRatio} (2026-05-16, zero sorry)
formalizes the arithmetic/algebraic chain from the FN texture to the
neutrino mass-squared ratio prediction.

\paragraph{The ratio.}
The UGP seesaw exponent $\alpha = 29/9$ and Braid Atlas RH-neutrino
b-values $\{5, 11, 19\}$ predict the mass-squared ratio
\[
  R \;=\; \frac{\Delta m^2_{21}}{\Delta m^2_{31}}
    \;=\; \frac{11^{58/9} - 5^{58/9}}{19^{58/9} - 5^{58/9}}
    \;\approx\; 0.02936.
\]
NuFIT~6.0 reports $0.02951 \pm 0.00098$ ($0.16\sigma$, $0.52\%$ agreement).

\begin{theorem}[Neutrino mass ratio, zero sorry]\label{thm:neutrino_mass_ratio}
Five theorems in \texttt{MassRelations.NeutrinoMassRatio} are machine-checked
with zero \texttt{sorry}:
\begin{itemize}
  \item \lstinline|fn_texture_gives_seesaw_exponent|:
    The FN charge pair $(q_1,q_2) = (3,2)$ satisfies
    $q_1 + q_2/N_c^2 = 3 + 2/9 = 29/9 = \mathtt{nuSeesawExponent}$.
  \item \lstinline|seesaw_ratio_independent_of_MR|:
    If $m_i = C \cdot x_i$ (common prefactor), then
    $(m_2^2 - m_1^2)/(m_3^2 - m_1^2) = (x_2^2 - x_1^2)/(x_3^2 - x_1^2)$
    (independent of $C$, hence independent of $M_R$).
  \item \lstinline|neutrino_mass_ratio_coarse_bound|:
    $0.029 < R < 0.030$, certified via monotone 9th-power integer comparisons
    and \lstinline|norm_num|.
  \item \lstinline|neutrino_mass_ratio_tight_bound|:
    $|R - 0.02936| < 0.0001$, proved via unit-width integer bounds
    $31950 < 5^{58/9} < 31951$, $5142772 < 11^{58/9} < 5142773$,
    $174123159 < 19^{58/9} < 174123160$ (all certified by \lstinline|norm_num|).
  \item \lstinline|neutrino_mass_ratio_within_1pct_of_nufit|:
    $|R - 0.02951| < 0.01 \times 0.02951$; $R$ is within $1\%$ of the
    NuFIT~6.0 central value $0.02951$.
\end{itemize}
\end{theorem}

\begin{leanref}
\lstinline|fn_texture_gives_seesaw_exponent|,
\lstinline|seesaw_ratio_independent_of_MR|,
\lstinline|neutrino_mass_ratio_coarse_bound|,
\lstinline|neutrino_mass_ratio_tight_bound|,
\lstinline|neutrino_mass_ratio_within_1pct_of_nufit|
\textup{(in \texttt{MassRelations.NeutrinoMassRatio}, zero \texttt{sorry})}
\end{leanref}

\paragraph{Tight-bound proof.}
The tight bound $|R - 0.02936| < 0.0001$ is machine-checked in Lean via
unit-width integer bounds on each $b^{58/9}$, reducing to \lstinline|norm_num|
evaluations of exact 74--digit integers.
The full Lagrangian bridge --- formal derivation that Yukawa couplings
scale as $b^{q_1}$ and $b^{q_2}$ from the FN texture --- remains open
and requires a companion \texttt{NeutrinoLagrangian.lean} module.
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{4.2cm}>{\raggedright\arraybackslash}p{5.2cm}>{\raggedright\arraybackslash}p{4.2cm}@{}}
\toprule
...
the CDM Cabibbo derivation $|V_{us}|_{\rm CDM} = \varepsilon_1^{\alpha_d}
= e^{-13\pi/27} \approx 0.2203$ from GUT rank theory
(\texttt{CKMMixing}, 2026-05-11, 20 theorems, zero sorry),
and the seesaw mass-squared ratio arithmetic chain: FN texture $(q_1,q_2)=(3,2)$
gives exponent $29/9$, $M_R$ cancels algebraically, tight bound
$|R - 0.02936| < 0.0001$ and within $1\%$ of NuFIT~6.0 certified
(\texttt{NeutrinoMassRatio}, 2026-05-16, 5 theorems, zero sorry).
