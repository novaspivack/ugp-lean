...
private lemma discrete_evolution_smul_commute
    (H : Matrix (Fin N) (Fin N) ℂ) (τ_c : ℝ) (m n : ℕ) :
    Commute (-(I * (↑m : ℂ) * ↑τ_c) • H) (-(I * (↑n : ℂ) * ↑τ_c) • H) := by
  rw [Commute.symm_eq, commute_smul_smul, Commute.refl]
...
theorem discrete_schrodinger_composition
  ...
  have hcomm := discrete_evolution_smul_commute H τ_c m n
  have hadd :
      (-(I * (↑(m + n) : ℂ) * ↑τ_c) • H) =
        (-(I * (↑m : ℂ) * ↑τ_c) • H + -(I * (↑n : ℂ) * ↑τ_c) • H) := by
    simp [Nat.cast_add, mul_add, add_mul, neg_mul, mul_neg, neg_add_rev, smul_add]
...
theorem discrete_evolution_unitary
  ...
  simp only [discrete_evolution]
  have hz :
      star (-(I * (↑n : ℂ) * ↑τ_c)) = (I * (↑n : ℂ) * ↑τ_c) := by
    simp [star_neg, star_mul, star_ofReal, star_I]
  have hconj :
      (-(I * (↑n : ℂ) * ↑τ_c) • H)ᴴ = (I * (↑n : ℂ) * ↑τ_c) • H := by
    simp [conjTranspose_smul, hH.eq, hz]
  rw [Matrix.exp_conjTranspose, hconj]
  have hcomm :
      Commute ((I * (↑n : ℂ) * ↑τ_c) • H) (-(I * (↑n : ℂ) * ↑τ_c) • H) := by
    rw [Commute.symm_eq, commute_smul_smul, Commute.refl]
  rw [← neg_add, ← smul_add, add_neg_cancel, zero_smul, Matrix.exp_add_of_commute _ _ hcomm,
    Matrix.exp_zero]
...
axiom f_mdl_cycle_hamiltonian_vacuum :
    ∃ (H_cyc : Matrix (Fin (7^5)) (Fin (7^5)) ℂ),
      H_cyc.IsHermitian ∧
      H_cyc *ᵥ (Pi.single (0 : Fin (7^5)) 1) = 0
...
private lemma chiralPositive_deriv
  ...
  have hsub : HasDerivAt (fun u => φ u - Complex.I * deriv φ u / (ω : ℂ))
      (deriv φ t - Complex.I * deriv (deriv φ) t / (ω : ℂ)) t :=
    hφ.sub hdiv
  exact hsub.div_const 2
...
private lemma chiralNegative_deriv
  ...
  have hdiv :
      HasDerivAt (fun u => Complex.I * deriv φ u / (ω : ℂ))
        (Complex.I * deriv (deriv φ) t / (ω : ℂ)) t :=
    (hφ'.const_mul Complex.I).div_const (ω : ℂ)
  have hadd : HasDerivAt (fun u => φ u + Complex.I * deriv φ u / (ω : ℂ))
      (deriv φ t + Complex.I * deriv (deriv φ) t / (ω : ℂ)) t :=
    hφ.add hdiv
...
private lemma kg_second_deriv
    ...
    deriv (deriv φ) t = -(ω : ℂ)^2 * φ t :=
  eq_neg_of_add_eq_zero (h_kg t)
...
theorem kg_factors_to_chiral_schrodinger (ω : ℝ) (φ : ℝ → ℂ)
  ...
  · exact ⟨fun t => φ t / 2, fun t => φ t / 2,
      ...
      fun t => by simp [hω],
      fun t => by ring⟩
  · refine ⟨chiralPositive ω φ, chiralNegative ω φ, ?_, ?_, ?_⟩
    · intro t
      have hderiv := (chiralPositive_deriv ω φ t (hφ t) (hφ' t)).deriv
      have hkg := kg_second_deriv ω φ t h_kg
      simp only [chiralPositive, hderiv]
      field_simp [hω]
      ring_nf
      rw [hkg]
      ring
    · intro t
      have hderiv := (chiralNegative_deriv ω φ t (hφ t) (hφ' t)).deriv
      have hkg := kg_second_deriv ω φ t h_kg
      simp only [chiralNegative, hderiv]
      field_simp [hω]
      ring_nf
      rw [hkg]
      ring
    · intro t
      simp only [chiralPositive, chiralNegative]
      field_simp [hω]
      ring
