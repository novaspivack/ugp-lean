...
the Particle Derivations repo at `braid_to_gte_mapping_search/braid_to_gte_mapper.py`, verified 12/12 fermions) produces GTE triples `(a, b, c)` where
...
* re-fit the 9 existing Lean-certified UCL coefficients in a 10-feature
 setting (separate structural research question),
