...
- `UgpLean.MassRelations.PhysicalMasses` — end-to-end physical-mass bridge. Given (m_e, m_μ) as input, defines `predictedLepton`, `predictedUpType`, `predictedDownType` via TT+VV+Koide closed form; proves TT formula, VV formula, and Koide identity hold by construction on the predicted physical masses; proves positivity of all predicted masses; proves predicted lepton vector is fixed point of the Newton-step Koide flow. This module upgrades the previously `True → trivial` FormulaHolds placeholders to real theorems on Lean-valued physical-mass predictions, closing the E_base Lean bridge except for the residual m_μ anchor
